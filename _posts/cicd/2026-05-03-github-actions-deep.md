...
```mermaid
sequenceDiagram
    Runner_Pool->>Runner_VM: Runner 할당
    Runner_VM->>Runner_VM: clone→실행
    Runner_VM->>GitHub: 결과/로그 전송
    Runner_VM->>Runner_Pool: VM 폐기
```
...
```mermaid
graph TB
    PR["외부 PR (악성 코드)"] -->|"Workflow 트리거"| RUNNER["Self-hosted Runner"]
    RUNNER --> RISK["시크릿 탈취 / 백도어 / 네트워"]
    DEF["방어: 포크 PR 승인 후 실행,"]
    RUNNER -.->|"방어"| DEF
```
...
```mermaid
graph LR
    PUSH["코드 Push"] --> TEST["1️⃣ Test<br/>(Grad"]
    TEST --> BUILD["2️⃣ Build<br/>(Doc"]
    BUILD --> STAGING["3️⃣ Deploy Staging"]
    STAGING --> SMOKE["4️⃣ Smoke Test"]
    SMOKE --> APPROVE{"5️⃣ 수동 승인"}
    APPROVE -->|"승인"| PROD["6️⃣ Deploy Product"]
    APPROVE -->|"거부"| STOP["중단"]
