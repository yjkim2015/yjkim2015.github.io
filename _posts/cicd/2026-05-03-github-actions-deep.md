...
```mermaid
graph LR
    GH["GitHub"] -->|"Job 큐 추가"| POOL["Runner Po..|"Runner 할당"| VM["Runner VM"]..|"clone→실행"| VM
    VM -->|"결과/로그 전송"| GH
    VM -->|"VM 폐기"| POOL
```
...
```mermaid
graph LR
    PR["외부 PR (악성 코드)"] -->|"Workflow 트리거"| RUNNER["Self-ho..|"방어"| DEF
```
...
graph LR
    ...
    APPROVE -->|"승인"| PROD["6️⃣ Deplo..|"거부"| STOP["중단"]
...
graph LR
    ...
    ORG --> REPO --> ENV
    ORG ---|"예: SONAR_TOKEN,..| ORG
    REPO ---|"예: DOCKER_PASSW..| REPO
    ENV ---|"예: KUBE_CONFIG_..| ENV
