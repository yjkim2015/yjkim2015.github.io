...
```mermaid
graph LR
    DEV1["개발자"] -->|"Push"| CI1["CI Svr"]
 ..|"kubectl apply"| K8S1["클러스터(Push..|"Push"| GIT["Git Save소"..|"감시"| GIT
    AGENT -->|"동기화"| K8S2["클러스터(Pull)"]
...
```mermaid
graph LR
    USER["사용자"] -->|"앱 등록/조회"| API["API Server..|"Git Clone"| GIT["Git Save소"..|"매니페스트"| CTRL["App Contr..|"상태 조회"| K8S["Kubernetes..|"Sync"| K8S
```
...
graph LR
    ...
    ARGO -->|"Auto"| APPLY["배포"]
   ..|"Manual"| APPROVE["승인 후 배..|"실패"| ALERT["알림"]
   ..|"성공"| DONE["완료"]
```
...
graph LR
    ...
    Pro..|"CrashLoopBackOff"| Degraded
    Pr..|"HPA/PDB 중단"| Suspended
    D..|"재시작 성공"| Progressing
   ..|"리소스 삭제됨"| Missing
    Mis..|"재생성"| Progressing
```
