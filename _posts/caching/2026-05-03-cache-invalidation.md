...
sequenceDiagram
    ...
    participant Redis
    participant DB
    App->>Redis: GET product:42:version → v3
    App->>Redis: GET product:42:v3 → Miss
    App->>DB: SELECT
    DB-->>App: 데이터
    App->>Redis: SET product:42:v3 (TTL 1h)
    App->>DB: UPDATE
    App->>Redis: INCR version → v4
    App->>Redis: GET product:42:version → v4
    App->>Redis: GET product:42:v4 → Miss, DB 조회 후 저장
```
...
sequenceDiagram
    ...
    participant T2 as Thread2~100
    participant R as Redis
    participant DB as DB
    Note over T1,DB: Cache Miss 동시 발생
    T1->>R: SET lock NX EX 5 → OK
    T2->>R: SET lock NX EX 5 → FAIL
    T1->>DB: SELECT product:42
    DB-->>T1: 데이터 반환
    T1->>R: SET cache + DEL lock
    T2->>R: GET product:42 → 캐시 Hit
```
