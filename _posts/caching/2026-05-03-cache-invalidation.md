...
TTL 짧음 → 신선도 높음, Cache Miss 빈번 / TTL 긺 → Cache Hit 높음, Stale Data 위험 / 적정 TTL → 데이터 성격에 따라 신선도와 성능 균형
...
```mermaid
graph LR
    T1["Thread1"] -->|"SET lock NX→OK"| R["Redis"]
    T2["Thread2"] -->|"SET lock NX→FAIL"| R
    T1 -->|"DB조회 후 캐시저장"| R
    T1 -->|"DEL lock"| R
    T2 -->|"GET cache→Hit"| R
```
...
TTL 9분 → TTL 5분 → TTL 1분 → TTL 10초 (만료에 가까울수록 갱신 확률 급등)
...
graph LR
    S1["서버A"] -->|"DB 업데이트"| S1
    S1 -->|"PUBLISH invalidat"| Redis["Redis"]
    Redis -->|"L1 삭제"| S1
    Redis -->|"L1 삭제"| S2["서버B/C"]
```
...
캐시 무효화 전략: 시간 기반(TTL) / 이벤트 기반(@CacheEvict, Pub/Sub) / 버전 기반(키에 버전 포함)
