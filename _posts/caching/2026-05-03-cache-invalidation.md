...
```mermaid
graph LR
    A["TTL 짧음"] -->|"신선도 높음"| B["Cache Miss 빈..|"Cache Hit 높음"| D["Stale Data 위..|"데이터 성격에 따라"| F["신선도와 성능"]
```
...
..|"SELECT"| DB["DB"]
    Ap..|"UPDATE → INCR v4"| Redis
    App -->|"GET v4 → Miss →..| Redis
```
...
graph LR
    S1["서버A"] -->|"DB 업데이트"| S1
    S1 -->|"PUBLISH invalid..| Redis["Redis"]
    Redis -->|"L1 삭제"| S1
...
graph LR
   ...
   ..|"binlog"| Debezium["Debez..|"변경 이벤트"| Kafka["Kafka"]
..|"구독"| Worker["캐시 무효화"..|"DEL"| Redis["Redis"]
..|"evict"| L1["각 서버 L1"]
```
