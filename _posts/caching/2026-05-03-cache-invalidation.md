...
sequenceDiagram
    App->>DB: SELECT
    App->>Redis: UPDATE → INCR v4
    App->>Redis: GET v4 → Miss → 저장
```
...
```mermaid
sequenceDiagram
    Thread1->>Redis: DB조회 후 캐시저장
    Thread1->>Redis: DEL lock
    Thread2->>Redis: GET cache→Hit
```
