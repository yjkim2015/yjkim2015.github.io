...
```mermaid
graph LR
    EE[실행 엔진] -->|"인덱스 idx_user_id..| InnoDB["InnoDB (Handler AP"]
...
```mermaid
graph LR
    DISK[(Disk)] -->|"신규 로드"| OLD["Old Sublis..|"자주 접근"| NY["New/Young"]..|"Young 유지"| NY
```
...
graph LR
    ...
    REC --> TRX["DB_TRX_ID<br>마지막 수"]
    REC --> PTR["DB_ROLL_PTR<br>Und"]
    PTR --> UNDO[(Undo Log)]
```
...
## 왜 InnoDB 내부를 알아야 하는가?

버퍼 풀, B+Tree, WAL은 단순한 이론이 아니라 실무 튜닝의 직접적인 근거다. `innodb_buffer_pool_size`를 RAM의 70~80%로 설정하는 이유는 버퍼 풀이 디스크 I/O를 흡수하기 때문이고, `innodb_log_file_size`를 크게 설정하면 체크포인트 빈도가 줄어 쓰기 성능이 올라가는 이유는 WAL 구조를 알아야 이해된다. 내부 구조 이해 없이는 `EXPLAIN` 결과도 올바르게 해석하기 어렵다.

---

## 실무에서 자주 하는 실수

**실수 1: innodb_buffer_pool_size 기본값(128MB) 방치**
프로덕션 서버에서 기본값을 그대로 쓰면 대부분의 페이지가 디스크에서 읽힌다. 전용 DB 서버라면 RAM의 70~80%로 설정해야 한다. `SHOW STATUS LIKE 'Innodb_buffer_pool_read_requests'` 대비 `Innodb_buffer_pool_reads` 비율(미스율)이 1% 이상이면 버퍼 풀이 부족하다는 신호다.

**실수 2: redo log 크기 기본값 방치**
MySQL 5.7 기본 `innodb_log_file_size=48MB`는 대용량 쓰기 워크로드에서 체크포인트가 너무 자주 발생한다. 1~4GB로 늘리면 쓰기 처리량이 크게 개선된다. 변경 후 반드시 MySQL 재시작이 필요하다.

**실수 3: MVCC로 인한 언두 로그 폭증 방치**
장시간 실행되는 트랜잭션이 있으면 해당 시점 이후의 모든 변경 버전을 언두 로그로 보존해야 한다. `SHOW ENGINE INNODB STATUS`에서 `History list length`가 수만~수십만이면 퍼지(purge) 스레드가 따라가지 못하는 것이다. 장기 트랜잭션을 즉시 종료하고, `innodb_purge_threads`를 늘려야 한다.

**실수 4: 대량 INSERT 후 통계 업데이트 안 함**
수백만 건을 로드한 후 `ANALYZE TABLE`을 실행하지 않으면 옵티마이저가 오래된 통계로 잘못된 실행 계획을 선택한다. 대량 데이터 변경 후에는 항상 통계를 갱신해야 한다.

**실수 5: 페이지 크기(16KB)를 고려하지 않은 행 설계**
VARCHAR, TEXT, BLOB 컬럼이 많아 행이 16KB를 초과하면 오버플로우 페이지에 저장된다. 조회 시 추가 I/O가 발생한다. 자주 조회하는 컬럼과 드물게 조회하는 대용량 컬럼을 별도 테이블로 분리(수직 분할)하면 성능이 개선된다.

---

## 면접 포인트

**Q1. InnoDB의 MVCC는 어떻게 동작하는가?**
각 행은 시스템 변수(DB_TRX_ID, DB_ROLL_PTR)를 가진다. UPDATE 시 현재 행을 새 버전으로 교체하고, 이전 버전은 언두 로그에 체인으로 연결된다. 읽기 트랜잭션은 자신의 스냅샷 시점 이전 버전을 언두 로그를 따라 찾는다. 따라서 읽기가 쓰기를 블로킹하지 않는다.

**Q2. WAL이 없다면 어떤 문제가 생기는가?**
데이터 페이지를 직접 수정하면 커밋 시마다 16KB 페이지 전체를 디스크에 써야 한다(Random I/O). WAL은 변경 내용만 순차적으로 로그에 기록(Sequential I/O)하므로 훨씬 빠르다. 장애 시 WAL을 재실행해 복구(Redo)하거나 취소(Undo)할 수 있다.

**Q3. 더티 페이지(Dirty Page)란?**
버퍼 풀에서 수정되었지만 아직 디스크에 기록되지 않은 페이지다. 체크포인트 시점에 디스크로 내려쓴다(flush). 더티 페이지 비율이 너무 높으면 체크포인트가 급격히 발생해 I/O 스파이크가 생긴다. `innodb_max_dirty_pages_pct`(기본 75%)로 제어한다.

**Q4. B+Tree와 B-Tree의 차이는?**
B-Tree는 내부 노드에도 데이터를 저장한다. B+Tree는 내부 노드에 키만 저장하고 데이터는 리프 노드에만 있다. 리프 노드들이 연결 리스트로 이어져 범위 스캔이 효율적이다. InnoDB 클러스터드 인덱스가 B+Tree를 사용하는 이유다.

**Q5. 클러스터드 인덱스와 세컨더리 인덱스의 관계는?**
InnoDB에서 클러스터드 인덱스(기본 키)의 리프 노드에는 실제 행 데이터가 있다. 세컨더리 인덱스의 리프 노드에는 기본 키 값이 있다. 세컨더리 인덱스로 조회 시 기본 키를 얻고, 그 기본 키로 클러스터드 인덱스를 한 번 더 탐색(이중 탐색)한다. 커버링 인덱스로 이 비용을 피할 수 있다.
