...
### Q1. InnoDB WAL(Write-Ahead Log)이 왜 성능과 내구성을 동시에 보장하는가?

WAL의 핵심 원칙은 "데이터 파일 변경 전에 Redo Log를 먼저 기록한다"이다. 커밋 시 Redo Log만 순차(sequential) 쓰기하면 되므로 디스크 I/O가 빠르다. 실제 데이터 파일(`.ibd`)은 나중에 체크포인트 시점에 비동기로 쓴다. 크래시가 발생해도 Redo Log를 재실행하면 커밋된 트랜잭션 전체를 복구할 수 있다. 순차 쓰기는 랜덤 쓰기보다 HDD 기준 100배, SSD 기준 10배 빠르다. 이것이 높은 쓰기 처리량과 ACID 내구성을 동시에 달성하는 원리다.

### Q2. Buffer Pool의 LRU 리스트가 일반 LRU와 다른 점은? Young/Old 영역 분리의 이유는?

일반 LRU는 새 페이지를 리스트 맨 앞에 올린다. 풀 테이블 스캔처럼 대량 페이지를 한 번만 읽으면 기존에 자주 쓰던 핫 페이지가 모두 밀려난다. InnoDB의 LRU는 Young(5/8)과 Old(3/8) 두 영역으로 분리된다. 새 페이지는 Old 영역 헤드로 진입한다. `innodb_old_blocks_time`(기본 1초) 이상 Old 영역에 있다가 다시 접근하면 비로소 Young 영역 헤드로 이동한다. 풀 스캔으로 읽힌 일회성 페이지는 1초 안에 다시 접근되지 않으므로 Old 영역에서 퇴출된다. 자주 쓰는 핫 페이지를 보호하는 적응형 LRU다.

### Q3. MVCC에서 Long-running Transaction이 서비스 전체에 영향을 미치는 원리는?

InnoDB MVCC는 트랜잭션 시작 시 Read View를 생성하고, 그 시점 이후의 변경은 Undo Log를 따라가 이전 버전을 읽는다. 트랜잭션 A가 3시간째 열려 있으면, Purge Thread는 A의 Read View보다 오래된 Undo Log를 정리하지 못한다. 그동안 다른 트랜잭션들이 만든 Undo Log가 계속 쌓인다. History list length가 수백만으로 치솟고, 쿼리마다 Undo Chain을 길게 따라가므로 p99 레이턴시가 폭등한다. 해결은 `information_schema.INNODB_TRX`에서 장기 트랜잭션을 찾아 KILL하고, 배치 처리에 트랜잭션 범위를 청크 단위로 제한하는 것이다.

### Q4. Doublewrite Buffer가 Torn Page 문제를 해결하는 원리는? 성능에 미치는 영향은?

InnoDB 페이지는 16KB지만 OS 파일 시스템의 쓰기 단위는 4KB다. 16KB 페이지를 쓰는 중 4KB만 기록되고 크래시가 나면 Torn Page(부분 손상)가 발생한다. Redo Log로 복구하려면 원본 페이지가 온전해야 하는데, Torn Page는 원본 자체가 손상되었으므로 복구 불가다. Doublewrite Buffer는 데이터 파일 쓰기 전에 시스템 테이블스페이스의 연속 공간에 먼저 한 번 순차 쓰기한다. 크래시 후 복구 시 Doublewrite Buffer의 온전한 복사본으로 데이터 파일을 복원한 뒤 Redo Log를 적용한다. 성능 오버헤드는 약 5~10%지만 MySQL 8.0.20+에서는 별도 파일로 분리되어 경합이 줄었다.

### Q5. InnoDB와 MyISAM의 `COUNT(*)` 성능 차이가 나는 구조적 이유는?

MyISAM은 테이블의 전체 행 수를 메타데이터로 별도 저장한다. `COUNT(*)`는 헤더 파일 하나를 읽으면 O(1)로 반환된다. InnoDB는 MVCC로 인해 트랜잭션마다 "보이는 행"이 다르다. Read COMMITTED에서는 커밋된 최신 데이터를, REPEATABLE READ에서는 트랜잭션 시작 시점의 스냅샷을 기준으로 행을 센다. 어떤 행이 현재 트랜잭션에 보이는지 Undo Chain을 확인하며 실제로 세야 하므로 O(N)이다. 정확한 행 수가 자주 필요하다면 별도 카운터 테이블을 유지하거나 `information_schema.tables`의 근사값을 활용하는 전략이 현실적이다.
