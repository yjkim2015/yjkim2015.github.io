...
```mermaid
graph LR
    TRX["트랜잭션 COMMIT"] -->|Redo Log 기록| RLOG["Redo Log Buffer"]
    RLOG -->|flush| DISK_LOG[(Redo Log File)]
    TRX -->|페이지 수정| DIRTY["Dirty Page (Buffer Pool)"]
...
```mermaid
graph LR
    COMMIT["COMMIT 요청"] -->|1 Sequential Write| RLOG["Redo Log Buffer"]
    RLOG -->|2 fsync| DISK_LOG[(Redo Log File)]
    DISK_LOG -->|3 응답 반환| CLIENT["클라이언트 OK"]
    DISK_LOG -->|4 나중에| DIRTY["Dirty Page Flush"]
```
...
```mermaid
graph LR
    TS["InnoDB Tablespace"] --> SYS["System Tablespace ibdata1"]
    TS --> FPT["File-Per-Table .ibd"]
    TS --> GEN["General Tablespace"]
    TS --> UNDO["Undo Tablespace undo_001"]
    TS --> TEMP["Temp Tablespace ibtmp1"]
```
...
```mermaid
graph LR
    SQL["SQL 문자열"] --> PARSE["Parser 문법 분석"]
    PARSE --> PRE["Preprocessor 의미 검증"]
    PRE --> OPT["Optimizer 실행계획"]
    OPT --> EXE["Executor Handler API"]
    EXE --> InnoDB["InnoDB 스토리지"]
```
...
```mermaid
graph LR
    QUERY["반복 쿼리"] -->|B+Tree 탐색 감지| AHI_BUILD["AHI 자동 생성"]
    AHI_BUILD --> HASH["Hash Table in Buffer Pool"]
    QUERY -->|이후 쿼리| HASH
    HASH -->|O(1) 조회| LEAF["Leaf Page 직접 접근"]
```
...
```mermaid
graph LR
    TRX["트랜잭션 시작"] --> MVCC["MVCC Read View 생성"]
    TRX --> UNDO["Undo Log 할당"]
    DML["DML 실행"] --> BP["Buffer Pool 페이지 수정"]
    DML --> RLOG["Redo Log Buffer 기록"]
    BP --> DIRTY["Dirty Page 생성"]
    RLOG -->|fsync| DISK_L[(Redo Log 파일)]
```
