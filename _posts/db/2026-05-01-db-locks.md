...
> **비유로 먼저 이해하기**: 락은 화장실 잠금장치와 같다. 한 명이 들어가 잠그면(트랜잭션 시작) 다른 사람은 문밖에서 기다려야 한다(대기). 나올 때 잠금 해제(커밋/롤백)하면 다음 사람이 들어갈 수 있다. 낙관적 락은 잠그지 않고 들어가되, 나올 때 다른 사람이 먼저 썼는지 확인하는 방식이다.
...
```mermaid
graph LR
    SLock["S Lock"] -->|"호환"| SS["S vs S: OK"]
    SLock -->|"충돌"| SX["S vs X: WAIT"]
    XLock["X Lock"] -->|"충돌"| XS["X vs S: WAIT"]
    XLock -->|"충돌"| XX["X vs X: WAIT"]
```
...
```mermaid
graph LR
    R1[1] --> R2[2] --> R3[3] --> R4[4] --> R5["[5] ← X Lock"] --> R6[6] --> R7[7] --> R8[8]
...
```mermaid
graph LR
    R1[1] --> R2[2] --> R3[3] --> R4[4] --> R5[5] --> GAP["Gap Lock<br>6~9 IN"] --> R10[10] --> R11[11]
...
```mermaid
graph LR
    R4[4] --> R5["[5] Record Lock"] --> GAP["Gap Lock<br>5~10 사"] --> R10["[10] Record Lock"] --> R11[...]
    NKL["Next-Key Lock = Re"]
    style R5 fill:#ff6b6b,color:#fff
    style GAP fill:#ffa500,color:#fff
    style R10 fill:#ff6b6b,color:#fff
    style NKL fill:#6b6bff,color:#fff
```
...
```mermaid
graph LR
    A1[App Server 1] --> M["MySQL<br>GET_LOCK("]
    A2[App Server 2] --> M
    A3[App Server 3] --> M
    M --> L1["락 보유 서버: 작업 수행"]
    M --> L2["나머지 서버: 대기 (timeou"]
```
...
락은 동시성 제어의 핵심이다. 락을 모르면 "왜 이 쿼리가 느린지", "왜 데드락이 발생했는지", "왜 `SELECT FOR UPDATE`가 필요한지"를 설명할 수 없다. 특히 재고 차감, 포인트 적립, 중복 주문 방지 같은 실무 시나리오는 모두 락 설계가 정확해야 올바르게 동작한다.

---

## 실무에서 자주 하는 실수

**실수 1: 트랜잭션 범위를 너무 크게 잡아 락 경합 유발**
외부 API 호출, 파일 I/O, 이메일 발송을 트랜잭션 안에 넣는다. 커넥션과 락이 수 초간 점유되어 다른 트랜잭션이 대기한다. 트랜잭션은 DB 작업만 포함하고, 외부 호출은 트랜잭션 밖으로 꺼내야 한다.

**실수 2: 데드락 가능성을 고려하지 않은 다중 테이블 업데이트**
트랜잭션 A가 `users` → `orders` 순으로, 트랜잭션 B가 `orders` → `users` 순으로 락을 잡으면 교착 상태가 된다. 항상 동일한 순서로 테이블과 행에 접근하도록 애플리케이션 레벨에서 규칙을 만들어야 한다.

**실수 3: 갭 락(Gap Lock)을 인지하지 못한 설계**
InnoDB REPEATABLE READ에서 `WHERE id BETWEEN 10 AND 20 FOR UPDATE`는 10~20 사이 존재하지 않는 행에도 갭 락을 건다. 다른 트랜잭션이 이 범위에 INSERT하면 블로킹된다. 의도치 않은 삽입 지연의 원인이 된다.

**실수 4: 낙관적 락에서 버전 충돌 처리를 빠뜨림**
`@Version`을 붙여 낙관적 락을 사용하면서 `OptimisticLockException` 처리를 하지 않는다. 충돌 시 예외가 그대로 사용자에게 전달되거나 데이터가 유실된다. 재시도 로직 또는 사용자에게 충돌 안내 후 재입력 요청이 반드시 필요하다.

**실수 5: `SELECT FOR UPDATE` 없이 읽고 쓰기**
```sql
-- 위험: 두 트랜잭션이 동시에 100을 읽고 각각 90으로 업데이트 → 한 번의 차감만 반영
SELECT stock FROM products WHERE id = 1;  -- 100 읽음
UPDATE products SET stock = stock - 10 WHERE id = 1;
```
재고나 포인트처럼 "읽고 나서 쓰는" 패턴은 반드시 `SELECT FOR UPDATE` 또는 원자적 `UPDATE stock = stock - 10 WHERE stock >= 10`을 사용해야 한다.

---

## 면접 포인트

**Q1. 낙관적 락과 비관적 락의 차이와 선택 기준은?**
낙관적 락은 충돌이 드물다는 전제 하에 버전 번호로 충돌을 감지하고 커밋 시점에 롤백한다. 비관적 락은 충돌 가능성이 높다는 전제 하에 선점적으로 락을 건다. 읽기 위주·충돌 드문 환경은 낙관적, 재고 차감·금융 잔액 처럼 충돌이 잦은 환경은 비관적 락이 적합하다.

**Q2. 데드락 발생 시 어떻게 대응하는가?**
MySQL/PostgreSQL은 데드락을 자동으로 감지해 한 트랜잭션을 롤백한다(`SHOW ENGINE INNODB STATUS`로 마지막 데드락 확인). 애플리케이션은 데드락 예외를 잡아 재시도해야 한다. 예방책은 ① 락 획득 순서 일관성 ② 트랜잭션 짧게 유지 ③ 적절한 인덱스(인덱스 없으면 범위 락 과다 발생).

**Q3. InnoDB에서 레코드 락, 갭 락, 넥스트 키 락의 차이는?**
레코드 락은 존재하는 행 하나에만 걸린다. 갭 락은 행과 행 사이 공간에 걸려 Phantom Read를 방지한다. 넥스트 키 락은 레코드 락 + 앞의 갭 락을 합친 것으로 InnoDB REPEATABLE READ의 기본 락이다. READ COMMITTED에서는 갭 락이 없어 동시성이 높아지지만 Phantom Read가 발생할 수 있다.

**Q4. `LOCK TABLES`와 `SELECT FOR UPDATE`의 차이는?**
`LOCK TABLES`는 테이블 수준의 명시적 잠금으로 DDL 마이그레이션 등 특수 상황에서만 쓴다. `SELECT FOR UPDATE`는 행 수준 잠금으로 트랜잭션 내에서만 유효하며 커밋/롤백 시 자동 해제된다. 일반적인 동시성 제어에는 항상 `SELECT FOR UPDATE`를 사용한다.

**Q5. 분산 환경에서 락은 어떻게 구현하는가?**
단일 DB 락은 한 서버에만 유효하다. 여러 서버에 걸친 동시성 제어는 Redis의 Redlock 알고리즘, ZooKeeper의 분산 락, 또는 DB의 advisory lock을 중앙화해 사용한다. 단, 완벽한 분산 락은 없으므로 DB 레벨 멱등성(중복 처리 방어)을 최후 방어선으로 설계해야 한다.
