...
<div class="mermaid">
graph LR
    subgraph 호환성["S/X Lock 호환성 행렬"]
        SS["S Lock vs S Lock<br>✓ 호환 (동시 읽기 허용)"]
        SX["S Lock vs X Lock<br>✗ 충돌 (쓰기 대기)"]
        XS["X Lock vs S Lock<br>✗ 충돌 (읽기 대기)"]
        XX["X Lock vs X Lock<br>✗ 충돌 (쓰기 대기)"]
    end
    SLock["S Lock<br>SELECT ... LOCK IN SHARE MODE<br>읽기 락 - S Lock과 공존 가능"] --> SS
    SLock --> SX
    XLock["X Lock<br>SELECT ... FOR UPDATE<br>쓰기 락 - 모든 락과 충돌"] --> XS
    XLock --> XX
</div>
...
<div class="mermaid">
graph LR
    R1[1] --> R2[2] --> R3[3] --> R4[4] --> R5[5] --> GAP["Gap Lock<br>6~9 INSERT 불가"] --> R10[10] --> R11[11]
...
<div class="mermaid">
graph LR
    R4[4] --> R5["[5] Record Lock"] --> GAP["Gap Lock<br>5~10 사이"] --> R10["[10] Record Lock"] --> R11[...]
    NKL["Next-Key Lock = Record Lock + Gap Lock<br>레코드 5 + 5~10 간격 전체 보호"]
    style R5 fill:#ff6b6b,color:#fff
    style GAP fill:#ffa500,color:#fff
...
<div class="mermaid">
graph LR
    A1[App Server 1] --> M["MySQL<br>GET_LOCK('resource_key')"]
    A2[App Server 2] --> M
    A3[App Server 3] --> M
    M --> L1["락 보유 서버: 작업 수행"]
    M --> L2["나머지 서버: 대기 (timeout)"]
</div>
