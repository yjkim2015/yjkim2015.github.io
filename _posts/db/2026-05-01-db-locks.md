...
### Q1. InnoDB가 "행 락(Row Lock)"이 아닌 "인덱스 레코드 락"을 사용한다는 것의 실무적 의미는?

InnoDB의 락은 물리적 행이 아닌 인덱스 레코드에 걸린다. 인덱스를 타지 않는 쿼리(`WHERE` 조건에 인덱스 없음)로 `UPDATE`를 실행하면 클러스터드 인덱스의 모든 레코드를 스캔하면서 락을 거므로, 결과적으로 테이블 전체가 잠긴다. 실무에서 "특정 행만 업데이트했는데 다른 트랜잭션이 전부 대기한다"는 증상의 원인이 여기에 있다. `EXPLAIN`으로 풀 스캔 여부를 확인하고 인덱스를 추가하는 것이 해결책이다.

### Q2. Gap Lock과 Next-Key Lock의 차이는? 팬텀 리드를 막는 원리는?

Record Lock은 인덱스의 단일 레코드에 건다. Gap Lock은 레코드 사이의 간격에 건다 — 실제 레코드가 아닌 빈 공간을 잠근다. Next-Key Lock은 Record Lock + Gap Lock의 조합으로 레코드와 그 앞의 간격을 함께 잠근다. `WHERE id BETWEEN 10 AND 20 FOR UPDATE`를 실행하면 id 10~20 범위의 레코드와 그 사이 간격에 Next-Key Lock이 걸려 다른 트랜잭션이 그 범위에 INSERT하지 못한다. 이것이 REPEATABLE READ에서 팬텀 리드를 막는 원리다. Gap Lock은 READ COMMITTED에서는 비활성화된다.

### Q3. 낙관적 락 `@Version`과 비관적 락 `SELECT FOR UPDATE`의 선택 기준은?

낙관적 락은 충돌이 드문 환경에서 성능이 좋다. 읽기 시 락이 없으므로 처리량이 높다. 단, 충돌 시 `OptimisticLockingFailureException`을 잡아 재시도 로직을 구현해야 한다. 재시도 비용이 낮고 충돌이 10% 미만일 때 적합하다. 비관적 락은 충돌이 잦거나 재고 차감, 잔액 변경처럼 손실 업데이트가 절대 허용되지 않는 경우에 사용한다. `FOR UPDATE`는 커밋까지 행을 잠그므로 트랜잭션을 짧게 유지하는 것이 필수다. 고TPS 환경에서 비관적 락은 `원자적 UPDATE` 방식(`UPDATE SET stock = stock - 1 WHERE stock > 0`)으로 대체하면 락 없이 충돌을 방지할 수 있다.

### Q4. 데드락이 발생했을 때 InnoDB가 피해자를 선택하는 기준은? 운영 중 데드락을 어떻게 진단하는가?

InnoDB는 Wait-for Graph(WFG)로 사이클을 탐지하고, **롤백 비용이 가장 적은 트랜잭션**을 피해자로 선택한다. 기준은 변경한 행의 수와 Undo Log 크기다. 진단은 `SHOW ENGINE INNODB STATUS`에서 `LATEST DETECTED DEADLOCK` 섹션을 확인한다. 관련 SQL, 보유 락, 대기 락이 모두 출력된다. `SET GLOBAL innodb_print_all_deadlocks = ON`으로 에러 로그에 모든 데드락을 기록할 수 있다. `EXPLAIN ... FOR UPDATE`로 락 범위를 사전에 파악하는 것이 예방의 핵심이다.

### Q5. 분산 환경에서 Redis 분산 락을 사용할 때 Redlock이 필요한 경우는?

단일 Redis 마스터 기반 락은 마스터 장애 후 Failover 과정에서 같은 락을 두 클라이언트가 동시에 획득할 수 있다. 마스터에 락을 쓰고 아직 레플리카에 복제되기 전에 마스터가 다운되면, 새 마스터는 락 정보가 없어 다른 클라이언트에게 락을 발급한다. 결제처럼 중복 실행이 치명적인 상황에서는 5개의 독립 Redis 노드 중 3개 이상에서 락을 획득하는 Redlock을 사용한다. 단, Redlock도 GC STW나 시계 편향 상황에서는 안전하지 않다는 비판이 있으며, 완전한 강일관성이 필요하다면 ZooKeeper나 etcd가 더 적합하다.
