...
도서관 서가를 상상해보세요. 책이 1억 권이라면 하나의 방에 몰아두면 찾는 데 한참 걸립니다. 하지만 **연도별·분야별로 방을 나눠두면** "2023년 책"을 찾을 때 그 방만 뒤지면 됩니다. 파티셔닝은 데이터베이스의 "방 나누기"입니다. 논리적으로는 하나의 테이블이지만, 물리적으로는 여러 저장 공간에 분산됩니다.
...
1️⃣ 단일 테이블에 1억 건이 쌓이면 인덱스 탐색 깊이가 증가하고 풀 스캔 비용이 폭증한다.
2️⃣ 파티션 키(예: 연도)를 기준으로 데이터를 물리적으로 분리한다.
3️⃣ 쿼리 실행 시 옵티마이저가 해당 파티션만 스캔한다(파티션 프루닝).
...
행(Row) 단위로 데이터를 분할한다. 같은 스키마를 가진 파티션들이 서로 다른 행 집합을 보유한다. 마치 하나의 명부를 가나다 순으로 분철한 것처럼, 구조는 같지만 담긴 데이터가 다르다.
...
열(Column) 단위로 데이터를 분할한다. 자주 조회되는 컬럼과 그렇지 않은 컬럼을 별도 테이블로 분리한다. 사용자 목록을 볼 때 프로필 이미지(BLOB)까지 매번 읽으면 낭비이므로, 자주 쓰는 컬럼만 "핫 테이블"에, 드물게 쓰는 컬럼은 "콜드 테이블"에 분리한다.
...
**RANGE 파티셔닝 INSERT 흐름**:

1️⃣ `INSERT INTO orders VALUES ('2023-06-15', 5000)` 실행
2️⃣ MySQL이 파티션 키 `YEAR(order_date) = 2023` 평가
3️⃣ 파티션 경계 맵에서 `2023 < 2024` → `p_2023` 선택
...
graph LR
    ...
    style P2024 fill:#FFB6C1
```

> **핵심 요약**: RANGE 파티셔닝은 `LESS THAN` 경계값으로 파티션을 정의한다. 항상 `MAXVALUE` 파티션을 마지막에 두어 범위를 벗어난 데이터가 오류 없이 저장되게 해야 한다.
...
## 왜 파티셔닝인가? (vs 샤딩 vs 인덱스 최적화)

| 방식 | 적합한 상황 | 한계 |
|------|-------------|------|
| **인덱스 최적화** | 수천만 건 이하, 쿼리 패턴 단순 | 수억 건 이상에서 인덱스 자체가 커짐 |
| **파티셔닝** | 단일 서버, 날짜/범위 기반 대용량 테이블 | 파티션 키 외 조건 시 전체 스캔, 크로스 파티션 쿼리 비효율 |
| **샤딩** | 단일 서버 한계 돌파, 수평 확장 필요 | 운영 복잡도 급증, 크로스 샤드 트랜잭션 어려움 |

**실무 판단**: 먼저 인덱스와 쿼리 최적화를 시도하고, 테이블이 수억 건을 넘어 I/O가 한계에 달할 때 파티셔닝을 도입한다. 샤딩은 단일 서버 수직 확장과 파티셔닝도 한계에 달한 최후의 수단이다.

---

## 실무에서 자주 하는 실수

**실수 1: 파티션 키를 WHERE 절에 항상 포함하지 않음**
파티션 키(`created_at`)가 WHERE 절에 없으면 파티션 프루닝이 동작하지 않아 모든 파티션을 스캔한다. 오히려 파티셔닝이 없을 때보다 느릴 수 있다. 파티션 키는 가장 자주 사용하는 필터 조건과 반드시 일치해야 한다.

**실수 2: 파티션을 미리 생성하지 않아 INSERT 실패**
RANGE 파티셔닝에서 범위를 벗어난 데이터가 들어오면 오류가 발생한다. `MAXVALUE` 파티션을 두거나, 배치 작업으로 미래 파티션을 주기적으로 생성해야 한다. 자동 파티션 생성 프로시저를 스케줄러에 등록하는 것이 실무 표준이다.

**실수 3: 파티션 수를 과도하게 설정**
일별 파티션을 10년치 미리 생성하면 파티션 메타데이터만으로도 오버헤드가 생긴다. MySQL 권장은 파티션 수 1,000개 미만이다. 월별·분기별 파티션 후 오래된 것을 아카이빙하는 전략이 현실적이다.

**실수 4: 글로벌 유니크 인덱스 제약 오해**
MySQL에서 UNIQUE 인덱스는 파티션 키를 포함해야 한다. 파티션 키 없이 유니크 제약을 걸면 오류가 발생한다. 파티션 키를 포함한 복합 유니크 키로 설계를 변경하거나 애플리케이션 레벨에서 중복을 방지해야 한다.

**실수 5: 파티션 삭제(DROP PARTITION)와 DELETE 혼용**
오래된 데이터를 `DELETE FROM logs WHERE created_at < '2023-01-01'`로 삭제하면 수억 건 DELETE로 언두 로그가 폭증하고 성능이 급락한다. RANGE 파티셔닝에서는 `ALTER TABLE logs DROP PARTITION p2023`이 파티션 파일을 즉시 삭제해 순식간에 완료된다. 파티셔닝의 핵심 이점이다.
...
### Q2. RANGE, LIST, HASH 파티셔닝 중 어떤 것을 언제 쓰는가?
RANGE: 날짜, 연속 숫자 — 시계열 로그, 아카이빙. LIST: 특정 값 목록 — 지역별, 카테고리별. HASH: 균등 분산이 목적 — 핫스팟 방지, 파티션 프루닝보다 분산이 우선일 때. 실무에서는 날짜 기반 로그 테이블에 RANGE + COLUMNS(created_at) 조합이 가장 흔하다.

### Q3. 파티션 프루닝(Partition Pruning)이란?
WHERE 절의 파티션 키 조건을 분석해 관련 파티션만 스캔하는 최적화다. `EXPLAIN`의 `partitions` 컬럼에서 실제 접근하는 파티션을 확인할 수 있다. 프루닝이 동작하지 않으면(`ALL` 파티션 스캔) 파티셔닝의 이점이 없다.

### Q4. 파티셔닝이 인덱스 성능에 미치는 영향은?
로컬 인덱스는 파티션 내에서만 유효하다. 파티션 키 없이 세컨더리 인덱스만으로 조회하면 모든 파티션의 인덱스를 순차 탐색한다. 글로벌 인덱스(MySQL 미지원, PostgreSQL 지원)를 사용하거나, 파티션 키를 항상 WHERE 절에 포함하는 쿼리 패턴을 강제해야 한다.
