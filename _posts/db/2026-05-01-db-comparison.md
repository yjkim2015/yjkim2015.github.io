...
```mermaid
graph LR
    A["ibdata1"] --> A1["Dictionary/Undo"]
    B["table.ibd"] --> B1["B+Tree 페이지"]
...
```mermaid
graph LR
    BP["Buffer Pool"] --> NEW["New/Old Sublist"]
    BP --> CB["Change Buffer"]
    SGA["Oracle SGA"] --> DBC["Buffer/SharedPool"]
...
```mermaid
graph LR
    A["테이블 페이지 (최신 버전)"] -->|"DB_ROLL_PTR"| B["Undo Log: TR..|"DB_ROLL_PTR"| C["Undo Log: TR..|"TRX_ID > 95 → Undo"| B
    D -->|"TRX_ID <= 95 → ..| C
```
...
```mermaid
graph LR
    RU["READ UNCOMMITTED"] -->|"허용"| ALL["Dirty/NR/P..|"방지"| DR["Dirty Read"..|"방지"| DR & NR["NR/Pha..|"방지"| ALL2["모든 이상현상"]
```
...
graph LR
  ...
  ..|"비동기"| CL1["Client"]
 ..|"ACK 후 커밋"| CL2["Client(반동기..|"Paxos"| N2["Node2/3"]
```
...
```mermaid
graph LR
    P1["Primary"] -->|WAL Stream| S1["Standby1 (읽..|WAL Stream| S2["Standby2 (읽..|"변경데이터(행단위)"| SUB["Subscriber (다른스키마"]
```
...
graph LR
    S["DB 선택"] --> Q1{"예산 제한?"}
    Q1 -->|"Yes"| PG["PostgreSQL/..|"No"| Q2{"생태계"}
    Q..|"MS"| SS["SQL Server"..|"기타"| OR["Oracle/PostgreSQL"]
```
