...
graph TD
    ...
    A2["PostgreSQL"] --> B2["연결당 fork"] --> C2["플랜캐시/옵티마이저"] --> D2["Heap/Shared Buffers"]
...
graph TD
    ...
    PM["PostgreSQL: postmaster"] -->|"fork()"| PR1["프로세스1"] & PR2["프로세스2"] & PR3["프로세스N"]
```
...
```mermaid
graph TD
    BP["MySQL Buffer Pool(70~80%)"] --> NEW["New Sublist(5/8) MRU"]
    BP --> OLD["Old Sublist(3/8) LRU"]
    BP --> CB["Change Buffer"]
...
graph TD
    ...
    SI["Secondary Index (name=Kim,PK=5)"] -->|"Double Lookup"| L1
```
...
graph LR
    ...
    PUB["Publisher (선택 테이블)"] -->|"변경데이터(행단위)"| SUB["Subscriber (다른스키마 가능)"]
```
