...
```mermaid
graph LR
    A["테이블(최신)"] -->|"ROLL_PTR"| B["Undo TRX=1"]
    B -->|"ROLL_PTR"| C["Undo TRX=9"]
    D["Read View 95"] -->|"TRX>95→Undo"| B
    D -->|"TRX<=95→가시"| C
```
...
```mermaid
graph LR
    P1["Primary"] -->|"WAL Stream"| S1["Standby1(읽기)"]
    P1 -->|"WAL Stream"| S2["Standby2(읽기)"]
    PUB["Publisher"] -->|"변경데이터"| SUB["Subscriber"]
```
