...
```mermaid
graph LR
    A1["MySQL"] --> D1["ThreadPool→InnoDB"]
    A2["PostgreSQL"] --> D2["fork→HeapBuffer"]
    A3["Oracle"] --> D3["Dedicated→SGA"]
```
...
```mermaid
graph LR
    A["ibdata1"] --> A1["Dictionary"]
    A --> A2["Undo Logs"]
    B["table.ibd"] --> B1["Leaf Pages"]
    B --> B2["Non-Leaf Pages"]
    C["ib_logfile"] --> C1["Redo Log"]
```
...
```mermaid
graph LR
    BP["Buffer Pool"] --> NEW["New Sublist"]
    BP --> OLD["Old Sublist"]
    BP --> CB["Change Buffer"]
    SGA["Oracle SGA"] --> DBC["Buffer Cache"]
    SGA --> SP["Shared Pool"]
    PGA["Oracle PGA"] --> SA["Sort Area"]
```
...
```mermaid
graph LR
    T1["Tuple: xmin=90"] --- T2["Tuple: xmin=100"]
    V["가시성 규칙"] --> V1["xmin < Snapshot"]
    V --> V2["xmin >= Snapshot"]
```
...
```mermaid
graph LR
    S1["Source"] -->|"BinLog"| R1["Replica1/2"]
    S1 -->|"즉시 반환"| CL1["Client(비동기)"]
    S2["Source"] -->|"ACK 후 커밋"| CL2["Client(반동기)"]
    N1["Node1"] <-->|"Paxos"| N2["Node2"]
    N2 <--> N3["Node3"]
```
...
```mermaid
graph LR
    SL["MySQL Server"] --> API["Storage Engine API"]
    API --> E1["InnoDB"]
    API --> E2["MyISAM"]
    API --> E3["Memory/NDB"]
```
