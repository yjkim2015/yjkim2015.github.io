...
```mermaid
graph TD
    A1["MySQL"] --> B1["Thread Pool"] --> C1["파서/옵티마이저"] --> D1["InnoDB/Buffer Pool"]
    A2["PostgreSQL"] --> B2["연결당 fork"] --> C2["플랜캐시/옵티마이저"] --> D2["Heap/Shared Buffer"]
    A3["Oracle"] --> B3["Dedicated/Shared"] --> C3["Library Cache"] --> D3["테이블스페이스/SGA"]
```
...
graph LR
    subgraph "InnoDB 파일 구조"
        A["ibdata1 (시스템 테이블스페"] --> A1["Data Dictionary"]
        A --> A2["Undo Logs"]
        A --> A3["Double Write Buffe"]
        B["table.ibd (테이블별 파일"] --> B1["Leaf Pages (실제 행 데"]
        B --> B2["Non-Leaf Pages (인덱"]
        C["ib_logfile0, ib_lo"] --> C1["Redo Log (WAL)"]
    end
```
...
graph LR
    subgraph "PostgreSQL 데이터 디렉터리 구조"
        A["PGDATA/base/{db_oi"] --> B["{relation_oid} (테이"]
        A --> C["{relation_oid}_fsm"]
        A --> D["{relation_oid}_vm"]
        A --> E["{index_oid} (인덱스 파"]
        F["pg_wal/"] --> G["WAL (Write-Ahead L"]
...
```mermaid
graph TD
    A["Tablespace (논리 단위)"] --> B["Segment (테이블, 인덱스,"]
    B --> C["Extent (연속된 블록 집합)"]
    C --> D["Oracle Block (2KB~"]
    D --> D1["Block Header"]
...
```mermaid
graph TD
    P1["MySQL: 메인 프로세스"] --> T1["스레드1"] & T2["스레드2"] & T3["스레드N"]
    PM["PostgreSQL: postma"] -->|"fork()"| PR1["프로세스1"] & PR2["프로세스2"] & PR3["프로세스N"]
```
...
```mermaid
graph TD
    BP["MySQL Buffer Pool("] --> NEW["New Sublist(5/8) M"]
    BP --> OLD["Old Sublist(3/8) L"]
    BP --> CB["Change Buffer"]
    BP --> AHI["Adaptive Hash Inde"]
    SGA["Oracle SGA(공유)"] --> DBC["Buffer Cache"]
    SGA --> SP["Shared Pool"]
    SGA --> RLB["Redo Log Buffer"]
    PGA["Oracle PGA(프로세스별)"] --> SA["Sort/Hash Area"]
    PGA --> SI["Session Info"]
```
...
```mermaid
graph LR
    A["테이블 페이지 (최신 버전)"] -->|"DB_ROLL_PTR"| B["Undo Log: TRX_ID=1"]
    B -->|"DB_ROLL_PTR"| C["Undo Log: TRX_ID=9"]
    D["Read View (TRX 95"] -->|"TRX_ID > 95 → Undo"| B
    D -->|"TRX_ID <= 95 → 가시적"| C
```
...
graph TD
    ...
    V["가시성 규칙"] --> V1["xmin < 내 Snapshot"]
    V --> V2["xmin < 내 Snapshot"]
    V --> V3["xmin >= 내 Snapshot"]
```
...
graph TD
    ...
    SI["Secondary Index (n"] -->|"Double Lookup"| L1
```
...
```mermaid
graph LR
    IDX["PostgreSQL 인덱스 타입"] --> BT["B-Tree: 동등, 범위, 정렬"]
    IDX --> HS["Hash: 동등 검색 특화 (="]
    IDX --> GS["GiST: PostGIS 지리데이"]
    IDX --> SP["SP-GiST: Quad-Tree"]
    IDX --> GN["GIN: 배열, JSONB, 전문"]
    IDX --> BR["BRIN: 대용량 시계열 (매우"]
```
...
```mermaid
graph LR
    RU["READ UNCOMMITTED"] -->|"허용"| DR["Dirty Read"] & NR["Non-Repeatable"] & PH["Phantom"]
    RC["READ COMMITTED"] -->|"방지"| DR
    RC -->|"허용"| NR & PH
    RR["REPEATABLE READ"] -->|"방지"| DR & NR & PH
    SE["SERIALIZABLE"] -->|"방지"| DR & NR & PH & WS["Write Skew"]
```
...
```mermaid
graph LR
    S1["Source"] -->|"BinLog"| R1["Replica1"]
    S1 -->|"BinLog"| R2["Replica2"]
    S1 -->|"커밋 즉시 반환"| CL1["Client(비동기)"]
    S2["Source"] -->|"Binlog+ACK"| R3["Replica(반동기)"]
    S2 -->|"ACK 후 커밋"| CL2["Client(Semi-Sync)"]
    N1["Node1(Primary)"] <-->|"Paxos"| N2["Node2"]
    N2 <--> N3["Node3"]
    N1 <--> N3
```
...
```mermaid
graph LR
    P1["Primary"] -->|WAL Stream| S1["Standby1 (읽기가능)"]
    P1 -->|WAL Stream| S2["Standby2 (읽기가능)"]
    PUB["Publisher (선택 테이블)"] -->|"변경데이터(행단위)"| SUB["Subscriber (다른스키마"]
```
...
graph TD
    S["DB 선택"] --> Q1{"예산 제한?"}
    Q1 -->|"YES"| Q2{"복잡 쿼리?"}
    Q2 -->|"YES"| R2["PostgreSQL"]
    Q2 -->|"NO"| R1["MariaDB / MySQL"]
    Q1 -->|"NO"| Q4{"생태계"}
    Q4 -->|"Microsoft"| R4["SQL Server"]
    Q4 -->|"Enterprise"| R5["Oracle"]
    Q4 -->|"기타"| R6["PostgreSQL"]
```
