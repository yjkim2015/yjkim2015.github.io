...
### Q1. B+Tree가 B-Tree보다 범위 검색에 유리한 구조적 이유는?

B-Tree는 내부 노드와 리프 노드 모두에 실제 데이터를 저장한다. `WHERE id BETWEEN 30 AND 60` 실행 시 id=30을 찾은 후 id=31을 찾으러 다시 루트부터 탐색해야 한다. 매 레코드마다 트리를 내려가므로 랜덤 I/O가 반복된다. B+Tree는 내부 노드에 키만 저장하고 리프 노드에만 데이터를 저장하며, 리프 노드들이 연결 리스트로 연결된다. 범위 검색 시 시작점을 찾은 후 리프 레벨을 선형 순회하면 끝이다. 순차 I/O로 범위 전체를 처리하므로 랜덤 I/O 없이 빠르다.

### Q2. InnoDB 클러스터드 인덱스와 세컨더리 인덱스의 구조 차이, Double Lookup 문제와 해결책은?

InnoDB는 PK 순서로 실제 행 데이터를 정렬 저장하는 클러스터드 인덱스를 사용한다. 세컨더리 인덱스의 리프 노드에는 인덱스 컬럼 값과 PK 값이 저장된다. 세컨더리 인덱스로 조회하면 ①세컨더리 인덱스 탐색으로 PK 획득 → ②PK로 클러스터드 인덱스를 다시 탐색해 실제 행 획득, 이 두 번의 탐색이 Double Lookup이다. 해결책은 커버링 인덱스다. SELECT에 필요한 컬럼을 모두 인덱스에 포함시키면 클러스터드 인덱스 재탐색 없이 세컨더리 인덱스만으로 결과를 반환한다. `EXPLAIN` Extra에 `Using index`가 표시된다.

### Q3. 복합 인덱스에서 컬럼 순서가 중요한 이유는? `(A, B, C)` 인덱스에서 `WHERE B = 10`이 인덱스를 타지 않는 이유는?

B+Tree는 첫 번째 컬럼 기준으로 정렬하고, 같은 값이면 두 번째 컬럼 순으로 정렬된다. `(A, B, C)` 인덱스는 A → B → C 순으로 중첩 정렬된 상태다. `WHERE B = 10`만 조건으로 주면 B=10인 레코드가 A=1 구간, A=2 구간 등에 분산되어 있다. B 기준 연속 구간이 존재하지 않으므로 인덱스 범위 스캔이 불가능하고 전체 인덱스를 순회해야 한다 — 풀 스캔과 동일하다. 선두 컬럼부터 순서대로 사용해야 인덱스가 효과를 발휘하는 것이 좌측 접두사 규칙(Leftmost Prefix Rule)이다.

### Q4. 카디널리티가 낮은 컬럼에 단독 인덱스가 비효율적인 이유는? 언제 예외가 되는가?

선택도(distinct / total) 낮은 컬럼, 예를 들어 `status`(3종류, 1000만 건)에 단독 인덱스를 걸면 `WHERE status = 'PAID'`는 약 333만 건의 PK를 반환한다. 각 PK마다 클러스터드 인덱스를 탐색하면 333만 번의 랜덤 I/O가 발생한다. Full Table Scan의 순차 I/O가 오히려 빠르다. 예외: ① 복합 인덱스의 선두 컬럼으로 사용할 때 ② 극단적으로 드문 값(0.01%)만 조회할 때 ③ MySQL 8.0 히스토그램으로 분포 정보를 옵티마이저에 제공할 때.

### Q5. `EXPLAIN`의 `Using index`와 `Using index condition`의 차이는? ICP가 성능에 미치는 영향은?

`Using index`는 커버링 인덱스 적용 상태다. 쿼리에 필요한 모든 컬럼이 인덱스에 있어 클러스터드 인덱스를 전혀 조회하지 않는다. 최고 성능이다. `Using index condition`은 Index Condition Pushdown(ICP) 적용 상태다. WHERE 조건의 일부를 스토리지 엔진 레벨로 내려 불필요한 행을 서버 레이어로 올리기 전에 필터링한다. 본 테이블 접근은 발생하지만 서버-엔진 간 불필요한 데이터 이동이 줄어 성능이 향상된다. 아무것도 없으면 인덱스 탐색 후 모든 조건을 서버 레이어에서 처리하는 가장 비효율적인 경우다.
