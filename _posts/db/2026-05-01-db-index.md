...
```mermaid
sequenceDiagram
    Root:__500->>Internal:__250: 호출
    Internal:__250->>Leaf:_id=100,_Kim: 호출
    Internal:__750->>Leaf:_id=300,_Park: 호출
    Leaf:_id=100,_Kim->>Leaf:_id=200,_Lee: Linked List
```
...
```mermaid
graph LR
    Q["WHERE email=kim"] --> SI["Secondary Index (P"]
    SI -->|"Double Lookup"| CI["Clustered Index (n"]
    Q2["SELECT email,name"] --> SI2["Covering Index"]
    SI2 -->|"추가 I/O 없음"| R["결과 반환"]
```
...
인덱스를 추가하면 조회가 빨라진다고 믿는다. 맞는 말이지만, 이면에는 반드시 따라오는 비용과 함정이 있다.

**함정 1: 쓰기 성능 저하 — 인덱스가 많을수록 INSERT/UPDATE가 느려진다**

```sql
-- 인덱스 5개짜리 테이블에 INSERT 1건 = B+Tree 5개를 모두 갱신
-- 인기 이벤트 티켓 오픈: 초당 10,000건 INSERT 발생
-- 인덱스 없음: 10,000 TPS 처리 가능
-- 인덱스 5개: 각 INSERT마다 5번의 B+Tree 갱신 → 처리량 급감 → 대기 큐 폭발

-- 실제로 이런 일이 생긴다:
-- 이벤트 오픈 직후 주문 INSERT가 밀림
-- DB 응답시간 10ms → 500ms → 타임아웃
-- 원인: 누군가 "조회 최적화"를 위해 인덱스를 7개 추가해 놓은 것
```

**함정 2: 인덱스가 있어도 안 타는 경우가 너무 많다**

```sql
-- 인덱스: idx_created_at ON orders(created_at)

-- 함수 감싸기 → 인덱스 무력화
SELECT * FROM orders WHERE YEAR(created_at) = 2026;        -- Full Scan
SELECT * FROM orders WHERE DATE(created_at) = '2026-01-01'; -- Full Scan

-- 타입 불일치 → 인덱스 무력화 (VARCHAR 컬럼에 숫자 비교)
SELECT * FROM users WHERE phone = 01012345678;  -- Full Scan

-- LIKE 전방 와일드카드 → 인덱스 무력화
SELECT * FROM users WHERE name LIKE '%김';      -- Full Scan

-- OR 조건 → Index Merge 시도하지만 비효율적
SELECT * FROM orders WHERE status = 'PAID' OR type = 'ONLINE'; -- 비효율

-- 이 중 하나라도 코드에 있으면 인덱스를 만든 의미가 없다
-- 개발자가 인덱스를 걸었다고 생각하지만 옵티마이저는 쓰지 않는다
```

**함정 3: 옵티마이저가 인덱스를 무시하는 경우**
...
-- 옵티마이저: "어차피 95% 다 읽어야 함 → Full Scan이 더 빠름"
-- EXPLAIN type: ALL → 인덱스 있어도 안 씀

-- 실제로 빠른 건 'PENDING' 조회인데, 이건 분포 정보가 없으면 옵티마이저가 모름
-- 해결: 히스토그램으로 분포 정보 제공
ANALYZE TABLE orders UPDATE HISTOGRAM ON status WITH 10 BUCKETS;
```

**최종 방어선: EXPLAIN ANALYZE로 실제 실행 계획을 반드시 확인한다**

```sql
-- EXPLAIN만으로는 부족하다. 예측이지 실제가 아니다.
-- EXPLAIN ANALYZE는 실제로 실행하고 결과를 보여준다.

EXPLAIN ANALYZE
SELECT * FROM orders WHERE status = 'PAID' AND created_at > '2026-01-01';

-- 결과에서 확인할 것:
-- type: ALL이면 인덱스 미사용 → 인덱스 재검토
-- actual rows >> estimated rows → 통계 오래됨 → ANALYZE TABLE 실행
-- Using filesort → ORDER BY에 인덱스 미활용 → 복합 인덱스 재설계
-- actual time이 예상보다 크면 → 인덱스 단편화 → OPTIMIZE TABLE 검토

-- 인덱스를 추가한 후 반드시 이 명령을 실행해서
-- 옵티마이저가 실제로 그 인덱스를 쓰는지 확인해야 한다
-- 안 쓴다면 인덱스를 만든 것이 오히려 쓰기 성능만 저하시킨 것이다
```
...
## 왜 B-Tree 인덱스인가? (vs Hash Index vs GIN vs BRIN)

| 인덱스 종류 | 최적 사용처 | 한계 |
|-------------|-------------|------|
| **B-Tree** (기본) | 등치, 범위, ORDER BY, LIKE 'abc%' | 배열·전문 검색 불가 |
| **Hash Index** | 등치 비교만 (=) | 범위 검색 불가, WAL 미지원(PG 10 이전) |
| **GIN** | 배열, JSONB, 전문 검색, tsvector | 쓰기 느림, 공간 큼 |
| **GiST** | 지리 데이터(PostGIS), 범위 타입 | 구성 복잡 |
| **BRIN** | 물리적 순서와 상관관계 높은 대용량 컬럼(타임스탬프) | 정밀도 낮음 |

**실무 판단 기준**: 대부분은 B-Tree로 충분하다. 전문 검색이 필요하면 GIN, 수십 억 행의 시계열 로그에서 날짜 범위 조회라면 BRIN이 스토리지와 성능 모두 유리하다.

---

## 면접 포인트

**Q1. 인덱스가 있는데도 풀스캔이 발생하는 이유는?**
옵티마이저가 비용을 계산해 인덱스보다 풀스캔이 싸다고 판단할 때다. ① 선택도가 낮음(전체 행의 20% 이상 반환) ② 함수 적용(`WHERE YEAR(created_at) = 2024`) ③ 타입 불일치(`WHERE varchar_col = 1`) ④ 통계 정보가 오래됨. `EXPLAIN ANALYZE`로 실제 행 수와 예측 행 수 차이를 확인한다.

**Q2. 복합 인덱스에서 컬럼 순서가 중요한 이유는?**
B-Tree는 왼쪽부터 정렬된 구조다. `(a, b, c)` 인덱스는 `a`, `a+b`, `a+b+c` 조건에 사용된다. `b`만 단독 조건으로는 사용 불가다. 카디널리티가 높은 컬럼을 앞에 두되, 자주 쓰는 쿼리 패턴을 우선으로 순서를 결정한다.

**Q3. 커버링 인덱스란?**
쿼리에 필요한 모든 컬럼이 인덱스에 포함되어 테이블 본체(heap)를 읽지 않아도 되는 인덱스다. MySQL은 `EXPLAIN`에서 `Using index`, PostgreSQL은 Index Only Scan으로 표시된다. `SELECT id, name FROM users WHERE status = 'active'`라면 `(status, id, name)` 인덱스로 커버링이 가능하다.

**Q4. 인덱스가 많을수록 좋은가?**
아니다. 인덱스는 쓰기(INSERT/UPDATE/DELETE) 성능을 저하시키고 스토리지를 소비한다. 인덱스마다 B-Tree를 유지해야 하므로 쓰기 시 모든 인덱스를 갱신한다. 사용되지 않는 인덱스는 `pg_stat_user_indexes`(PG) 또는 `sys.schema_unused_indexes`(MySQL)로 주기적으로 확인하고 제거해야 한다.

**Q5. 인덱스 재구성(REBUILD)은 언제 해야 하는가?**
DELETE/UPDATE가 많아 인덱스 블록에 빈 공간(fragmentation)이 쌓일 때다. MySQL InnoDB는 `OPTIMIZE TABLE`, PostgreSQL은 `REINDEX CONCURRENTLY`(잠금 없이)를 사용한다. 단, PostgreSQL의 일반 `REINDEX`는 테이블 잠금이 발생하므로 운영 중에는 CONCURRENTLY 옵션이 필수다.
