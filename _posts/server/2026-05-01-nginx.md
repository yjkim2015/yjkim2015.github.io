...
```mermaid
graph LR
    W["NginxWorker"] -->|"epoll_wait()"| K["LinuxKernel"..|"패킷 도착"| K
    K -->|"소켓A 준비 알림"| W
    W -->|"read/처리/write"| S1
    W -->|"epoll_wait() 재대기"| K
```
...
```mermaid
graph LR
    C1["192.168.1.10"] -->|"hash%3=2"| S3["Server3(세션:..|"hash%3=2"| S3
    C3["10.2..|"hash%3=0"| S1["Server1(세션:A)"]
    S2["Server2(세션:B)"]
```
...
graph LR
    Q1{"서버 스펙이"}
    Q1 -->|"예"| Q2{"세션 고정이"}
  ..|"아니오"| WRR["Weighted R..|"예"| IPHASH["IP Hash..|"아니오"| Q3{"Req  시간이"}
..|"예"| LC["Least Conne..|"아니오"| RR["Round Robin"]
...
graph LR
    ...
    ..|X-Real-IP 헤더 추가| N
    N -->|프록시 전달| B["Spring Boot ..|200 OK| N
    N -->|버퍼링 후 전달| C
```
...
```mermaid
graph LR
    C["Client"] -->|"ClientHello"| N["Nginx(SSL)"]..|"ServerHello+Certificate"| C
    C -->|"KeyExchange+Fin..| N
    N -->|"Session Key 공유"| C
    C -->|"HTTPS 요청"| N
    N -->|"HTTP(평문)"| B["Backend"]
...
graph LR
    REQ["요청"] --> LOOKUP{"캐시 상태"}
    LOOKUP -->|HIT| SERVE["캐시 직접 반환..|MISS| BACKEND["백엔드 Re..|EXPIRED| REVAL["조건부 요청"]
    BACKEND --> STORE["캐시 저장"]
...
graph LR
    ...
    BUCKET -->|10개/초| PROCESS["정상 "]
..|초과 20개| REJECT["429 반환"]
