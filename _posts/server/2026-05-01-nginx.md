...
```mermaid
sequenceDiagram
    participant W as NginxWorker
    participant K as LinuxKernel
    participant S1 as 소켓A
    W->>K: epoll_create/ctl
    W->>K: epoll_wait()
    S1->>K: 패킷 도착
    K-->>W: 소켓A 준비
    W->>S1: read/처리/write
...
sequenceDiagram
    ...
    Note over A,N: 전통(복사 2회)
    A->>K: read → write
    K->>N: 전송
    Note over A,N: sendfile(복사 1회)
    A->>K: sendfile()
    K->>N: DMA 직접 전송
```
...
sequenceDiagram
    ...
    participant S as Servers(1/2/3)
    C->>N: 요청1 → N->>S: Server1(idx0)
    C->>N: 요청2 → N->>S: Server2(idx1)
    C->>N: 요청3 → N->>S: Server3(idx2)
    C->>N: 요청4 → N->>S: Server1(순환)
```
...
```mermaid
graph LR
    C1["192.168.1.10"] -->|"hash%3=2"| S3["Server3(세션:C)"]
    C2["192.168.1.10(재접속)"] -->|"hash%3=2"| S3
    C3["10.20.30.40"] -->|"hash%3=0"| S1["Server1(세션:A)"]
    S2["Server2(세션:B)"]
```
...
sequenceDiagram
    ...
    C->>N: ClientKeyExchange+Finished
    N-->>C: Finished(Session Key 공유)
    C->>N: HTTPS 요청
    N->>B: HTTP(평문)
    B-->>N: 응답
    N-->>C: HTTPS 응답
...
```mermaid
graph LR
    REQ["50개 요청/s"] --> BUCKET["Leaky Bucket"]
    BUCKET -->|10개/초| PROCESS["정상 처리"]
    BUCKET -->|초과 20개| REJECT["429 반환"]
...
sequenceDiagram
    ...
    participant OW as OldWorker
    participant NW as NewWorker
    Note over M: SIGHUP 수신
    M->>NW: fork() 새 Worker
    NW->>NW: 새 요청 처리
    M->>OW: SIGQUIT
    OW-->>M: 완료 후 종료
...
sequenceDiagram
    ...
    N->>S1: 요청1 → 200 OK
    N->>S2: 요청2~4 → fail
    Note over N,S2: max_fails 초과→제외
    N->>S1: 요청5
    N->>S2: 30s 후 복구 확인
```
...
```mermaid
graph LR
    DNS["DNS Round Robin"] --> N1["Nginx 1"]
    DNS --> N2["Nginx 2"]
    DNS --> N3["Nginx 3"]
    N1 & N2 & N3 --> UPSTREAM["백엔드 서버 풀"]
    CACHE["Redis"] --> N1 & N2 & N3
```
...
## 왜 Nginx인가? (vs Apache vs HAProxy)

| 도구 | 아키텍처 | 특징 | 선택 기준 |
|------|--------|------|---------|
| **Nginx** | 이벤트 기반, 비동기 | 정적 파일 + 리버스 프록시 + 로드밸런서 통합 | 범용 웹서버, 고성능 필요 시 |
| **Apache** | 프로세스/스레드 기반 | .htaccess, 모듈 생태계 방대 | 공유 호스팅, PHP 레거시 환경 |
| **HAProxy** | 이벤트 기반 | L4/L7 로드밸런싱 전문, 헬스체크 강력 | 순수 로드밸런서, 고가용성 |
| **Envoy** | C++ 이벤트 기반 | 서비스 메시 (Istio), gRPC 최적화 | MSA 사이드카 프록시 |
| **Caddy** | Go 이벤트 기반 | HTTPS 자동 갱신 (Let's Encrypt 내장) | 소규모, 자동 TLS |

```
Nginx vs Apache 핵심 차이:

Apache (1995):
  요청마다 프로세스/스레드 생성
  동시 접속 1만 → 스레드 1만개 → 메모리 폭발 (C10K 문제)
  .htaccess로 디렉토리별 설정 가능 (공유 호스팅에 유리)

Nginx (2004):
  Worker 프로세스 수 고정 (CPU 코어 수)
  각 Worker가 epoll로 수천 연결을 이벤트로 처리
  동시 접속 1만 → Worker 8개 → 메모리 안정적
  정적 파일 서빙 속도 Apache 대비 2~3배 빠름
```
...
#### 실수 1: Worker 프로세스 수를 CPU 코어 수보다 많이 설정

```nginx
# 나쁜 예 — 4코어 서버에 32개 Worker
worker_processes 32;
# → Worker끼리 CPU 경쟁 → 컨텍스트 스위칭 오버헤드 증가

# 좋은 예 — CPU 코어 수에 맞게 자동 설정
worker_processes auto;  # nproc 결과값 자동 사용
```

#### 실수 2: upstream 헬스체크 없이 장애 서버로 트래픽 전달

```nginx
# 나쁜 예 — 헬스체크 없음
upstream backend {
    server app1:8080;
    server app2:8080;  # app2가 죽어도 트래픽 계속 전달
}

# 좋은 예 — max_fails + fail_timeout 설정
upstream backend {
    server app1:8080 max_fails=3 fail_timeout=30s;
    server app2:8080 max_fails=3 fail_timeout=30s;
    # 30초 내 3번 실패 → 30초간 제외 후 재시도
}
```

#### 실수 3: 대용량 파일 업로드 시 client_max_body_size 미설정

```nginx
# 증상: 이미지/파일 업로드 시 413 Request Entity Too Large
# 원인: 기본값 1MB

# 해결
server {
    client_max_body_size 50m;   # 50MB까지 허용
    client_body_timeout 60s;    # 업로드 타임아웃
}
```

#### 실수 4: SSL 설정에서 취약한 프로토콜/암호화 허용

```nginx
# 나쁜 예 — SSLv3, TLS 1.0 허용 (POODLE, BEAST 취약점)
...
#### 실수 5: proxy_pass에서 trailing slash 혼동

```nginx
# location /api/ → proxy_pass http://backend/
#   /api/users → http://backend/users  (경로 변환 발생)

# location /api/ → proxy_pass http://backend
#   /api/users → http://backend/api/users  (경로 유지)

# 의도한 동작에 따라 trailing slash 명확히 결정 필요
```
...
#### Q. Nginx가 Apache보다 동시 접속 처리에 유리한 이유는?

```
Apache: 요청마다 스레드/프로세스 생성
  동시 접속 10,000개 → 스레드 10,000개 → 메모리 수십 GB

Nginx: 이벤트 기반(epoll), Worker 프로세스 수 고정
  동시 접속 10,000개 → Worker 8개 → 각 Worker가 epoll로 다중 처리
  → 메모리 안정적, CPU 효율적
  → C10K 문제 해결을 위해 설계된 구조
```

#### Q. Nginx의 로드밸런싱 알고리즘 종류와 선택 기준은?

```
Round Robin (기본): 순서대로 분배
  → 서버 성능이 균일하고 요청 처리 시간이 비슷할 때

Weighted Round Robin: 가중치 비율로 분배
  → 서버 사양이 다를 때 (4코어 서버에 weight=4, 2코어에 weight=2)

IP Hash: 클라이언트 IP 기반 고정 서버
  → 세션 유지 필요 시 (sticky session)
  → 단점: 특정 IP 집중 시 서버 불균형

Least Connection: 현재 연결 수 가장 적은 서버로
  → 요청 처리 시간이 들쭉날쭉할 때 (파일 업로드 등)
```

#### Q. Nginx에서 SSL Termination이란?

```
SSL Termination:
  클라이언트 ↔ Nginx: HTTPS (암호화)
  Nginx ↔ Backend: HTTP (평문)

  장점:
  1. Backend 서버가 SSL 핸드셰이크 CPU 부담을 안 함
  2. SSL 인증서를 Nginx 한 곳에서만 관리
  3. Backend 코드에 TLS 설정 불필요

  보안 고려:
  내부 네트워크 신뢰 필요 (VPC 내부, 암호화 필요 없음)
  더 강한 보안이 필요하면 Backend까지 HTTPS 유지 (mTLS)
```
