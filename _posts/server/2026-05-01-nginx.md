...
sequenceDiagram
    ...
    W->>K: epoll_create()
    W->>K: epoll_ctl(ADD, 소켓1000개)
    W->>K: epoll_wait() (CPU 0% 대기)
    S1->>K: 패킷 도착
    K-->>W: 소켓A 준비됨
    W->>S1: read(소켓A)
    W->>W: 요청 처리
...
sequenceDiagram
    participant C as Client
    participant N as Nginx
    participant S1 as S1(5)
    participant S2 as S2(3)
    C->>N: 요청1 → min=3
    N->>S2: S2 선택(3→4)
    C->>N: 요청2 → min=4
    N->>S2: S2 선택(4→5)
...
sequenceDiagram
    ...
    participant OW as OldWorkers
    participant NW as NewWorkers
    participant C as Client
    Note over M: SIGHUP(nginx -s reload)
    M->>M: nginx.conf 읽기/검사
    M->>NW: fork() 새 Worker 생성
    C->>NW: 새 요청 처리
    C->>OW: 기존 요청 계속 처리
...
sequenceDiagram
    ...
    participant S2 as Server2(장애)
    participant S3 as Server3(백업)
    N->>S1: 요청1 → 200 OK
    N->>S2: 요청2~4 → fail 1/3, 2/3, 3/3
    Note over N,S2: max_fails=3 초과 → 30s 제외
    N->>S1: 요청5(S2 제외)
    N->>S2: 30s 후 재시도 → 200 OK 복구
    N->>S3: S1,S2 모두 장애 시 백업 활성화
```
