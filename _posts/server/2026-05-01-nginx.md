...
sequenceDiagram
    Worker_1개->>요청_A: 소켓 A
    Worker_1개->>요청_B: 소켓 B
    요청_A->>Worker_1개: I/O 대기→루프
    요청_B->>Worker_1개: I/O 대기→루프
```
...
```mermaid
sequenceDiagram
    LinuxKernel->>NginxWorker: 소켓A 준비 알림
    NginxWorker->>소켓A: read/처리/write
    NginxWorker->>LinuxKernel: epoll_wait_ 재대기
```
...
```mermaid
graph LR
    M["Master"] -->|fork| W1["Worker1"]
    M -->|fork| W2["Worker2"]
    M -->|fork| CP["CacheManager"]
    C["Client들"] --> W1
    C --> W2
```
...
```mermaid
sequenceDiagram
    Nginx_RR->>Server1: idx=0
    Nginx_RR->>Server2: idx=1
    Nginx_RR->>Server3: idx=2
    Nginx_RR->>Server1: idx=3 순환
```
...
```mermaid
sequenceDiagram
    Nginx->>Server2: 최소연결_3→S2
    Client->>Nginx: 요청2
    Nginx->>Server2: 최소연결_4→S2
    Client->>Nginx: 요청3
    Nginx->>Server1: S1완료→S1선택
```
...
```mermaid
sequenceDiagram
    Nginx->>Nginx: X-Real-IP 헤더 추가
    Nginx->>Spring_Boot_:8080: 프록시 전달
    Spring_Boot_:8080->>Nginx: 200 OK
    Nginx->>클라이언트: 버퍼링 후 전달
```
...
```mermaid
sequenceDiagram
    Nginx_SSL->>Client: ServerHello+Certificate
    Client->>Nginx_SSL: KeyExchange+Finished
    Nginx_SSL->>Client: Session Key 공유
    Client->>Nginx_SSL: HTTPS 요청
    Nginx_SSL->>Backend: HTTP_평문
    Backend->>Client: 응답→HTTPS
```
...
graph LR
    REQ["요청"] --> LOOKUP{"캐시 상태"}
    LOOKUP -->|HIT| SERVE["캐시 직접 반환"]
    LOOKUP -->|MISS| BACKEND["백엔드 요청"]
    LOOKUP -->|EXPIRED| REVAL["조건부 요청"]
    BACKEND --> STORE["캐시 저장"]
...
```mermaid
sequenceDiagram
    DNS_Round_Robin->>Nginx_2: 호출
    DNS_Round_Robin->>Nginx_3: 호출
    Nginx_1->>백엔드_서버_풀: 호출
    Nginx_2->>백엔드_서버_풀: 호출
    Nginx_3->>백엔드_서버_풀: 호출
    Redis->>Nginx_1: 호출
