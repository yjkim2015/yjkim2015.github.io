...
graph TD
    ...
    PROBLEM["문제: CPU는 놀고 있음"] --> T1 & T2 & T3 & T4 & T5
...
graph TD
    subgraph "rate=10r/s, burst=20 설정"
        ...
        BUCKET -->|"20개 대기열 가득\n나머지 20"| REJECT["429 Too Many Reque"]
    end
    subgraph "타임라인"
        T1["t=0.0s: 50개 도착"]
        T2["t=0.1s: 대기열에서 1개 처"]
        T3["t=1.0s: 대기열 10개 처리"]
    end
...
graph TD
    ...
    CACHE["Redis Cluster"] --> N1 & N2 & N3
    subgraph "각 Nginx 노드 설정"
        WP["worker_processes 1"]
