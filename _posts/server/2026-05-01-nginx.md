...
```mermaid
graph LR
    W["NginxWorker"] -->|"epoll_wait()"| K["LinuxKernel"]
    S1["소켓A"] -->|"패킷 도착"| K
    K -->|"소켓A 준비 알림"| W
    W -->|"read/처리/write"| S1
    W -->|"epoll_wait() 재대기"| K
...
```mermaid
graph LR
    C["Client"] -->|"요청1"| N["Nginx"]
    N -->|"Server1(idx0)"| S1["Server1"]
    C -->|"요청2"| N
    N -->|"Server2(idx1)"| S2["Server2"]
    C -->|"요청3"| N
    N -->|"Server3(idx2)"| S3["Server3"]
    C -->|"요청4→순환"| N
    N -->|"Server1"| S1
```
...
graph LR
    C["Client"] -->|"요청1"| N["Nginx"]
    N -->|"최소연결(3)→S2"| S2["Server2"]
    C -->|"요청2"| N
    N -->|"최소연결(4)→S2"| S2
    C -->|"요청3"| N
    N -->|"S1완료→S1선택"| S1["Server1"]
```
...
```mermaid
graph LR
    C["Client"] -->|"ClientHello"| N["Nginx(SSL)"]
    N -->|"ServerHello+Certificate"| C
    C -->|"KeyExchange+Finished"| N
    N -->|"Session Key 공유"| C
    C -->|"HTTPS 요청"| N
    N -->|"HTTP(평문)"| B["Backend"]
    B -->|"응답→HTTPS"| C
