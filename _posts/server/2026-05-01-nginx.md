...
```mermaid
graph LR
    LK["Linux Kernel"] -->|"소켓A 준비 알림"| NW["Nginx Worker"]
    NW -->|"read/처리/write"| SA["소켓 A"]
    NW -->|"epoll_wait 재대기"| LK
```
...
```mermaid
graph LR
    CL["Client"] -->|"요청"| NX["Nginx"]
    NX -->|"최소연결→S2"| S2["Server2"]
    S2 -->|"완료"| NX
    NX -->|"S1완료→S1선택"| S1["Server1"]
```
...
```mermaid
graph LR
    CL["클라이언트"] -->|"요청"| NX["Nginx"]
    NX -->|"X-Real-IP 추가"| NX
    NX -->|"프록시 전달"| SB["Spring Boot:8080"]
    SB -->|"200 OK"| NX
    NX -->|"버퍼링→전달"| CL
```
...
```mermaid
graph LR
    NX["Nginx SSL"] -->|"ServerHello"| CL["Client"]
    CL -->|"KeyExchange"| NX
    NX -->|"Session Key 공유"| CL
    CL -->|"HTTPS 요청"| NX
    NX -->|"HTTP 평문"| BE["Backend"]
    BE -->|"응답→HTTPS"| CL
```
...
```mermaid
graph LR
    M["Master"] -->|"SIGHUP→fork"| NW["NewWorker"]
    NW -->|"새 요청 처리"| NW
    M -->|"SIGQUIT"| OW["OldWorker"]
    OW -->|"완료 후 종료"| M
```
...
```mermaid
graph LR
    N["Nginx"] -->|"요청→200 OK"| S1["Server1"]
    N -->|"요청2~4→fail"| S2["Server2"]
    S2 -->|"max_fails 초과→제외"| N
    N -->|"30s 후 복구 확인"| S2
```
...
```mermaid
graph LR
    DNS["DNS RoundRobin"] -->|"호출"| N1["Nginx 1"]
    DNS -->|"호출"| N2["Nginx 2"]
    DNS -->|"호출"| N3["Nginx 3"]
    N1 -->|"호출"| BP["백엔드 서버 풀"]
    N2 --> BP
