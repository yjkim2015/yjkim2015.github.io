...
sequenceDiagram
    ...
    NginxWorker->>LinuxKernel: epoll_wait() 재대기
```
...
```mermaid
sequenceDiagram
    Nginx->>Server2: 최소연결(3)→S2
    Client->>Nginx: 요청2
    Nginx->>Server2: 최소연결(4)→S2
    Client->>Nginx: 요청3
...
```mermaid
sequenceDiagram
    Nginx(SSL)->>Client: ServerHello+Certificate
    Client->>Nginx(SSL): KeyExchange+Finished
    Nginx(SSL)->>Client: Session Key 공유
    Client->>Nginx(SSL): HTTPS 요청
    Nginx(SSL)->>Backend: HTTP(평문)
...
```mermaid
sequenceDiagram
    DNS_Round_Robin->>Nginx_2: 
    DNS_Round_Robin->>Nginx_3: 
    Nginx_1->>백엔드_서버_풀: 
    Nginx_2->>백엔드_서버_풀: 
    Nginx_3->>백엔드_서버_풀: 
    Redis->>Nginx_1: 
