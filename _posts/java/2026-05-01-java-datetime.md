...
```mermaid
graph LR
    A["Date/Calendar 문제"] --> B["가변(Mutable)"]
    A --> C["월 0 기준, 연도 1900 기준"]
    A --> D["타입 안전 없음"]
    A --> E["시간대 처리 어려움"]
    A --> F["SimpleDateFormat 비"]
```
...
### Q. Date/Calendar 대신 java.time을 쓰는 이유는?
...
### Q. ZonedDateTime에서 DST 전환 시 어떻게 동작하나요?
...
### Q. DateTimeFormatter를 static final로 선언해도 안전한 이유는?
...
```mermaid
graph LR
    A["타입 선택"] --> B["LocalDate: 날짜만"]
    A --> C["LocalDateTime: 날짜+"]
    A --> D["ZonedDateTime: 시간대"]
    A --> E["Instant: 로그/타임스탬프"]
    A --> F["Period/Duration: 기"]
```
...
### Q. java.util.Date를 레거시 코드에서 java.time으로 마이그레이션하는 방법은?
```java
// Date → Instant
Instant instant = legacyDate.toInstant();

// Date → LocalDateTime (시스템 시간대 기준)
LocalDateTime ldt = legacyDate.toInstant()
    .atZone(ZoneId.systemDefault()).toLocalDateTime();

// Calendar → ZonedDateTime
ZonedDateTime zdt = legacyCalendar.toInstant()
    .atZone(legacyCalendar.getTimeZone().toZoneId());

// LocalDateTime → Date (역변환, 레거시 API 전달 시)
Date date = Date.from(localDateTime.atZone(ZoneId.systemDefault()).toInstant());
```
JPA `@DateTimeFormat`, Jackson `@JsonFormat`도 `java.time` 타입을 직접 지원하므로 변환 없이 사용 가능합니다. `spring.jackson.serialization.write-dates-as-timestamps=false` 설정 시 ISO-8601 문자열로 직렬화됩니다.
