...
```mermaid
graph LR
    A["Date / Calendar 7대"] --> B["1️⃣ 가변(Mutable) →"]
    A --> C["2️⃣ 월이 0 기준 (1월=0)"]
    A --> D["3️⃣ 연도가 1900 기준 (D"]
    A --> E["4️⃣ 타입 안전 없음 (정수 상"]
    A --> F["5️⃣ 시간대 처리 어려움"]
    A --> G["6️⃣ 날짜 연산 불편"]
    A --> H["7️⃣ SimpleDateForm"]
```
...
graph LR
    ...
    A --> C["물리적 시간 기반 → Durati"]
    B --> D["단위: 년/월/일"]
    B --> E["대상: LocalDate"]
    B --> F["윤년 자동 반영 ✓"]
    C --> G["단위: 초/나노초"]
    C --> H["대상: LocalTime, Ins"]
    C --> I["DST 무시 (절대 초)"]
```
...
graph LR
    ...
    A --> D["시간대 불필요 날짜+시간 → Lo"]
    A --> E["시간대 포함 글로벌 → Zoned"]
    A --> F["DB 저장 / API 응답 → O"]
    A --> G["로그/타임스탬프 → Instant"]
    A --> H["날짜 기반 기간 → Period"]
    A --> I["시간 기반 경과 → Duratio"]
```
