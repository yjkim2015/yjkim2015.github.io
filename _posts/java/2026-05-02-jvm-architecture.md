...
```mermaid
flowchart TD
    A[".java"] -->|"javac"| B[".class 바이트코드"]
    B --> CL["ClassLoader: 로딩→링킹→초기화"]
    CL --> MEM["Runtime Data Area: Method Area / Heap / JVM Stack / PC / Native Stack"]
    MEM --> EE["Execution Engine: Interpreter + JIT Compiler + GC"]
    EE --> JNI["JNI → Native Libraries"]
```
...
```mermaid
flowchart TD
    A["new 객체 생성"] --> B{"Eden 공간?"}
    B -->|Yes| C["Eden 할당 TLAB"]
    B -->|No| D["Minor GC STW"]
    D --> F["Survivor 복사 age+1"]
    F --> G{"age >= threshold?"}
    G -->|Yes| H["Old Gen Promotion"]
    G -->|No| I["Survivor 대기"]
    H --> J{"Old Gen 부족?"}
    J -->|Yes| K["Full GC"]
    J -->|No| L["Old Gen 장기 거주"]
```
