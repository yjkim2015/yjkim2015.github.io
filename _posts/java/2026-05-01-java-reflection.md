...
마치 설계도를 보고 집을 짓는 것처럼, 리플렉션은 `Class` 메타데이터(설계도)를 보고 런타임에 객체를 만들고 조립합니다. Spring의 `@Autowired`와 JPA의 프록시가 이 방식으로 동작합니다. 설계도를 읽는 비용이 있기 때문에, 반복 호출 시에는 `MethodHandle`로 캐싱하는 것이 중요합니다.
