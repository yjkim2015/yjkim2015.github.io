...
```mermaid
graph LR
    Table["Node[] table"] --> B0["Bucket 0\nNode 헤드"]
    Table --> B1["Bucket K\nnull (빈 버킷)"]
    Table --> B2["Bucket N\nTreeBin 헤드"]
    B0 --> Chain["Node 연결 리스트\n길이<8"]
...
```mermaid
graph LR
    SQ["SynchronousQueue"] --> Fair["fair=true\nTransferQueue\n(FIFO 대기열)"]
    SQ --> Unfair["fair=false\nTransferStack\n(LIFO 스택, 기본)"]
    Fair --> FQ["대기 스레드\nLinked 큐로 관리"]
    Unfair --> FS["대기 스레드\nStack으로 관리"]
```
...
```mermaid
graph LR
    RWL["ReadWriteLock\n읽기 락도 CAS 필요"] --> ReadCAS["readLock()마다\n카운터 CAS 경합"]
    SL["StampedLock\n낙관적 읽기"] --> NoLock["tryOptimisticRead\n락 획득 없음"]
    NoLock --> Validate["validate(stamp)\n단순 long 비교"]
    ReadCAS --> Contention["32스레드 동시 읽기\nCAS 경합 폭증"]
    Validate --> Fast["경합 없음\n최대 처리량"]
```
...
```mermaid
graph LR
    Q1["키-값 저장\n범용"] --> CHM["ConcurrentHashMap"]
    Q2["읽기 압도적\n리스트"] --> COW["CopyOnWriteArrayList"]
    Q3["정렬+범위쿼리"] --> CSL["ConcurrentSkipListMap"]
    Q4["생산자-소비자\n블로킹 필요"] --> BQ["BlockingQueue 계열"]
    Q5["락 없는 큐\n폴링 방식"] --> CLQ["ConcurrentLinkedQueue"]
