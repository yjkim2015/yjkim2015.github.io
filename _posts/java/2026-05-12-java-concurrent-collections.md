...
마치 대형 슈퍼마켓처럼, Java 7은 구역(Segment)별로 자물쇠를 걸었습니다. 구역 A를 잠그는 동안 구역 B는 열려 있습니다. Java 8은 아예 진열대 한 칸(Node) 단위로 자물쇠를 걸어 동시 접근 범위를 훨씬 좁혔습니다. 대부분의 경우 자물쇠 없이 CAS(낙관적 잠금)로 처리합니다.
...
### Q. ConcurrentHashMap이 null을 허용하지 않는 이유
...
### Q. LinkedBlockingQueue가 ArrayBlockingQueue보다 처리량이 높은 이유
...
### Q. ConcurrentSkipListMap이 Red-Black Tree보다 동시성에서 유리한 이유
...
### Q. SynchronousQueue의 내부 구조와 cachedThreadPool 연관성
...
### Q. StampedLock 낙관적 읽기의 validate() 메모리 의미론
