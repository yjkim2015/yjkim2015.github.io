...
graph LR
    ...
    JVM --> META["Metaspace (Native)"]
    JVM --> STACK["Thread Stack"]
    HEAP --> YOUNG["Young Gen"]
    HEAP --> OLD["Old Gen (Tenured)"]
    YOUNG --> EDEN["Eden"]
...
```mermaid
graph LR
    BEF["Before: A 빈공간 B 빈공간"] --> AFT["After Compact: A B 큰빈공간"]
    AFT --> BUMP["bump-the-pointer 할당"]
```
