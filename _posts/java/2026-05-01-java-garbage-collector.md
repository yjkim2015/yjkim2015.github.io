...
메모리 누수 (Memory Lea / 댕글링 포인터 (Dangling
...
From: A* B C* D / To: A* C* / From: 전체 비움
...
Major GC Old →(STW 더 김)→ Full GC

### 객체 승격(Promotion) 과정

Eden →(Minor GC 생존)→ Survivor S0(age=1), Survivor S0(age=1) →(age>=15)→ Old Gen
...
gantt title Serial GC 동작 (단일 GC 스레드) dateFormat X
...
gantt title Parallel GC 동작 dateFormat X
...
2. Concurrent Mark → 3. Mixed GC STW
...
Young Region RemSet → Young GC: RS만 확인
...
힙이 너무 작음 → 권장: 실제 워킹셋의 2~3배 설, 힙이 너무 큼 → 권장: 실제 워킹셋의 2~3배 설
...
앱 특성 분석 → 힙 < 1GB?, 힙 < 1GB? → NO, 처리량 최우선? → NO
