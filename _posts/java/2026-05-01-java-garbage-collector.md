...
```mermaid
graph TD
    Heap --> Young["Young: Eden + Survivor S0/S1"]
    Heap --> Old["Old Generation (Tenured)"]
    Young -->|"age 임계 초과"| Old
    JVM --> Heap & Metaspace["Metaspace: 클래스메타/메서드/static"] & Stack["Stack: 스레드별 프레임"]
```
...
```mermaid
graph TD
    ROOT["GC Root (Stack/static/JNI)"] --> A --> B --> C
    ROOT -.미도달.-> D & E & F
...
```mermaid
graph TD
    LOG["GC 로그 분석\nGC(42) Pause Young 512M→256M 45ms"]
    LOG --> A["GC(42): 42번째 GC"]
    LOG --> B["Pause Young: Minor GC"]
...
graph TD
    GC["GC 선택"]
    GC --> PAR["처리량 최우선 → Parallel GC"]
    GC --> G1["범용 균형 → G1 GC (기본값)"]
    GC --> ZGC["저지연 → ZGC / Shenandoah"]
    GC --> SER["소규모 → Serial GC"]
    TUNE["튜닝 3원칙: 측정먼저 / 하나씩 변경 / Xmx 우선"]
    LEAK["누수 방지: static컬렉션 remove 쌍 / ThreadLocal.remove() / try-with-resources / HeapDump 활성화"]
```
