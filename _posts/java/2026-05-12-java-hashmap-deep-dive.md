...
마치 도서관 사서가 책 제목을 받아 "서가 번호"를 계산하는 것처럼, 해시 함수는 임의의 키를 배열 인덱스로 변환합니다. 사서가 없으면 책 한 권을 찾으려고 전체 서가를 훑어야 합니다(O(n)). 사서가 공식으로 바로 서가 번호를 계산하면 직접 꺼낼 수 있습니다(O(1)).
...
마치 엘리베이터 배분 시스템처럼, RBT는 "빨간/검정" 두 색으로 높이 균형을 유지합니다. AVL은 모든 삽입 후 엄격하게 높이를 맞추지만(매번 균형 검사), RBT는 색 규칙만 지키면 되므로 재균형 횟수가 훨씬 적습니다. HashMap처럼 삽입이 빈번한 자료구조에서 RBT가 AVL보다 실측 빠른 이유입니다.
...
마치 식당이 손님이 많아지면 테이블을 두 배로 늘리는 것처럼, HashMap은 원소 수가 임계값을 넘으면 배열을 2배로 확장합니다. 그런데 테이블을 새로 배치할 때 모든 손님을 다시 안내해야 하는 것처럼, 기존 모든 노드를 재배치(rehash)해야 합니다. Java 8은 이 비용을 비트 트릭으로 절반으로 줄입니다.
...
### Q. Java 7에서 HashMap 무한 루프가 왜 생기는가
...
### Q. equals를 오버라이드하면 hashCode도 오버라이드해야 하는 이유
