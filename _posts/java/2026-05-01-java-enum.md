...
graph LR
    ...
    B --> C["final class Season"]
    C --> D["public static fina"]
    C --> E["public static fina"]
    C --> F["public static fina"]
    C --> G["public static fina"]
```
...
```mermaid
graph LR
    A["EnumSet 내부 비트 구조"] --> B["SPRING=bit0, SUMME"]
    B --> C["EnumSet.of(SPRING,"]
    C --> D["contains(FALL) = 0"]
    C --> E["contains(SPRING) ="]
```
...
```mermaid
graph LR
    A["Enum"] --> B["기본: 타입안전 상수 / fina"]
    A --> C["확장: 필드+메서드+생성자 / 추"]
    A --> D["컬렉션: EnumSet(비트벡터)"]
    A --> E["싱글톤: 리플렉션/직렬화 공격에"]
    A --> F["주의: ordinal() 금지 /"]
```
...
**Q1. Enum이 싱글턴 패턴에 적합한 이유는?**
JVM이 각 Enum 상수를 클래스 로딩 시 딱 한 번만 인스턴스화합니다. 직렬화/역직렬화 후에도 같은 인스턴스를 반환하고, 리플렉션으로 새 인스턴스 생성이 불가합니다(생성자가 private이고 `Constructor.newInstance()` 호출 시 예외 발생). Effective Java에서 Joshua Bloch가 "싱글턴 구현의 최선"으로 추천하는 방식입니다. `DatabaseConnection.INSTANCE.getConnection()` 형태로 사용합니다.

**Q2. Enum에 상태(mutable field)를 두면 어떤 문제가 생기는가?**
Enum 상수는 JVM에서 공유 인스턴스이므로 mutable field를 두면 스레드 안전 문제가 발생합니다. 멀티스레드 환경에서 `STATUS.count++`는 race condition입니다. Enum 필드는 반드시 불변(`final`)으로 설계해야 합니다. 상태가 필요하면 Enum을 키로 사용하는 `EnumMap` 또는 `ConcurrentHashMap`을 외부에 따로 유지합니다.

**Q3. ordinal()을 사용하면 안 되는 이유는?**
`ordinal()`은 Enum 선언 순서 기반의 정수 값입니다. 새 상수를 중간에 추가하면 이후 상수의 ordinal이 모두 바뀝니다. DB에 `ORDER_STATUS`를 ordinal(0, 1, 2...)로 저장했다가 상수 순서가 변경되면 기존 DB 데이터가 다른 상태를 가리킵니다. 반드시 명시적인 코드 필드(`private final int code`)를 정의하고 DB에는 해당 코드를 저장합니다.

**Q4. Enum을 switch 문에서 사용할 때 주의사항은?**
Java 14 이전 switch 문에서 Enum을 사용할 때 `default` 케이스를 생략하면, 나중에 새 Enum 상수가 추가됐을 때 해당 상수에 대한 처리가 누락됩니다. 컴파일 에러가 나지 않아 런타임에야 발견됩니다. Java 14+ switch 표현식은 모든 케이스를 다루지 않으면 컴파일 에러가 발생해 안전합니다. Java 21 Pattern Matching for switch로 더욱 강력한 타입 안전 분기가 가능합니다.

**Q5. EnumMap이 HashMap보다 빠른 이유와 사용 기준은?**
EnumMap은 내부적으로 Enum ordinal을 인덱스로 사용하는 배열입니다. 해싱 연산이 전혀 없고 배열 접근이므로 HashMap 대비 2~3배 빠릅니다. 메모리도 적습니다. 키가 Enum 타입으로 확정된 경우 항상 EnumMap을 사용합니다. 예: 요일별 영업시간 `EnumMap<DayOfWeek, BusinessHour>`, 상태별 처리 전략 `EnumMap<OrderStatus, OrderHandler>`.
