...
```mermaid
graph TD
    A["Enum"] --> B["기본: 타입안전 상수 / final class / == 비교"]
    A --> C["확장: 필드+메서드+생성자 / 추상메서드 전략패턴 / 인터페이스"]
    A --> D["컬렉션: EnumSet(비트벡터) / EnumMap(배열)"]
    A --> E["싱글톤: 리플렉션/직렬화 공격에 안전"]
    A --> F["주의: ordinal() 금지 / valueOf() 예외 → 래퍼 사용"]
```
