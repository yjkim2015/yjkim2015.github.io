...
마치 메뉴판처럼, `sealed interface`는 가능한 타입 목록을 고정합니다. 요리사(switch)는 메뉴판에 있는 것만 처리하면 되고, 새 메뉴가 추가되면 요리사가 처리법을 반드시 추가해야 합니다(컴파일 에러). 열린 상속 계층에서는 새 요리가 몰래 추가되어도 요리사가 모를 수 있습니다.

```mermaid
graph LR
    SI["sealed Shape"] --> C["Circle record"]
    SI --> R["Rectangle record"]
    SI --> T["Triangle record"]
    SW["switch(shape)"] -->|"패턴 매칭"| SI
```
...
### Q. Record에서 equals를 커스터마이징해야 하는 경우는?
...
### Q. Record를 JPA Entity로 쓸 수 없는 근본 이유는?
...
### Q. sealed interface + Record 패턴의 장점은?
