...
```mermaid
graph LR
  D["[1,2,3,4,5]"] -->|"for 루프"| R1["결과"]
  S["소..|"1. filter"| F["중간 연산"]
  F ..|"2. map"| M["중간 연산"]
  M ..|"3. collect"| R2["최종 연산"]
```
...
```mermaid
graph LR
  A["Collection/Array"] -->|"Stream 반환"| B["filter → map..|"즉시 실행"| C["collect/count"]
```
...
```mermaid
graph LR
  A["요소 A"] -->|filter| FA["통과"] -->|map| MA["변환"] -->|forEach| RA["출력"]
  B["요..|filter| FB["통과"] -->|map| MB["변환"]
```
