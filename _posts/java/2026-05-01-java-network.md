...
```mermaid
graph LR
    A["Java 네트워크 API"] --> B["java.net: Socket/S"]
    A --> C["java.nio: SocketCh"]
    A --> D["java.net.http: Htt"]
    E["상황별 권장"] --> E1["단순 HTTP → HttpClie"]
    E --> E2["소규모 TCP → ServerSo"]
    E --> E3["대규모 TCP → NIO/Nett"]
    E --> E4["UDP → DatagramSock"]
```
