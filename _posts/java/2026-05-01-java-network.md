...
```mermaid
graph LR
    C["클라이언트"] -->|SYN| S["ServerSocket..|SYN-ACK| C
    C -->|ACK| S
    S -->|accept| CS["Socket(전용)"..|송수신| CS
```
...
```mermaid
graph LR
    A["UDP 클라이언트"] -->|"DatagramPacket ..| B["UDP 서버 (9090)"]
    B -->|"응답 패킷"| A
    Note1["패킷 유실 가능 (비신뢰성)"]
    Note2["순서 보장 없음"]
    Note3["연결 설정 없음 (빠름)"]
...
```mermaid
graph LR
    A["Java 네트워크"] --> B["java.net"] & C["java.nio"] & D["java.net.http"]
    B --> E["소규모 TCP"]
    C --> F["대규모 NIO"]
```
