...
```mermaid
graph TD
    A["4. 응용\nHTTP/FTP/DNS · HttpClient"] --> B["3. 전송\nTCP/UDP · Socket · DatagramSocket"]
    B --> C["2. 인터넷\nIP · InetAddress"]
    C --> D["1. 네트워크\nEthernet · Wi-Fi (OS)"]
```
...
```mermaid
graph TD
    A["Java 네트워크 API"] --> B["java.net: Socket/ServerSocket/UDP"]
    A --> C["java.nio: SocketChannel/Selector"]
    A --> D["java.net.http: HttpClient(Java11+)"]
    E["상황별 권장"] --> E1["단순 HTTP → HttpClient"]
    E --> E2["소규모 TCP → ServerSocket+스레드풀"]
    E --> E3["대규모 TCP → NIO/Netty"]
    E --> E4["UDP → DatagramSocket"]
```
