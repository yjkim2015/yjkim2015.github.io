...
```mermaid
graph LR
    INTERP["인터프리터"] -->|"1500 호출"| C1["C1 컴파일"]
    C1 -->|"10000 호출"| C2["C2 최적화"]
    C2 -->|"탈최적화"| INTERP
```
...
### Q. "Tiered Compilation의 5단계를 설명해 주세요"
...
### Q. JIT 컴파일이 코드 실행에 영향을 미치는 Warm-up 현상은?
JVM 시작 직후에는 모든 코드가 인터프리터로 실행됩니다. 호출 횟수가 임계값(C1: 2000, C2: 10000)을 넘어야 JIT 컴파일이 시작됩니다. 이 "워밍업" 구간에서 응답 시간이 느리고 처리량이 낮습니다. 실무 영향: 쿠버네티스 롤링 배포 시 새 Pod가 트래픽을 받기 시작하면 처음 수분간 P99 응답이 높습니다. 대응: ① Readiness Probe를 워밍업 완료 후 통과하도록 설정 ② GraalVM PGO(Profile Guided Optimization)로 워밍업 데이터를 AOT에 반영 ③ JVM 공유 클래스 캐시로 JIT 결과를 재사용.

### Q. 메서드 인라이닝의 한계와 `final` 키워드의 관계는?
JIT는 메서드를 인라이닝할 때 해당 메서드가 오버라이딩되지 않는다는 것을 확인해야 합니다. 클래스/메서드가 `final`이면 오버라이딩이 불가하므로 JIT가 안전하게 인라이닝합니다. 인터페이스 메서드는 구현체가 여러 개일 수 있어 CHA(Class Hierarchy Analysis)로 단일 구현체임을 확인해야 인라이닝합니다. 코드에서 성능 크리티컬한 메서드를 `final`로 선언하면 JIT 최적화 힌트가 됩니다. 단, 현대 JVM은 Speculative Inlining으로 `final` 없이도 인라이닝하고, 가정이 깨지면 역최적화(deoptimization)합니다.

### Q. JIT 컴파일이 적용된 코드가 역최적화(Deoptimization)되는 경우는?
JIT는 Speculative Optimization으로 "이 코드는 항상 이 경로를 따른다"고 가정해 최적화합니다. 이 가정이 깨지면 인터프리터 모드로 돌아갑니다(역최적화). 예: 단일 구현체로 인라이닝된 인터페이스 메서드에 두 번째 구현체가 로딩되면 역최적화 발생. JVM 시작 후 시간이 지나 플러그인이나 라이브러리가 동적 로딩될 때 갑자기 응답 시간이 올라가는 원인이 될 수 있습니다. `-XX:+PrintCompilation`으로 컴파일/역최적화 이벤트를 추적할 수 있습니다.

### Q. GraalVM Native Image의 장단점은?
장점: AOT 컴파일로 JVM 없이 실행. 시작 시간 10ms 이하(JVM 대비 100배 빠름). 메모리 사용량 50~80% 감소. 컨테이너 환경에서 시작 시간이 중요한 FaaS(AWS Lambda)에 적합. 단점: 빌드 시간 수분~수십 분. 리플렉션, 동적 클래스 로딩 제한(`reflect-config.json` 수동 등록 필요). JIT 최적화 없어 장기 실행 처리량이 JVM 대비 낮을 수 있음. Spring Boot 3.x + GraalVM의 조합이 클라우드 네이티브 표준으로 자리잡고 있습니다.

### Q. 프로파일링 기반 최적화(PGO)란 무엇인가?
