...
flowchart LR
    A["인터프리터 실행"] --> B{"호출수 초과?"}
    B -- No --> A
    B -- Yes --> C["C1 컴파일(경량 최적화)"]
    C --> D{"C2 임계값 초과?"}
    D -- No --> C
    D -- Yes --> E["C2 컴파일(공격적 최적화)"]
    E --> F{"가정 깨짐?"}
    F -- No --> E
    F -- Yes --> A
```
...
**Q1. JIT 컴파일이 코드 실행에 영향을 미치는 Warm-up 현상은?**
...
**Q2. 메서드 인라이닝의 한계와 `final` 키워드의 관계는?**
...
**Q3. JIT 컴파일이 적용된 코드가 역최적화(Deoptimization)되는 경우는?**
