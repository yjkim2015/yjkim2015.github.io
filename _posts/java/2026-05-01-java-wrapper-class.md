...
### Q. Integer 캐시(-128~127)의 원리와 실무적 위험성을 설명하세요.
...
### Q. OptionalInt와 Optional<Integer>의 차이는?
...
### Q. switch 문에서 Integer를 쓸 때 주의사항은?
...
### Q. Integer 캐시 범위(-128~127)가 실무에서 버그를 만드는 경우는?
```java
Integer a = 127;
Integer b = 127;
...
### Q. null이 포함된 컬렉션에서 언박싱 시 NPE가 발생하는 이유는?
```java
Map<String, Integer> map = new HashMap<>();
...
### Q. 숫자 타입 간 변환 시 정밀도 손실이 발생하는 경우는?
...
### Q. Optional과 래퍼 클래스 중 null 표현에 무엇을 사용하는가?
