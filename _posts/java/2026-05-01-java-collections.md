...
마치 아파트 우편함처럼, HashMap은 버킷(우편함)을 배열로 관리합니다. 각 우편함에 편지(엔트리)가 쌓이다가 8개를 넘으면 검색 속도를 위해 색인(Red-Black Tree)을 붙입니다. 우편함이 꽉 차면 아파트를 2배 확장(resize)하고 편지를 재배치합니다.
...
### Q. HashMap의 로드 팩터가 0.75인 이유는 무엇인가?
...
### Q. Java 7의 HashMap이 멀티스레드에서 무한루프를 일으키는 원리는?
...
### Q. ConcurrentHashMap의 size()가 정확하지 않을 수 있는 이유는?
...
### Q. ArrayList의 add(E)가 amortized O(1)인 이유를 수식으로 설명하라.
...
### Q. LinkedList가 이론상 중간 삽입 O(1)임에도 실전에서 ArrayList보다 느린 이유는?
