...
graph TD
    ...
    List --> ArrayList & LinkedList & Vector & COWAL["CopyOnWriteArrayList"]
...
graph TD
    ...
    N3 --> N1["1B"] & N4["4B"]
    N7 --> N6["6B"] & N9["9B"]
    RULE["규칙: 루트BLACK / RED자식BLACK / 경로BLACK수동일 / O(log n)"]
```
...
graph TD
    ...
    N3 --> N7["7(3)"] & N4["4(4)"]
    N2 --> N5["5(5)"] & N6["6(6)"]
    RULE["부모(i-1)/2 / 좌2i+1 / 우2i+2 / 부모<=자식(Min-Heap)"]
```
...
```mermaid
graph TD
    INIT["초기: [null][null][null][A←head][B][C][null←tail][null]"]
    AFTER["addFirst(Z): [null][null][Z←head][A][B][C][null←tail][null]"]
    RULE["addFirst: head=(head-1+cap)%cap / addLast: tail=(tail+1)%cap"]
    INIT --> AFTER --> RULE
```
...
graph TD
    ...
    M3 -->|YES| M4["TreeMap"]
    M3 -->|NO| M5{"순서 보존?"}
    M5 -->|YES| M6["LinkedHashMap (LRU 캐시)"]
    M5 -->|NO| M7{"Key=Enum?"}
