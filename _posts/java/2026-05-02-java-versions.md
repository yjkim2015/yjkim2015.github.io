...
```mermaid
graph TD
    OS["OS 스레드 (1MB/개): 10,000개=10GB, 대부분 대기 낭비"]
    CT1["Carrier Thread 1 (OS)"] --> VT1["VT1 (KB)"] & VT2["VT2 (KB)"]
    CT2["Carrier Thread 2 (OS)"] --> VT3["VT3 (KB)"] & VTN["VT-N (KB)"]
    NOTE["Virtual Thread: 100만개 가능 / I/O 대기시 Carrier 반납"]
    OS -.개선.-> CT1 & CT2
```
