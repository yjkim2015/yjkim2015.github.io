...
```mermaid
graph TD
    IS["InputStream"] --> FIS["FileInputStream"] & BAIS["ByteArrayInputStre"] & PIS["PipedInputStream"] & FLIS["FilterInputStream"]
    FLIS --> BIS["BufferedInputStrea"] & DIS["DataInputStream"] & PBIS["PushbackInputStrea"]
    OS["OutputStream"] --> FOS["FileOutputStream"] & BAOS["ByteArrayOutputStr"] & POS["PipedOutputStream"] & FLOS["FilterOutputStream"]
    FLOS --> BOS["BufferedOutputStre"] & DOS["DataOutputStream"] & PS["PrintStream (Syste"]
```
...
```mermaid
graph TD
    RD["Reader"] --> ISR["InputStreamReader"]
    ISR --> FR["FileReader"]
    RD --> BR["BufferedReader (re"] & CAR["CharArrayReader"] & SR["StringReader"] & PR["PipedReader"]
    WR["Writer"] --> OSW["OutputStreamWriter"]
    OSW --> FW["FileWriter"]
    WR --> BW["BufferedWriter (ne"] & CAW["CharArrayWriter"] & SW["StringWriter"] & PW["PrintWriter"] & PIW["PipedWriter"]
```
...
graph TD
    subgraph "파일 복사 성능 비교 (1GB 파일)"
        M1["FileInputStream 버퍼"]
        M2["FileInputStream +"]
        M3["FileChannel.transf"]
        M4["MappedByteBuffer<b"]
        M5["Files.copy NIO.2 ←"]
    end
```
...
```mermaid
graph TD
    IO["java.io"] --> BS["바이트: InputStream/O"] & CS["문자: Reader/Writer"] & SER["직렬화: ObjectStream"] & RAF["RandomAccessFile"]
    NIO["java.nio"] --> BUF["Buffer (ByteBuffer"] & CH["Channel (FileChann"] & SEL["Selector/Selection"] & MBB["MappedByteBuffer"]
    NF["java.nio.file"] --> PF["Path/Files/FileSys"] & WS["WatchService"] & FA["FileVisitor/BasicF"]
    IO & NIO & NF --> GUIDE["텍스트→Files.readStri"]
```
