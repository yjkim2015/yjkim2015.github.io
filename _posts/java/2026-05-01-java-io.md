...
```mermaid
graph TD
    IS["InputStream"] --> FIS["FileInputStream"] & BAIS["ByteArrayInputStream"]
    IS --> FLIS["FilterInputStream"] --> BIS["BufferedInputStream"] & DIS["DataInputStream"]
    OS["OutputStream"] --> FOS["FileOutputStream"] & FLOS["FilterOutputStream"]
    FLOS --> BOS["BufferedOutputStream"] & PS["PrintStream"]
```
...
graph TD
    ...
    WR --> BW["BufferedWriter"] & PW["PrintWriter"]
```
...
```mermaid
graph TD
    MBB["MappedByteBuffer 가상 메모리"]
    FILE["OS Page Cache 파일 데이터"]
...
```mermaid
graph TD
    M1["FileInputStream 버퍼"] --> M2["FileChannel.transferTo"]
    M2 --> M3["MappedByteBuffer"]
    M3 --> M4["Files.copy NIO.2 권장"]
```
