...
프로그램 →(출력 스트림 (OutputStre)→ 데이터 목적지
...
Java Object<br>(메모 →(직렬화 (Serialize))→ byte stream<br>(파일, byte stream<br>(파일 →(역직렬화 (Deserialize))→ Java Object<br>(메모
...
MappedByteBuffer →(직접 접근 Zero-Copy)→ OS Page Cache
