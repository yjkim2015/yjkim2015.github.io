...
> **비유로 이해하기**: Java I/O는 물류 시스템과 같습니다. 전통적인 `java.io`는 직원 한 명이 택배 하나씩 창고에서 꺼내 배달하는 방식(블로킹)입니다. `java.nio`는 대형 컨베이어 벨트(Buffer)와 고속 통로(Channel)를 도입해 여러 택배를 한꺼번에 처리하고, 감시 시스템(Selector)이 어느 통로에 택배가 쌓였는지 모니터링해 한 명이 여러 통로를 효율적으로 관리합니다. `java.nio.file`은 이 복잡한 시스템을 "이 파일을 저기로 복사해"라고 간단히 지시할 수 있는 관리자 인터페이스입니다.
...
```mermaid
graph LR
    SRC["데이터 소스"] -->|"입력 스트림 (InputStrea"| PROG["프로그램"]
    PROG -->|"출력 스트림 (OutputStre"| DEST["데이터 목적지"]
```
...
```mermaid
graph LR
    BS["Byte Stream"] --> IS["InputStream"] & OS["OutputStream"]
    BS --> B1["1byte 단위 / 이미지,영상,"]
    CS["Character Stream"] --> R["Reader"] & W["Writer"]
    CS --> C1["2byte char / 텍스트 /"]
```
...
```mermaid
graph LR
    IS["InputStream"] --> FIS["FileInputStream"] & BAIS["ByteArrayIS"]
    IS --> FLIS["FilterInputStream"] --> BIS["BufferedIS"] & DIS["DataIS"]
    OS["OutputStream"] --> FOS["FileOS"] & FLOS["FilterOS"]
    FLOS --> BOS["BufferedOS"] & PS["PrintStream"]
```
...
```mermaid
graph LR
    RD["Reader"] --> ISR["InputStreamReader"] --> FR["FileReader"]
    RD --> BR["BufferedReader"] & SR["StringReader"]
    WR["Writer"] --> OSW["OutputStreamWriter"] --> FW["FileWriter"]
    WR --> BW["BufferedWriter & PrintWriter"]
```
...
```mermaid
graph LR
    FIS["FileInputStream<br"]
    BIS["BufferedInputStrea"]
    DIS["DataInputStream<br"]
    READ["읽기 작업<br>← 최종적으로 모"]
    FIS --> BIS --> DIS --> READ
```
...
```mermaid
graph LR
    BUF["Buffer<br>데이터를 담는"]
    CH["Channel<br>실제 I/O를"]
    SEL["Selector<br>다수의 채널"]
    BUF <-->|"읽기/쓰기"| CH
    CH --> SEL
```
...
```mermaid
stateDiagram-v2
    [*] --> 초기: allocate(10) pos=0 lim=cap=10
    초기 --> 쓰기후: put(A/B/C) pos=3 lim=10
    쓰기후 --> 읽기모드: flip() pos=0 lim=3
    읽기모드 --> 쓰기초기화: clear() pos=0 lim=cap=10
    읽기모드 --> 읽기반복: rewind() pos=0 lim유지
```
...
```mermaid
graph LR
    OBJ["Java Object<br>(메모"]
    BYTES["byte stream<br>(파일"]
...
```mermaid
graph LR
    M1["FileInputStream"] --> M2["FileChannel.transferTo"]
    M2 --> M3["MappedByteBuffer"]
    M3 --> M4["Files.copy(권장)"]
```
...
```mermaid
graph LR
    IO["java.io"] --> BS["바이트 스트림"] & CS["문자 스트림"]
    NIO["java.nio"] --> BUF["Buffer/Channel"] & SEL["Selector"]
    NF["java.nio.file"] --> PF["Path/Files"]
    IO & NIO & NF --> GUIDE["상황별 API 선택"]
```
...
**실수 1: 스트림을 닫지 않아 파일 핸들 누수**

```java
// 잘못된 코드 — 예외 발생 시 스트림이 닫히지 않음
FileInputStream fis = new FileInputStream("data.txt");
int data = fis.read(); // 예외 발생 시 fis.close() 호출 불가
fis.close();

// 올바른 코드 — try-with-resources 반드시 사용
try (FileInputStream fis = new FileInputStream("data.txt")) {
    int data = fis.read();
} // 자동으로 close() 호출
```

파일 핸들은 OS 자원이므로 누수가 쌓이면 `Too many open files` 오류로 서버 전체가 다운될 수 있습니다. `try-with-resources`는 선택이 아니라 필수입니다.

**실수 2: 문자 인코딩을 지정하지 않아 한글 깨짐**

```java
// 잘못된 코드 — OS 기본 인코딩에 의존 (Windows: MS949, Linux: UTF-8)
FileReader fr = new FileReader("korean.txt"); // 플랫폼 의존!
BufferedWriter bw = new BufferedWriter(new FileWriter("output.txt")); // 플랫폼 의존!

// 올바른 코드 — 항상 명시적으로 UTF-8 지정
...
**실수 3: 버퍼 없이 바이트 단위 읽기**

```java
// 매우 느린 코드 — 바이트 하나마다 OS 시스템 콜 발생
FileInputStream fis = new FileInputStream("large_file.bin");
int b;
while ((b = fis.read()) != -1) { // 1GB 파일이면 10억 번 시스템 콜!
    process(b);
}

// 올바른 코드 — 버퍼로 감싸거나 배열로 읽기
try (BufferedInputStream bis = new BufferedInputStream(
        new FileInputStream("large_file.bin"), 65536)) { // 64KB 버퍼
    byte[] buf = new byte[8192];
    int bytesRead;
    while ((bytesRead = bis.read(buf)) != -1) {
        process(buf, bytesRead); // 8192바이트씩 처리
    }
}
```

**실수 4: NIO ByteBuffer에서 flip() 누락**

```java
// 잘못된 코드 — flip() 없이 읽기 시도
ByteBuffer buf = ByteBuffer.allocate(1024);
channel.read(buf); // position이 읽은 바이트 수만큼 증가
// buf.flip() 누락!
while (buf.hasRemaining()) { // position=limit이므로 hasRemaining()=false → 아무것도 읽지 못함
    process(buf.get());
}

// 올바른 코드
ByteBuffer buf = ByteBuffer.allocate(1024);
channel.read(buf);    // 쓰기 모드: position이 증가
buf.flip();           // 읽기 모드 전환: limit=position, position=0
while (buf.hasRemaining()) {
    process(buf.get()); // limit까지 읽음
}
buf.clear();          // 재사용: position=0, limit=capacity
```

**실수 5: 직렬화에서 serialVersionUID 미선언**

```java
// 위험한 코드 — 클래스 변경 시 역직렬화 실패
public class UserSession implements Serializable {
    private String userId; // serialVersionUID 없음
    // 필드 추가 후 이전 직렬화 데이터 읽으면 InvalidClassException!
}
...
public class UserSession implements Serializable {
    ...
    // 필드 추가해도 동일 serialVersionUID면 하위 호환 유지
}
```
...
단순한 `java.nio.file`의 `Files` 유틸리티로 충분합니다. 설정 파일 읽기, 로그 쓰기, 소용량 파일 처리 모두 `Files.readString()`, `Files.writeString()`, `Files.lines()`로 처리합니다.

```java
// 100 TPS: NIO.2 유틸리티로 충분
...
대용량 파일 처리와 I/O 성능이 중요해집니다. `FileChannel.transferTo()`의 Zero-Copy 기능과 버퍼 크기 최적화가 핵심입니다.

```java
// 10K TPS: Zero-Copy 파일 전송 + 버퍼 최적화
public void serveFile(Path filePath, OutputStream httpOutput) throws IOException {
    // Zero-Copy: JVM 힙을 거치지 않고 OS 버퍼에서 직접 소켓으로 전송
    try (FileChannel fc = FileChannel.open(filePath, StandardOpenOption.READ);
         WritableByteChannel dst = Channels.newChannel(httpOutput)) {
        fc.transferTo(0, fc.size(), dst);
    }
}

// 대용량 CSV 처리: 버퍼 크기 최적화
int bufferSize = 256 * 1024; // 256KB — OS 페이지 크기의 배수
try (BufferedReader reader = new BufferedReader(
...
파일 I/O 자체를 최소화하고, 메모리 맵 파일(MappedByteBuffer)과 비동기 I/O(AsynchronousFileChannel)를 활용합니다.

```java
// 100K TPS: 메모리 맵 파일 — OS가 페이징 관리
// 여러 프로세스가 같은 파일을 읽을 때 메모리를 공유
try (FileChannel fc = FileChannel.open(
        Path.of("lookup_table.dat"), StandardOpenOption.READ)) {
    // OS의 가상 메모리에 파일 매핑 — read() 시스템 콜 불필요
    MappedByteBuffer mmap = fc.map(FileChannel.MapMode.READ_ONLY, 0, fc.size());
    mmap.load(); // OS가 페이지 캐시에 미리 로드

    // 이후 읽기는 메모리 접근과 동일한 속도
    for (int i = 0; i < recordCount; i++) {
        int value = mmap.getInt(i * 4); // 오프셋 직접 접근
        processRecord(value);
    }
}

// 비동기 파일 I/O — 스레드 블로킹 없이 I/O 처리
AsynchronousFileChannel asyncFc = AsynchronousFileChannel.open(
    Path.of("data.bin"), StandardOpenOption.READ);

ByteBuffer buffer = ByteBuffer.allocate(4096);
asyncFc.read(buffer, 0, buffer, new CompletionHandler<Integer, ByteBuffer>() {
    @Override
    public void completed(Integer bytesRead, ByteBuffer buf) {
        buf.flip();
        processData(buf); // I/O 완료 후 콜백으로 처리
    }
    @Override
    public void failed(Throwable exc, ByteBuffer buf) {
        log.error("비동기 읽기 실패", exc);
    }
});
```

100K TPS 규모에서는 파일 I/O 자체보다 **네트워크 I/O**(HTTP, DB, 메시지 큐)가 주 병목입니다. Netty나 Project Reactor 같은 비동기 I/O 프레임워크를 도입하거나, Virtual Thread(Java 21)로 블로킹 I/O를 비동기처럼 다루는 것이 현실적인 해결책입니다.
...
**Q1. BIO(Blocking I/O)와 NIO(Non-blocking I/O)의 차이는?**
BIO: 스레드가 read() 호출 후 데이터가 도착할 때까지 블로킹. 클라이언트 1개당 스레드 1개 필요. 10만 연결 = 10만 스레드 → 메모리 부족(스레드당 약 1MB 스택). NIO: Selector가 여러 Channel을 단일 스레드로 모니터링. 이벤트(읽기 가능, 쓰기 가능) 발생 시 해당 Channel만 처리. 10만 연결을 수십 개 스레드로 처리 가능. Netty, Spring WebFlux의 근간. 단, NIO 코드가 복잡하므로 Netty 같은 프레임워크를 통해 사용합니다.

**Q2. BufferedReader를 반드시 사용해야 하는 이유는?**
`FileReader.read()`는 시스템 콜을 매 문자마다 발생시킵니다. 시스템 콜 전환 비용이 크기 때문에 100만 자를 읽으면 100만 번의 시스템 콜. `BufferedReader`는 8KB 버퍼를 메모리에 유지하고 한 번에 8KB를 읽어 버퍼에 채운 후 애플리케이션이 버퍼에서 꺼냅니다. 시스템 콜 횟수가 1/8000로 줄어 성능이 수십~수백 배 향상됩니다. 파일이나 네트워크 스트림을 처리할 때는 항상 버퍼 스트림으로 감쌉니다.

**Q3. NIO의 Buffer는 어떻게 사용하는가?**
Buffer는 내부적으로 position, limit, capacity 세 포인터를 관리합니다. 쓰기 후 읽으려면 반드시 `flip()`을 호출해 limit을 현재 position으로 설정하고 position을 0으로 되돌려야 합니다. `flip()` 없이 읽으면 아무것도 읽지 못하거나 이상한 값을 읽습니다. 읽기 완료 후 재사용하려면 `clear()` 또는 `compact()`를 호출합니다. NIO 버그의 50% 이상이 `flip()` 누락에서 발생합니다.

**Q4. Files.readAllBytes()와 BufferedReader의 선택 기준은?**
`Files.readAllBytes()`: 파일 전체를 한 번에 메모리에 올립니다. 수 MB 이하 소파일에 적합. `Files.readString()`(Java 11+)로 더 간결하게 사용 가능. `BufferedReader` + `lines()`: 파일을 스트리밍으로 처리. 수 GB 로그 파일도 메모리 초과 없이 처리 가능. `Files.lines(path).filter(...).forEach(...)`로 함수형 처리 가능. 파일 크기가 JVM Heap의 10% 이상이면 스트리밍 방식을 선택합니다.

**Q5. 직렬화(Serializable) 사용 시 주의사항은?**
`serialVersionUID`를 명시하지 않으면 JVM이 자동 계산합니다. 클래스에 필드 추가 시 자동 계산값이 바뀌어 기존 직렬화된 데이터를 역직렬화할 때 `InvalidClassException`이 발생합니다. 반드시 `private static final long serialVersionUID = 1L`을 명시합니다. 또한 직렬화된 데이터를 외부에 저장하거나 전송하면 보안 취약점이 됩니다(`ObjectInputStream.readObject()`는 임의 코드 실행 가능). 현대 애플리케이션에서는 JSON/Protobuf 직렬화를 사용하고 Java 직렬화는 RMI 등 레거시 목적 외에 지양합니다.
