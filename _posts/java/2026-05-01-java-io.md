...
```mermaid
graph LR
    BS["ByteStream"] --> IS["InputStream"]
    CS["CharStream"] --> RD["Reader"]
```
...
```mermaid
graph LR
    RD["Reader"] --> FR["FileReader"]
    WR["Writer"] --> FW["FileWriter"]
```
...
```mermaid
graph LR
    OBJ["Java Object"] -->|"직렬화"| BS["byte stream"]
    BS -->|"역직렬화"| OBJ2["Java Object"]
```
