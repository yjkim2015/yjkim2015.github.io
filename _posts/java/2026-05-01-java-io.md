...
```mermaid
sequenceDiagram
    Byte_Stream->>InputStream: 호출
    Character_Stream->>Reader: 호출
```
...
```mermaid
sequenceDiagram
    InputStream->>FileIS: 호출
    OutputStream->>FileOS: 호출
```
...
```mermaid
sequenceDiagram
    Reader->>FileReader: 호출
    Writer->>FileWriter: 호출
```
...
```mermaid
sequenceDiagram
    Java_Object<br>메모->>byte_stream<br>파일: 직렬화 Serialize
    byte_stream<br>파일->>Java_Object<br>메모: 역직렬화 Deserialize
```
