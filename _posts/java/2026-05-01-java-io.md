...
graph LR
    ...
    BS --> B1["1byte 단위 / 이미지,영상,음성"]
    CS["Character Stream"] --> R["Reader"] & W["Writer"]
    CS --> C1["2byte char / 텍스트 / 인코딩 자동처리"]
```
...
```mermaid
graph TD
    IS["InputStream"] --> FIS["FileInputStream"] & BAIS["ByteArrayInputStream"] & PIS["PipedInputStream"] & FLIS["FilterInputStream"]
    FLIS --> BIS["BufferedInputStream"] & DIS["DataInputStream"] & PBIS["PushbackInputStream"]
    OS["OutputStream"] --> FOS["FileOutputStream"] & BAOS["ByteArrayOutputStream"] & POS["PipedOutputStream"] & FLOS["FilterOutputStream"]
    FLOS --> BOS["BufferedOutputStream"] & DOS["DataOutputStream"] & PS["PrintStream (System.out)"]
```
...
```mermaid
graph TD
    RD["Reader"] --> ISR["InputStreamReader (바이트→문자 Bridge)"]
    ISR --> FR["FileReader"]
    RD --> BR["BufferedReader (readLine)"] & CAR["CharArrayReader"] & SR["StringReader"] & PR["PipedReader"]
    WR["Writer"] --> OSW["OutputStreamWriter (문자→바이트 Bridge)"]
    OSW --> FW["FileWriter"]
    WR --> BW["BufferedWriter (newLine)"] & CAW["CharArrayWriter"] & SW["StringWriter"] & PW["PrintWriter"] & PIW["PipedWriter"]
```
...
```mermaid
graph TD
    IO["java.io"] --> BS["바이트: InputStream/OutputStream"] & CS["문자: Reader/Writer"] & SER["직렬화: ObjectStream / Serializable"] & RAF["RandomAccessFile"]
    NIO["java.nio"] --> BUF["Buffer (ByteBuffer/CharBuffer...)"] & CH["Channel (FileChannel/SocketChannel)"] & SEL["Selector/SelectionKey"] & MBB["MappedByteBuffer"]
    NF["java.nio.file"] --> PF["Path/Files/FileSystem"] & WS["WatchService"] & FA["FileVisitor/BasicFileAttributes"]
    IO & NIO & NF --> GUIDE["텍스트→Files.readString | 고성능→FileChannel | 논블로킹→Selector | 감시→WatchService"]
```
