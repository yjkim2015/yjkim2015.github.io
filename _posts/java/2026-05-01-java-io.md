...
```mermaid
graph LR
    WRITE["쓰기 모드\npos→데이터끝"] -->|"flip()"| READ["읽기 모드\npos=0,lim=데이터끝"]
    READ -->|"clear()"| INIT["초기화\npos=0,lim=cap"]
    READ -->|"compact()"| WRITE
    READ -->|"rewind()"| READ2["다시읽기\npos=0"]
```
...
graph LR
    ...
    Q1 -->|"10MB ~ 1GB"| LINES["Files.lines() 스트림"]
    Q1 -->|"> 1GB"| MMAP["MappedByteBuffer"]
    FILES --> Q2["단순 읽기/쓰기"]
    MMAP --> Q3["랜덤 접근?"]
    Q3 -->|"Yes"| FC["FileChannel + seek"]
    Q3 -->|"No"| COPY["transferTo zero-copy"]
```
...
### Q1. java.io와 java.nio의 근본적 차이는 무엇인가?
...
### Q2. ByteBuffer의 flip(), compact(), clear() 차이와 내부 동작은?
...
### Q3. FileChannel.transferTo()가 zero-copy인 이유는?
...
### Q4. MappedByteBuffer와 일반 ByteBuffer의 차이는?
...
### Q5. Java 직렬화의 대안과 serialVersionUID 관리 전략은?
