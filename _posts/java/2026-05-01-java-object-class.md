...
graph LR
    ...
    R1["a == b → false"]
    A2["변수 a2"] -->|"내용: hello"| C1["✓ 같은 내용"]
    B2["변수 b2"] -->|"내용: hello"| C1
    R2["a.equals(b) → true"]
```
...
graph LR
    ...
    EQUALS["3. 버킷에서 key.equals"]
    STORE["4. 일치하면 value 저장/반"]
    CHAIN["해시 충돌 시: 같은 버킷에 체이"]
...
```mermaid
graph LR
    P["생산자"] -->|"synchronized 진입"| M["모니터"]
    P -->|"버퍼 가득→wait()"| M
    M -->|"락 반납 WAITING"| P
    C["소비자"] -->|"synchronized 진입"| M
    C -->|"소비 후 notifyAll()"| M
    M -->|"생산자 깨움→재개"| P
```
...
graph LR
    ...
    OBJECT --> TOSTRING["toString() - 항상 오버"]
    OBJECT --> EQUALS["equals() - 5가지 계약"]
    OBJECT --> HASHCODE["hashCode() - equal"]
    OBJECT --> CLONE["clone() - 복사 생성자 권"]
    OBJECT --> FINALIZE["finalize() - 사용 금지"]
    OBJECT --> WAITNOTIFY["wait/notify - sync"]
