...
```mermaid
graph LR
    PA["프로세스A"] --> SHARED["공유: Code/Data/Heap"]
    PA --> T1["Thread1 Stack"] & T2["Thread2 Stack"] & T3["Thread3 Stack"]
    PB["프로세스B(격리)"] --> SB["Code/Data/Heap"] & TB["Thread Stack"]
```
...
```mermaid
graph LR
    T1 -->|1:1| KT1[KernelThread1]
    T2 -->|1:1| KT2[KernelThread2]
    U1[T1] -->|N:1| KU1[KernelThread]
    U2[T2] -->|N:1| KU1
    U3[T3] -->|N:1| KU1
    VT1 -->|M:N| KV1[KT1]
    VT2 -->|M:N| KV1
...
```mermaid
graph TD
    MW["MarkWord(unlock/biased/thin/fat)"] --> MON["Monitor: owner,count,EntrySet,WaitSet"]
    TA([ThreadA]) --> ME[monitorenter]
    ME --> CHK{owner==null?}
...
```mermaid
graph LR
    TA[ThreadA] --> CELL["AtomicLong: count(경쟁)"]
    TB[ThreadB] --> CELL
    TC[ThreadC] --> CELL
...
```mermaid
graph TD
  TA["Thread A"] --> MA["Map: tlA→사용자A, tlB→connA"]
  TB["Thread B"] --> MB["Map: tlA→사용자B, tlB→connB"]
  NOTE["동일 ThreadLocal, 스레드별 독립 값"]
```
...
```mermaid
graph TD
    KEY["KEY: WeakRef&lt;ThreadLocal&gt;"] --- VAL["VALUE: 강한 참조"]
    TL["ThreadLocal=null"] -->|"GC가 KEY 수거"| GC["KEY=null"]
    GC -->|"VALUE는 잔존"| LEAK["메모리 누수!"]
    REQ1["요청1: set(userId=kim)"] --> POOL["스레드 재사용"]
    POOL -->|"remove() 없으면"| LEAK2["userId=kim 노출!"]
```
