...
```mermaid
graph LR
    T1["JavaThread"] -->|1:1| KT1["KernelThre..|N:1| KU["KernelThrea..|M:N| KV["KT1/KT2"]
```
...
graph LR
    ...
    RU..|락 실패| BLOCKED
    BLO..|락 획득| RUNNABLE
    RU..|wait/sleep| WAITING["WAITIN..|notify/경과| RUNNABLE
```
...
graph LR
    ...
    CHK -->|YES| OWN["owner=A, c..|NO| BLK["EntrySet→B..|exit| REL["owner=null, 재경쟁"]
```
...
```mermaid
graph LR
  CHK1 -->|YES| NEW1[새 Thread C..|NO| CHK2{workQueue ..|NO| ENQUEUE[workQue..|YES| CHK3{현재 Thread ..|YES| NEW2[새 Thread C..|NO| REJ[RejectedExecutionHandler 실행]
```
...
flowchart LR
  ...
  B --> C{"3. CAS(addr, val.."}
  C -->|"YES → 원자적으로 교체\..| D["성공 — return newVal"]
  C -->|"NO → 다른 Thread가..| A
  D2["하드웨어: x86 CMPXCHG"]
```
...
```mermaid
graph LR
  P([생산자]) -->|"put() - 큐 꽉 참 시..| BQ["BlockingQueue"]
  BQ -->|"take() - 큐 비어있으..| C([소비자])
```
...
```mermaid
graph LR
    VT1["VThread1"] -->|마운트| CT["Carrier Thr..|I/O→언마운트| VT1H["VT1 힙 Sav..|마운트| VT2["VThread2"]..|I/O완료| CT
```
...
..|VALUE 잔존| LEAK["메모리 누수"]
..|remove() 없으면| LEAK2["이전값 노출!"]
```
