...
graph LR
  ...
  NOTE1["Stack: 기본 512KB~.."]
  CONT["Heap의 Continuation"]
  NOTE2["mount 시만 carrier.."]
```
...
```mermaid
graph LR
    VA["VThread-A"] -->|마운트| C1["Carrier-1"]..|I/O→언마운트| VAH["VT-A 힙 Sav..|마운트| VB["VThread-B"]..|I/O완료| C1
```
...
```mermaid
graph LR
    V1["VThread"] -->|I/O| UM["Unmount → 반..|synchronized| SYN["고정(Pinned)..|I/O| PINNED["Carrier 점유!"]
```
