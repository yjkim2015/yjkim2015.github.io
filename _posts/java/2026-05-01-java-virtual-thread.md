...
graph LR
  ...
  NOTE1["Stack: 기본 512KB~1MB"]
  CONT["Heap의 Continuation"]
  NOTE2["mount 시만 carrier 점유"]
```
...
```mermaid
graph LR
    V1["VThread"] -->|I/O| UM["Unmount → 반환"]
    V2["VThread2"] -->|synchronized| SYN["고정(Pinned)"]
    SYN -->|I/O| PINNED["Carrier 점유!"]
```
