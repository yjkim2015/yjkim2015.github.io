...
```mermaid
graph LR
    TH["Throwable"] -->|"extends"| ERR["Error"]
    TH -->|"extends"| EX["Exception"]
    EX -->|"Checked"| IOE["IOException"]
    EX -->|"Unchecked"| RE["RuntimeException"]
```
...
```mermaid
graph LR
    G["Infra"] --> F["DataAccessEx"]
    F --> D["ServiceEx"]
    D --> A["Controller"]
    A --> B["ControllerAdvice"]
```
...
```mermaid
graph LR
    A["예외 처리"] --> B["계층: Error/Checked/"]
    A --> C["규칙: 무시금지/cause보존/로"]
...
### Q. Checked 예외와 Unchecked 예외를 언제 선택하는가?
...
### Q. try-with-resources가 finally보다 나은 이유는?
...
### Q. 예외를 잡아서 로깅 후 다시 던질 때 올바른 방법은?
```java
// 잘못된 방식: 스택 트레이스 손실
} catch (Exception e) {
...
### Q. 전역 예외 핸들러(`@ControllerAdvice`)와 개별 try-catch의 사용 기준은?
...
### Q. NullPointerException을 방지하는 실무 전략은?
