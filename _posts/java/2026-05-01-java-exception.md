...
graph TD
    ...
    C --> D["IOException (Checked)"] & E["SQLException (Checked)"] & F["RuntimeException (Unchecked)"]
    D --> D1["FileNotFoundException / SocketException"]
    F --> F1["NullPointerException / IllegalArgumentException"]
```
...
```mermaid
graph TD
    A["예외 처리"] --> B["계층: Error/Checked/Unchecked"]
    A --> C["규칙: 무시금지/cause보존/로깅1곳"]
    A --> D["커스텀: ErrorCode Enum+도메인계층"]
    A --> E["Spring: @RestControllerAdvice+rollback"]
```
