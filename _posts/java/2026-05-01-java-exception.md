...
```mermaid
graph LR
    A["Throwable"] --> B["Error (복구불가)"] & C["Exception"]
    B --> B1["OutOfMemoryError /"]
    C --> D["IOException (Check"] & E["SQLException (Chec"] & F["RuntimeException ("]
    D --> D1["FileNotFoundExcept"]
    F --> F1["NullPointerExcepti"]
```
...
```mermaid
graph LR
    A["Controller Layer"] --> B["@RestControllerAdv"]
    C["Service Layer"] --> D["비즈니스 예외 발생 / 저수준 예"]
    E["Repository Layer"] --> F["DataAccessExceptio"]
    G["Infrastructure Lay"] --> H["IOException/SQLExc"]
    H --> F
    F --> D
    D --> A
    A --> B
