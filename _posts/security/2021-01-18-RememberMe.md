...
sequenceDiagram
    ...
    participant RMF as RememberMeFilter
    participant UDS as UserDetailsService
    participant S as Session
    B->>RMF: 요청(JSESSIONID없음, remember-me쿠키있음)
    RMF->>RMF: SecurityContext null 확인, 쿠키 토큰 추출/디코딩
    RMF->>UDS: loadUserByUsername()
    UDS-->>RMF: UserDetails 반환
    RMF->>RMF: 토큰 서명 검증
    RMF->>S: 새 세션 생성, Authentication 저장
    RMF-->>B: 인증 성공
```
