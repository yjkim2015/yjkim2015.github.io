...
```mermaid
graph LR
    O1["상속 계층"] -.->|"불일치"| D1["슈퍼타입/서브타입"]
    O2["참조(Association)"] -.->|"불일치"| D2["외래키(FK)"]
    O3["그래프 탐색"] -.->|"불일치"| D3["SQL JOIN"]
```
...
```mermaid
graph TD
    XML["persistence.xml"] -->|"createEntityManagerFactory()"| EMF["EntityManagerFactory\n(앱 전체 1개)"]
    EMF -->|"createEntityManager()"| EM1["EntityManager\n(요청마다 생성)"] & EM2["EntityManager\n(요청마다 생성)"]
    EM1 & EM2 -->|"쿼리 실행"| DB["Database"]
```
