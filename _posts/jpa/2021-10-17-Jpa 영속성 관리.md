...
```mermaid
graph LR
    APP["App"] -->|em.persist| PC1["1차 캐시"] & PC2["쓰기지연 SQL"]
...
```mermaid
graph TD
    PA["persist(A)"] --> SQL["쓰기지연 저장소"]
    PB["persist(B)"] --> SQL
    SQL --> TC["commit()"] --> FL["flush()"]
    FL --> DB1["INSERT A → DB"]
    FL --> DB2["INSERT B → DB"]
    DB1 & DB2 --> CM["DB 커밋"]
```
...
```mermaid
graph TD
    A["em.find() → 1차 캐시 + 스냅샷 저장"] --> B["setXxx() 값 변경"]
    B --> C["transaction.commit()"]
    C --> D["flush() 엔티티 vs 스냅샷 비교"]
    D --> E["변경 감지 → UPDATE SQL 생성 → DB 전송"]
```
