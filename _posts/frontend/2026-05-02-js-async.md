...
```mermaid
graph LR
    A1["동기: 코드실행"] --> B1["요청"] -->|"3초 블로킹"| FREEZE["UI 멈춤"]
    B1 --> C1["응답"] --> D1["다음코드"]
    A2["비동기: 코드실행"] --> B2["요청시작"] --> C2["다른코드/UI 유지"]
    B2 -->|"3초 후"| E2["응답처리"]
```
...
```mermaid
graph TD
    ALL["Promise.all: 하나라도 실패 → 전체 실패"]
    SETTLED["Promise.allSettled: 모두 기다려 전체 결과 반환"]
    RACE["Promise.race: 가장 먼저 완료된 것 반환"]
    ANY["Promise.any: 첫 번째 성공한 것 반환"]
    ALL --- SETTLED
    RACE --- ANY
```
