...
flowchart LR
    Q1{"무엇을 최적화?"}
    Q1 -->|"컴포넌트 재렌더링"| MEMO["React.mem..|"함수 참조 동일성"| USECB["useCallb..|"계산 결과 캐싱"| USEMEMO["useMemo"]
...
flowchart LR
    Q["상태 관리 도구 선택"]
    Q -->|"한 컴포넌트만 사용"| US["useState / ..|"몇 개 컴포넌트 공유\n소규모"| CTX["Context AP..|"앱 전반 공유\n간단한 구조"| ZS["Zustand"]
 ..|"복잡한 비즈니스 로직\n대규모 팀"| RDX["Redux Tool..|"서버에서 온 데이터"| RQ["React Query / SWR\"]
