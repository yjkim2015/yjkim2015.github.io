...
```mermaid
graph LR
    APP["App"] -->|"prop drilling"| MID["Layout→Sid..|"theme 전달"| ICON["Icon (실제사용)"]
...
..|"reducer(state, act"| NEW_STATE["새 St..|"상태 업데이트"| VIEW
    MIDDLE..|"비동기 처리"| STORE
...
```mermaid
flowchart LR
    COMP["컴포넌트"] -->|useQuery| RQ["React Query..|"캐시 확인"| CACHE["QueryCac..|"stale 데이터 즉시 반환"| COMP
    RQ -->|"백그라운드 재요청"| API["API Svr"]
..|"새 데이터"| RQ
...
```mermaid
flowchart LR
    A["상태 필요?"] -->|"단일 컴포넌트"| C["useState/use..|"공유 필요"| D{"Svr 데이터?"}
    F ..|"소"| G["Context API"..|"중"| H["Zustand/Jota..|"대"| I["Redux Toolkit"]
```
