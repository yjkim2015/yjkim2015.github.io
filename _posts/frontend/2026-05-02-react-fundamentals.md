...
```mermaid
graph LR
    A["전통"] -->|"DOM 전체 교체"| B["느림/깜빡임"]
    C["React"] --> D["Virtual DOM Diff"]
    D -->|"변경분만"| E["빠름/부드러움"]
```
...
```mermaid
sequenceDiagram
    부모:_user,_posts->>PostList: props
    PostList->>PostItem: props
    UserCard->>부모:_user,_posts: 이벤트
    PostItem->>PostList: 콜백
```
...
### Q. Virtual DOM이 항상 실제 DOM보다 빠른가요?
...
### Q. React의 Reconciliation(재조정) 과정을 설명하세요.
...
### Q. React 18의 Concurrent Mode가 해결하는 문제는?
