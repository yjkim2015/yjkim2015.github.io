...
```mermaid
flowchart TD
    UI["UI 레이어: Atoms → Molecules → Organisms → Pages"]
    STATE["상태 레이어: useState / Zustand / React Query / Router"]
    API["API 레이어: 커스텀 훅 → API 서비스 → axios/fetch"]
    DOMAIN["도메인 레이어: 타입 정의 / 유틸 함수"]
    UI -->|"데이터 요청"| STATE
    STATE -->|"서버 상태"| API
    API -->|"타입 사용"| DOMAIN
```
...
```mermaid
flowchart LR
    A["Atoms: Button, Input, Badge, Icon"]
    M["Molecules: SearchBar, FormField"]
    O["Organisms: Header, ProductCard"]
    T["Templates: ShopLayout"]
    P["Pages: ProductListPage"]
    A --> M --> O --> T --> P
```
...
```mermaid
flowchart TD
    PC["컨테이너: ProductCard (fetch/상태/이벤트)"]
    PV["프레젠테이션: ProductCardView (UI렌더링)"]
    HK["Hooks: useProduct, useAddToCart"]
    API["productApi, cartApi"]
    PC -->|"name,price,isLoading,onAddToCart"| PV
    PC --> HK --> API
```
...
```mermaid
flowchart TD
    C["컴포넌트: ProductList (표시만)"]
    H["훅: useProductList (React Query)"]
    S["API 서비스: productApi.getList()"]
    CL["HTTP 클라이언트: 인터셉터/토큰/에러"]
    NET["백엔드 서버 API"]
    C -->|"useProductList()"| H
...
```mermaid
flowchart LR
    B1["main.bundle.js 3MB (전체)"]
    B2["main.js 200KB (핵심)"]
    B3["admin.chunk 500KB"]
    B4["chart.chunk 800KB"]
    B5["dashboard.chunk 300KB"]
    B1 -->|"코드 스플리팅"| B2
    B2 -.->|"필요시"| B3
    B2 -.->|"필요시"| B4
    B2 -.->|"필요시"| B5
