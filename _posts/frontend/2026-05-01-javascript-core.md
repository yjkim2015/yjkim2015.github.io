...
```mermaid
flowchart TD
    GEC["Global Execution Context (VO/ScopeChain/this=window)"]
    FEC["Function Execution Context (지역변수/매개변수/this)"]
    CS["Call Stack: printSquare/square/multiply/(Global)"]
    GEC --> FEC --> CS
```
...
```mermaid
flowchart TD
    F["makeCounter(10): count=10 생성"] --> RET["increment/decrement/getCount 반환"]
    RET -.->|클로저 참조| ENV["렉시컬환경: count=10 (공유)"]
