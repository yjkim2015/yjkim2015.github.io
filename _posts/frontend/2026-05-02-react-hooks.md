...
### Q. `useState`의 함수형 업데이트(`setState(prev => ...)`)를 언제 사용해야 하나요?
...
### Q. `useReducer`를 `useState` 대신 선택하는 기준은?
