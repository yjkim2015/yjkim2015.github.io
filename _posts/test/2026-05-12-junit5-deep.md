...
```mermaid
graph LR
    IDE["IDE / Build Tool"] -->|실행 요청| Platform["JUnit Platform"]
...
> **비유**: `@ParameterizedTest`는 레시피가 미리 정해진 요리 실습이고, `@TestFactory`는 냉장고를 열어서 재료(런타임 데이터)를 보고 그날 메뉴를 정하는 즉흥 요리다.

```mermaid
graph LR
    FACTORY["@TestFactory"] --> SOURCE["데이터 소스"]
    SOURCE -->|"DB 조회"| DB["동적 시나리오"]
    SOURCE -->|"파일 로드"| FILE["파일 기반 케이스"]
    DB --> DYN["DynamicTest 생성"]
    FILE --> DYN
    DYN --> RUN["JUnit 실행"]
```
...
> **비유**: 테스트 격리는 요리사가 각 요리를 깨끗한 도마에서 만드는 것과 같다. 전 요리의 잔재가 남아 있으면 맛이 섞인다. 각 테스트도 이전 테스트의 상태가 남아 있으면 결과를 신뢰할 수 없다.

```mermaid
graph LR
    PREV["이전 테스트"] --> STATE["공유 상태"]
    STATE -->|"격리 없음"| FAIL["다음 테스트 오염"]
    STATE -->|"@Transactional"| ROLLBACK["자동 롤백"]
    STATE -->|"@DirtiesContext"| RESET["컨텍스트 재생성"]
    ROLLBACK --> CLEAN["격리된 다음 테스트"]
    RESET --> CLEAN
```
...
```mermaid
graph LR
    CODE["코드"] --> HAPPY["정상 경로 Happy Path"]
    CODE --> EDGE["경계값 Edge Case"]
    CODE --> ERR["오류 경로 Error Path"]
    HAPPY --> COV["커버리지 달성"]
    EDGE --> COV
    ERR --> COV
    COV -->|"의미 있는 테스트"| CONF["신뢰 가능한 코드"]
```
...
### Q. JUnit 5의 Extension Model이 JUnit 4의 @RunWith/@Rule과 어떻게 다른가?
...
### Q. ParameterizedTest에서 null을 테스트하는 방법은?
...
### Q. 테스트 커버리지 80%를 달성했는데 버그가 계속 나온다. 왜인가?
...
```mermaid
graph LR
    TM["TestMethod"] -->|"주입 요청"| ME["MockitoExtension"]
    ME -->|"Mock 생성"| MK["Mock"]
    ME -->|"InjectMocks"| TM
    TM -->|"pay(1L)"| SUT["SUT"]
    SUT -->|"findById"| MK
