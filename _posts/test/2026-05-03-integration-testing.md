...
**통합 테스트를 작성하는 이유는 단위 테스트가 개별 컴포넌트는 검증하지만 컴포넌트 간 상호작용의 오류는 발견하지 못하기 때문이다.**

| 대안 | 문제점 | 통합 테스트의 해결 |
|------|--------|-----------------|
| 단위 테스트만 | Mock이 실제 DB/API와 다르게 동작할 수 있음 | 실제 인프라와 연동해 계약 준수 검증 |
| 수동 QA | 느리고 반복 불가, 회귀 발견 늦음 | 자동화로 배포마다 검증 |
| E2E 테스트만 | 느리고 비용 높음, 실패 원인 파악 어려움 | 서비스 경계 단위로 빠른 피드백 |

@SpringBootTest + Testcontainers로 실제 PostgreSQL 컨테이너를 띄워 테스트하면 SQL 쿼리 오류, 제약 조건 위반, 트랜잭션 롤백 동작을 실제와 동일한 환경에서 검증할 수 있다.
