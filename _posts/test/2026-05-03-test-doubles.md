...
> **비유**: Mock을 남용하는 것은 요리사가 실제 재료 없이 가짜 재료로만 연습하는 것과 같다. 연습(테스트)은 통과하지만 실전(운영)에서 진짜 재료를 사용하면 전혀 다른 결과가 나온다.

```mermaid
graph LR
    TEST["테스트 작성"] --> REAL["실제 객체 사용 가능?"]
    REAL -->|"빠르고 결정적"| USE["실제 객체 사용"]
    REAL -->|"느리거나 외부 의존"| DOUBLE["테스트 더블 선택"]
    DOUBLE -->|"단순 응답 필요"| STUB["Stub"]
    DOUBLE -->|"호출 검증 필요"| MOCK["Mock"]
    DOUBLE -->|"실제 동작 필요"| FAKE["Fake"]
```
