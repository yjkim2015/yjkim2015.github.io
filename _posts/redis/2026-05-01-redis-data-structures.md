...
|--------|---------|---------|---------|
| bigkey 수 (`redis-cli --bigkeys`) | 0개 | 1개 이상 증가 추세 | 대용량 List/Set/Hash에 원소 무제한 적재 |
| 메모리 단편화율 (`mem_fragmentation_ratio`) | 1.0~1.5 | 1.5 초과 또는 1.0 미만 | 잦은 키 삭제·생성, active defrag 미설정 |
| slowlog 발생 빈도 | 0건/분 | 1건 이상/분 | KEYS 명령, LRANGE 전체 조회, bigkey DEL |
| used_memory 증가율 | 완만한 상승 | 지속적 가파른 증가 | TTL 없는 키 누적, bigkey 미정리 |
| 컴팩트 인코딩 초과 키 수 | 소수 | 급증 | hash/list/set 임계값 초과 원소 추가 |
| KEYS 명령 호출 수 | 0 | 1 이상 | 개발·운영 코드에서 KEYS 패턴 사용 |
| evicted_keys 카운터 | 0 | 증가 | maxmemory 도달, allkeys-lru 정책 작동 |

## 실제 장애 사례

### 사례 1: bigkey DEL로 Redis 전체 블로킹

**상황**: 이벤트 기간 중 채팅 메시지를 Redis List에 무제한 축적했다. 이벤트 종료 후 배치 정리 스크립트가 `DEL chat:room:event` 명령으로 2,000만 원소짜리 List를 삭제하려 했다.

**근본 원인**: DEL은 동기적으로 메모리를 해제한다. 2,000만 원소 List를 해제하는 데 수 초가 걸렸고, 그 동안 Redis 싱글 스레드가 완전히 블로킹됐다.

**해결책**:
- 즉시 조치: `UNLINK` 명령 사용(비동기 백그라운드 삭제)
- 예방: List 삽입 시 `LTRIM`으로 최대 크기 제한
- 탐지: `redis-cli --bigkeys`를 주기적으로 실행해 bigkey 사전 식별

**교훈**: 10만 원소 이상 키는 UNLINK로만 삭제하고, 모든 List/Set/Hash에는 크기 상한을 설정하라.

---

### 사례 2: KEYS 명령이 운영 환경 전체를 멈춤

**상황**: 개발팀이 "캐시 키 목록 조회" 기능을 `KEYS cache:*`로 구현했다. 개발 환경에서는 키 1,000개로 문제없었다. 운영 배포 후 키가 500만 개로 늘었을 때 배치 작업이 `KEYS`를 실행했다.

**근본 원인**: KEYS는 전체 키를 순회하는 O(N) 블로킹 명령어다. 500만 개 키 순회에 3초 이상 걸렸고, 그 동안 모든 Redis 요청이 타임아웃됐다.

**해결책**:
- `SCAN cursor MATCH cache:* COUNT 100`으로 커서 기반 분할 조회 전환
- CODE REVIEW 체크리스트에 "KEYS 명령 사용 금지" 항목 추가
- Redis 설정에 `rename-command KEYS ""` 으로 명령 자체 비활성화

**교훈**: KEYS는 코드 리뷰에서 반드시 차단해야 한다. 운영에서는 항상 SCAN을 쓴다.

---

### 사례 3: String+JSON 과다 사용으로 메모리 2배 증가

**상황**: 사용자 프로필을 JSON 직렬화해 String에 저장했다. 프로필 필드가 20개로 늘어나면서 Redis 메모리 사용량이 예측보다 2배 높았다. 서비스 사용자 1,000만 명의 프로필 데이터가 30GB를 차지했다.

**근본 원인**: JSON에는 필드 이름이 문자열로 매번 포함된다. `"name"`, `"email"`, `"age"` 등 필드 이름이 1,000만 번 반복 저장됐다. Hash는 listpack 인코딩으로 메모리를 효율적으로 압축한다.

**해결책**:
- String+JSON에서 Hash로 점진적 마이그레이션
- listpack 임계값 확인 및 적정 조정(`hash-max-listpack-entries 128`)
- 마이그레이션 후 메모리 30% 절감 확인

**교훈**: 필드 단위 업데이트가 있는 구조화 데이터는 Hash가 메모리와 성능 모두 유리하다.
...
게임 서버에서 1억 명 유저의 실시간 랭킹을 운영합니다. 이벤트 기간 중 초당 50만 건 점수 업데이트가 발생합니다.

**DB로 접근 시 문제:**
- `SELECT rank FROM (SELECT user_id, RANK() OVER (ORDER BY score DESC)) WHERE user_id = ?`
- 1억 행 테이블에서 순위 계산: 매 조회마다 Full Scan 또는 대규모 정렬 → 수백 ms
- 초당 50만 건 UPDATE는 MySQL 단일 인스턴스 한계(약 2만 TPS)를 25배 초과

**Redis Sorted Set으로 해결:**
```java
// 점수 업데이트: O(log N)
redisTemplate.opsForZSet().add("game:leaderboard", userId, newScore);

// 순위 조회: O(log N)
Long rank = redisTemplate.opsForZSet()
    .reverseRank("game:leaderboard", userId);  // 0-based, 1등 = 0

// 상위 100명 조회: O(log N + 100)
Set<ZSetOperations.TypedTuple<String>> top100 = redisTemplate.opsForZSet()
    .reverseRangeWithScores("game:leaderboard", 0, 99);

// 성능: 점수 업데이트 < 1ms, 순위 조회 < 1ms
// 초당 50만 TPS: Redis 클러스터 3노드로 처리 가능
```

**실전 수치:**
- Redis Sorted Set ZADD 성능: 단일 노드에서 약 15만 TPS
- 50만 TPS: 샤딩 필요 → `game:leaderboard:{shardId}` (유저 ID % 4)
- 전체 순위가 필요한 경우: 각 샤드의 상위 N을 합산 후 최종 정렬

### 시나리오 2: 세션 스토어 — 쇼핑몰 블랙프라이데이 100만 동시 접속

블랙프라이데이 피크 시 100만 명이 동시에 로그인 상태를 유지합니다. 세션당 평균 2KB 데이터.

**메모리 계산:**
- 100만 세션 × 2KB = 2GB
- Redis 단일 노드 메모리 8GB → 여유 있음
- 단, 세션 만료(TTL) 없으면 메모리 계속 증가
...
public RedisSessionRepository sessionRepository(RedisOperations<String, Object> sessionRedisOperations) {
    ...
    repository.setDefaultMaxInactiveInterval(Duration.ofMinutes(30));  // 30분 비활성 만료
    return repository;
}

// 세션 저장 구조: HASH 타입 사용
// KEY: "spring:session:sessions:{sessionId}"
// FIELD: "sessionAttr:userId", "sessionAttr:cart", "lastAccessedTime"
// TTL: 1800초 (30분)
```

**장애 시나리오:** Redis 다운 시 100만 세션 전체 소실 → 전원 재로그인 필요
**대응:** Redis Sentinel(고가용성) + `spring.session.redis.save-mode=on_set_attribute`로 최소 쓰기

### 시나리오 3: 재고 차감 — 한정판 상품 1만 개, 초당 10만 요청

한정판 운동화 1만 켤레가 오픈 동시에 초당 10만 건 구매 요청.

```java
// Lua 스크립트로 원자적 재고 차감
private static final String DECREASE_STOCK_SCRIPT =
    "local stock = redis.call('GET', KEYS[1]) " +
    "if not stock then return -1 end " +       // 상품 없음
    "stock = tonumber(stock) " +
    "if stock <= 0 then return 0 end " +        // 재고 소진
    "redis.call('DECR', KEYS[1]) " +
    "return stock - 1";

public long decreaseStock(Long productId) {
    DefaultRedisScript<Long> script = new DefaultRedisScript<>(DECREASE_STOCK_SCRIPT, Long.class);
    return redisTemplate.execute(script,
        Collections.singletonList("stock:" + productId));
    // 반환값: -1(상품없음), 0(재고소진), N(남은재고)
}
// 처리 속도: 단일 Redis 노드에서 10만 TPS 처리 가능
// DB에는 Kafka Consumer가 1초마다 배치로 실제 재고 반영
```

**이걸 안 하면 생기는 장애:** DB의 `UPDATE stock SET quantity = quantity - 1 WHERE quantity > 0`를 직접 호출하면 초당 10만 건의 row-level lock 경합으로 DB 응답이 수십 초로 폭증하고 재고 오버셀이 발생합니다.

---

## 왜 이 자료구조인가

**Redis가 다양한 자료구조를 제공하는 이유는 문제에 맞는 구조를 쓰면 애플리케이션 코드가 단순해지고 성능이 올라가기 때문이다.**

| 자료구조 | 핵심 적용 사례 | 이유 |
|---------|-------------|------|
| String | 카운터, 캐시, 세션 | INCR 원자성, 단순 키-값 |
| Hash | 사용자 프로필, 객체 필드 | 객체 전체 직렬화 없이 필드별 접근 |
| List | 작업 큐, 최근 활동 피드 | LPUSH/RPOP으로 FIFO, BRPOP 블로킹 팝 |
| Sorted Set | 실시간 랭킹, 우선순위 큐 | score 기반 O(log N) 정렬 유지 |
| Set | 태그, 친구 목록, 중복 제거 | SINTER/SUNION으로 교집합/합집합 |

String + JSON 직렬화 대신 Hash를 쓰면 특정 필드만 업데이트할 때 전체 객체를 읽고 쓰는 오버헤드가 없다. Sorted Set은 ZADD/ZRANGE만으로 실시간 순위를 O(log N)에 유지한다.
