...
```mermaid
sequenceDiagram
    Sentinel_1->>Redis_Master_:6379: PING 감시
    Sentinel_2->>Redis_Master_:6379: PING 감시
    Sentinel_3->>Redis_Master_:6379: PING 감시
    Sentinel_1->>Replica_1_:6379: PING 감시
    Sentinel_2->>Replica_1_:6379: PING 감시
    Sentinel_3->>Replica_1_:6379: PING 감시
    Sentinel_1->>Replica_2_:6379: PING 감시
    Sentinel_2->>Replica_2_:6379: PING 감시
    Sentinel_3->>Replica_2_:6379: PING 감시
    Redis_Master_:6379->>Replica_1_:6379: 비동기 복제
    Redis_Master_:6379->>Replica_2_:6379: 비동기 복제
```
...
```mermaid
sequenceDiagram
    ReplicaA->>MasterC: 투표 요청
    MasterB->>ReplicaA: 승인
    MasterC->>ReplicaA: 승인
    ReplicaA->>ReplicaA: 새 마스터 승격
```
...
|--------|---------|---------|---------|
| 페일오버 소요 시간 | 30초 이내 | 60초 초과 | `down-after-milliseconds` 너무 크거나 quorum 미충족 |
| 클러스터 상태 (`CLUSTER INFO cluster_state`) | ok | fail | 마스터+레플리카 동시 장애, 슬롯 미커버 |
| Sentinel ODOWN 오탐 빈도 | 0 | 증가 | `down-after-milliseconds` 너무 작아 일시 지연도 ODOWN 판정 |
| 슬롯 커버리지 | 100% | 100% 미만 | 마스터 장애 후 레플리카 승격 실패 |
| 노드 간 repl_offset 차이 | 소량 | 급증 | 네트워크 분리, 레플리카 처리 지연 |
| MOVED 리다이렉션 비율 | 0% | 증가 | 클러스터 토폴로지 변경 후 클라이언트 캐시 미갱신 |
| 핫 슬롯 QPS 집중도 | 균등 분산 | 단일 슬롯에 80%+ 집중 | 해시 태그 과도 사용으로 데이터 편중 |

## 실제 장애 사례

### 사례 1: Sentinel 짝수 구성으로 Split Brain 발생

**상황**: 비용 절감을 위해 Sentinel을 2대로 구성했다(`quorum=2`). 네트워크 스위치 점검으로 Sentinel 1대가 10초간 격리됐다. 격리된 Sentinel이 마스터를 ODOWN으로 판정해 단독으로 failover를 시도했고, 레플리카를 새 마스터로 승격했다. 네트워크가 복구된 후 마스터가 2대인 Split Brain 상태가 됐다. 두 마스터에 동시 쓰기가 발생해 약 1분치 데이터가 충돌했다.

**근본 원인**: Sentinel 2대에 `quorum=2`를 설정하면 1대만 격리돼도 quorum이 충족된다. 격리된 Sentinel 1대가 단독으로 ODOWN 판정과 failover를 수행할 수 있었다. Sentinel은 반드시 홀수 대(3, 5, 7)로 구성해야 Split Brain을 방지할 수 있다.

**해결책**:
- Sentinel을 3대로 증설하고 `quorum=2` 유지(1대 장애 허용)
- 각 Sentinel을 서로 다른 물리 서버/가용 영역에 배치
- `min-replicas-to-write 1` 설정으로 마스터 단독 시 쓰기 거부

**교훈**: Sentinel은 반드시 홀수 대로 구성하라. 짝수 구성은 Split Brain의 씨앗이다.

---

### 사례 2: 클러스터 노드 추가 후 리샤딩 미실시로 신규 노드 무부하

**상황**: 트래픽 증가로 Redis Cluster에 마스터 노드 2대를 추가했다. 이후에도 기존 3대의 CPU가 계속 높았고, 새로 추가한 2대는 CPU가 거의 0%였다. 모니터링에서 신규 노드에 슬롯이 0개임을 뒤늦게 발견했다.

**근본 원인**: `redis-cli --cluster add-node`는 노드를 클러스터에 참가시킬 뿐 슬롯을 자동으로 배분하지 않는다. 리샤딩(`reshard`)을 별도로 실행해야 기존 노드에서 슬롯 일부를 이전받는다. 슬롯 없는 노드는 어떤 요청도 처리하지 않는다.

**해결책**:
- `redis-cli --cluster reshard <existing-node>:7000` 실행으로 슬롯 균등 재분배
- 노드 추가 후 `redis-cli --cluster check`로 슬롯 분배 상태 즉시 검증
- 배포 매뉴얼에 "노드 추가 → 리샤딩 → 검증" 3단계를 필수 절차로 문서화

**교훈**: 노드 추가와 리샤딩은 별개 작업이다. 추가만 하고 리샤딩을 잊으면 비용만 쓰고 효과는 없다.

---

### 사례 3: `cluster-require-full-coverage yes`로 일부 장애가 전체 다운으로 확대

**상황**: 6노드 클러스터에서 마스터 1대와 그 레플리카가 동시에 네트워크 장애로 격리됐다. 해당 노드가 담당하던 슬롯(약 5,461개)에 대한 접근이 불가해졌다. 예상과 달리 나머지 정상 슬롯에 대한 읽기/쓰기도 전부 `CLUSTERDOWN` 에러로 실패했다. 전체 서비스가 중단됐다.

**근본 원인**: `cluster-require-full-coverage yes`(기본값)는 클러스터 내 단 하나의 슬롯이라도 커버되지 않으면 클러스터 전체를 에러 상태로 전환한다. 이 설정이 부분 장애를 전체 장애로 확대했다.

**해결책**:
- `cluster-require-full-coverage no`로 변경하면 장애 슬롯을 제외한 나머지는 정상 서비스
- 마스터마다 최소 1개 레플리카를 유지해 단일 노드 장애 시 자동 승격 보장
- 클러스터 노드 간 네트워크를 이중화(본딩, 이중 스위치)

**교훈**: `cluster-require-full-coverage yes`는 데이터 정합성을 위한 설정이지만 가용성을 희생한다. 부분 서비스가 전체 중단보다 낫다면 `no`로 설정하라.
