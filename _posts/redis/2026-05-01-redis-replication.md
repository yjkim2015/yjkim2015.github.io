...
|--------|---------|---------|---------|
| replication lag (`lag` in INFO replication) | 0~1초 | 3초 초과 지속 | 마스터 쓰기 TPS 급증, 네트워크 대역폭 부족 |
| master_repl_offset - slave_offset (바이트 차이) | 소량 | 지속 증가 | 레플리카 처리 속도 < 마스터 쓰기 속도 |
| 전체 재동기화 횟수 (`full_sync_count`) | 드묾 | 빈번 (하루 1회+) | repl-backlog-size 부족, 네트워크 불안정 |
| 연결된 레플리카 수 (`connected_slaves`) | 설정값 유지 | 감소 | 레플리카 다운, 네트워크 파티션 |
| repl_backlog 잔여 비율 | 50% 이상 여유 | 80% 초과 사용 | backlog 크기 부족, 높은 쓰기 TPS |
| WAIT 명령 응답 레플리카 수 | 설정 numreplicas | 0 | 레플리카 지연 심각, 네트워크 단절 |

## 실제 장애 사례

### 사례 1: 전체 재동기화 폭풍으로 마스터 OOM

**상황**: 배포 파이프라인이 레플리카 5대를 10초 간격으로 순차 재시작했다. 각 레플리카가 재연결 시 repl-backlog(1MB, 기본값)를 초과해 Full Sync를 요청했다. 마스터가 동시에 5개의 RDB 스냅샷 생성을 시도했고, fork()로 인한 메모리 사용량이 2배로 치솟아 OOM Killer가 마스터 프로세스를 종료했다.

**근본 원인**: `repl-backlog-size`가 기본값 1MB로 너무 작았다. 재시작 10초 동안 발생한 쓰기 데이터가 1MB를 초과하면 Partial Sync가 불가하고 Full Sync로 폴백된다. 동시 Full Sync 5개 = 5번의 fork = 메모리 5배 압박.

**해결책**:
- `repl-backlog-size 512mb`로 backlog를 충분히 확보
- 배포 스크립트에서 레플리카 재시작 간격을 60초 이상으로 분산
- `repl-diskless-sync yes`로 RDB를 디스크 없이 소켓으로 직접 전송(메모리 절약)

**교훈**: repl-backlog-size 기본값 1MB는 현대 서비스에 턱없이 부족하다. 최소 수백 MB로 설정하라.

---

### 사례 2: 비동기 복제 지연으로 failover 후 데이터 유실

**상황**: 마스터 장애 후 Sentinel이 레플리카를 승격시켰다. 새 마스터로 서비스가 재개됐는데 최근 5분치 주문 데이터 약 200건이 사라졌다는 것이 익일 정산에서 발견됐다.

**근본 원인**: 마스터 장애 직전 5분 동안 레플리카 복제 지연이 30초까지 벌어져 있었다(높은 TPS로 인한 backlog 누적). Sentinel이 레플리카를 승격할 때 그 30초치 쓰기는 복제되지 않은 상태였다.

**해결책**:
- `min-replicas-to-write 1`, `min-replicas-max-lag 10` 설정으로 복제 지연 10초 초과 시 쓰기 거부
- 중요 쓰기 후 `WAIT 1 500` 명령으로 최소 1개 레플리카 동기화 확인
- 주문 데이터는 Redis 외 MySQL에도 동기 저장하는 이중 기록 적용

**교훈**: 비동기 복제에서 데이터 유실은 장애가 아닌 설계 특성이다. 유실 불가 데이터는 Redis를 유일한 저장소로 쓰면 안 된다.

---

### 사례 3: 레플리카 읽기에서 stale 데이터 반환으로 주문 오류

**상황**: 읽기 부하를 줄이기 위해 상품 재고 조회를 레플리카에서 처리하도록 설정했다. 주문 완료 직후 재고 확인 페이지에서 이전 재고가 그대로 표시됐다. 일부 사용자가 "재고 있다고 나왔는데 품절"이라는 클레임을 제기했다.

**근본 원인**: 복제 지연이 평균 200ms였다. 마스터에 재고를 차감한 직후 레플리카에서 읽으면 200ms 동안 이전 재고를 반환했다. 레플리카 응답 속도가 더 빠른 경우 애플리케이션이 레플리카를 선택했다.

**해결책**:
- 재고 차감 직후 읽기는 강제로 마스터 연결 사용(`ReadFrom.MASTER`)
- Spring Data Redis의 `@ReadPreference(MASTER)` 어노테이션 적용
- 정합성이 중요한 읽기와 그렇지 않은 읽기를 코드 레벨에서 명시적으로 구분

**교훈**: 레플리카 읽기는 "eventually consistent"다. 방금 쓴 데이터를 즉시 읽어야 한다면 반드시 마스터에서 읽어야 한다.
