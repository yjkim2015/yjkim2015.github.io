...
**위임(Delegation)은 상속의 대안이다.** 상속이 "나는 ~이다(is-a)" 관계라면, 위임은 "나는 ~를 갖고 그 일은 그 객체가 한다(has-a + forwarding)" 관계다.

비유: 프랜차이즈 본사와 가맹점을 생각해보자. 가맹점(클래스)은 메뉴 개발, 물류, 마케팅을 직접 하지 않는다. 본사(위임 객체)에 맡기고 결과만 받는다. `by` 키워드는 "이 메서드는 저 객체에게 맡긴다"는 선언이다. 가맹점이 특별히 바꾸고 싶은 것만 `override`로 재정의하면 된다.

```mermaid
graph LR
    A[클래스 위임] --> B["by 위임객체"]
    B --> C[인터페이스 자동 포워딩]
    D[프로퍼티 위임] --> E["by 위임프로퍼티"]
    E --> F["getValue/setValue 호출"]
```
...
```mermaid
graph LR
    A["프로퍼티 접근"] --> B{"_value 초기화?"}
    B -->|예| C["캐시값 반환"]
    B -->|아니오| D["synchronized 진입"]
    D --> E{"다시 확인"}
    E -->|이미 초기화| C
    E -->|미초기화| F["initializer 실행"]
    F --> G["_value 저장 후 반환"]
```
...
### Q. observable과 vetoable의 차이점은 무엇인가
