...
Planning이 왜 필요한지는 이사 준비를 비유로 이해할 수 있다. "이사해"라는 지시를 받은 사람이 짐 싸기 → 트럭 예약 → 주소 변경 → 인터넷 이전 신청을 **무작정 순서대로** 처리하면 비효율적이다. 트럭 예약과 인터넷 이전 신청은 동시에 할 수 있다. Planning은 전체 목표를 하위 태스크로 분해하고, 의존 관계를 파악해 독립적인 작업을 병렬로 처리한다.
...
<details>
<summary>Q. ReAct 패턴에서 Thought가 왜 중요한가?</summary>
...
<details>
<summary>Q. Tool Use에서 LLM은 함수를 직접 실행하는가?</summary>
...
<details>
<summary>Q. 단기 기억과 장기 기억을 함께 쓸 때 주의점은?</summary>
...
<details>
<summary>Q. Multi-Agent에서 오케스트레이터가 병목이 되는 상황을 어떻게 해결하는가?</summary>
...
<details>
<summary>Q. 에이전트 평가는 어떻게 하는가?</summary>
