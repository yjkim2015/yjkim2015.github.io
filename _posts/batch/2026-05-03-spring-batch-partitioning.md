...
flowchart LR
    ...
    B -->|파티션1| C["Worker1: R→P..|파티션2| D["Worker2: R→P..|파티션3| E["Worker3: R→P..|완료 응답| B
```
...
graph LR
    A["성능 개선"] --> B{"단일 Step?"}
    B -->|"No"| F["Parallel Ste..|"Yes"| C{"단일 JVM?"}
  ..|"Yes"| L["Local 파티셔닝"]..|"No"| K["Remote 파티셔닝"]
```
