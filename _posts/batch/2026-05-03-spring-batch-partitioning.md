...
```mermaid
flowchart TD
    A["Master(Partitioner)"] --> B["MessageChannel(Kafka)"]
...
```mermaid
flowchart TD
    A["성능 개선 필요"] --> B{"단일 Step?"}
    B -->|"아니오"| F["Parallel Step"]
    B -->|"예"| C{"Reader thread-safe?"}
    C -->|"예"| D["Multi-threaded Step"]
    C -->|"아니오"| E{"단일 JVM?"}
    D -->|"부족"| E
    E -->|"예"| L["Local Partitioning"]
    E -->|"아니오"| K["Remote Partitioning"]
```
