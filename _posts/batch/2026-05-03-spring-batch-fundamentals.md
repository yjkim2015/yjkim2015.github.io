...
```mermaid
flowchart LR
    A["ItemReader"] -->|"chunk-size 반복"| B["ItemProcesso..|"List로 모아서"| C["ItemWriter"]..|"커밋 후 다음"| A
```
...
```mermaid
erDiagram
    JOB_INSTANCE ||--o{ JOB_EXECUTI..||--o{ STEP_EXECUT..||--o{ JOB_PARAMS : "파라미터"
```
