...
flowchart TD
    ...
    B --> C["Step(s)"]
    C -->|"Tasklet"| F["단일 작업"]
    C -->|"Chunk"| G["Reader → Processor → Writer"]
    B --- J[("JobRepository")]
```
...
```mermaid
erDiagram
    BATCH_JOB_INSTANCE { bigint JOB_INSTANCE_ID PK }
    BATCH_JOB_EXECUTION { bigint JOB_EXECUTION_ID PK }
    BATCH_STEP_EXECUTION { bigint STEP_EXECUTION_ID PK }
    BATCH_JOB_EXECUTION_PARAMS { bigint JOB_EXECUTION_ID FK }
