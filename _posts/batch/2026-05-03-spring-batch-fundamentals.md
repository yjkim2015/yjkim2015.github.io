...
ItemReader →(chunk-size 반복)→ ItemProcessor →(List로 모아서)→ ItemWriter →(커밋 후 다음)→ 다시 Reader로 순환
...
erDiagram JOB_INSTANCE ||--o{ JOB_EXECUTION : "실행" JOB_EXECUTION ||--o{ STEP_EXECUTION : "Step"
