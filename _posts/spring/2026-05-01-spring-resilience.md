...
```mermaid
sequenceDiagram
    CLOSED_정상->>OPEN_차단: 실패율 임계치 초과
    OPEN_차단->>HALF_OPEN_테스트: 대기 시간 경과
    HALF_OPEN_테스트->>CLOSED_정상: 시험 요청 성공
    HALF_OPEN_테스트->>OPEN_차단: 시험 요청 실패
```
...
```mermaid
sequenceDiagram
    Bulkhead->>Circuit_Breaker: 호출
    Circuit_Breaker->>Retry: 호출
    Retry->>서비스_호출: 호출
    서비스_호출->>Retry: 실패
