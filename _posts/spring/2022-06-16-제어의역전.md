...
```mermaid
graph TD
    DEV["개발자(기존)"] --> A["new UserService()"]
    DEV --> B["new MySQLRepository()"]
    DEV --> C["service.setRepository(repo)"]
    CTX["스프링 컨테이너(IoC)"] --> D["UserService 빈 생성"]
    CTX --> E["MySQLRepository 빈 생성"]
    CTX --> F["의존관계 자동 주입"]
    APP["개발자(IoC)"] --> G["context.getBean(UserService)"]
    CTX --> G
```
...
sequenceDiagram
    ...
    participant BL as BeanRegistry
    D->>AC: new AnnotationConfigApplicationContext(AppConfig)
    AC->>AC: @Configuration 분석
    AC->>BL: @Bean 목록 등록(userService, userRepository)
    D->>AC: getBean("userService")
    AC->>BL: "userService" 검색
    BL-->>AC: 빈 정의 반환
    AC-->>D: 생성된 빈(싱글톤) 반환
```
