...
### Q. 파티션 경계를 ID 범위로 나누는 이유와 주의점은?

날짜, 카테고리 등 분포가 불균형한 기준으로 파티션을 나누면 특정 파티션이 다른 파티션보다 훨씬 많은 데이터를 처리해 전체 실행 시간이 가장 느린 파티션에 종속된다. ID(PK)는 일반적으로 순차 생성되므로 `minId~maxId` 범위로 나누면 각 파티션의 크기가 균등하다. 주의점은 범위 경계 설정이다. `minId <= id <= maxId`로 정의하면 파티션 경계 ID가 인접한 두 파티션에 중복 포함되어 중복 처리가 발생한다. 상한 미포함 방식(`minId <= id < maxId`)이나 마지막 파티션만 `Long.MAX_VALUE`를 상한으로 설정하는 방식으로 방지해야 한다.

### Q. 로컬 파티셔닝과 원격 파티셔닝의 차이는?

로컬 파티셔닝은 `TaskExecutor`를 사용해 동일 JVM 내 별도 스레드에서 Worker Step을 병렬 실행한다. 설정이 간단하고 JVM 하나의 CPU와 메모리를 최대한 활용한다. 원격 파티셔닝은 Master가 파티션 정보를 메시지 큐(Kafka, RabbitMQ)나 원격 호출로 여러 Worker 서버에 전달한다. 단일 서버의 자원 한계를 넘어 수평 확장이 가능하다. 단, 네트워크 장애, Worker 서버 실패 처리, Master-Worker 통신 오버헤드가 추가된다. 처리 시간이 2~3시간을 초과하거나 단일 서버 자원이 한계에 달하면 원격 파티셔닝을 고려한다.

### Q. @StepScope가 파티셔닝에서 필수적인 이유는?

파티셔닝에서 각 Worker Step은 서로 다른 파티션 범위(minId, maxId)의 `ExecutionContext`를 갖는다. `@StepScope` 없이 `ItemReader`를 싱글톤으로 만들면 모든 Worker Step이 같은 `ItemReader` 인스턴스를 공유한다. 스레드마다 다른 파라미터(minId, maxId)로 초기화해야 하는데 공유 인스턴스는 하나의 파라미터만 가질 수 있다. `@StepScope`는 Step 실행마다 새 인스턴스를 생성하므로, 각 Worker Step이 독립적인 `ItemReader` 인스턴스와 파라미터를 갖는다. `@Value("#{stepExecutionContext['minId']}")`로 Step별 파라미터를 주입받는다.

### Q. 파티션 수를 어떻게 결정하는가?

파티션 수 = Worker 스레드(또는 서버) 수와 일치시키는 것이 기본이다. 스레드가 8개면 파티션도 8개로 나누면 각 스레드가 하나의 파티션을 담당한다. 파티션을 스레드보다 많이(예: 스레드 8개, 파티션 32개) 설정하면 하나의 스레드가 완료 후 다음 파티션을 처리해 부하가 균등해지는 동적 부하 분산 효과가 있다. 처리 시간 차이가 큰 파티션이 있을 때 유용하다. 파티션이 너무 많으면 `JobRepository`에 저장되는 `StepExecution` 레코드가 늘어나고 관리 오버헤드가 증가한다. 보통 스레드 수의 2~4배가 적정하다.

### Q. 원격 파티셔닝에서 Worker 서버가 중간에 죽으면 어떻게 되는가?

Worker 서버 장애 시 해당 Worker가 담당하던 파티션의 `StepExecution`이 `STARTED` 상태로 남는다. Master는 모든 파티션 완료를 기다리므로 타임아웃까지 대기한다. 재시작 시 `STARTED` 상태의 파티션을 `FAILED`로 변경하고, 해당 파티션부터 재처리한다. `ItemReader`가 `saveState=true`이면 청크 오프셋도 복원되어 파티션 내 처리 위치부터 재개한다. 원격 파티셔닝 환경에서는 메시지 큐의 `ack` 전략이 중요하다. Worker가 처리 완료 전에 `ack`를 보내면 장애 시 해당 파티션이 처리됐다고 오판할 수 있다.
