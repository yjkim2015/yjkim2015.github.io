...
flowchart LR
    ...
    B -->|"예 — 없으면 동작 불가"| C["1️⃣ Create자 ..|"아니오 — 없어도 됨"| D{"런타임 변경 필요?"}..|"예"| E["2️⃣ Update자 ..|"아니오"| C
    C --> G["장점: final, 테스트 쉬움,"]
    E --> H["장점: 선택적 의존관계,"]
```
...
sequenceDiagram
    ...
    Client->>Singleton: logic() 요청1
    Singleton->>Prototype: count++ = 1
    Client->>Singleton: logic() 요청2
...
```mermaid
graph LR
    SVC1["UserService"] -->|컴파일타임 의존| IFACE["UserRepo..|런타임 생성| IMPL["MySQLUser..|생성+주입| SVC2["UserServi..|실제 사용| IMPL
```
...
| 방식 | 의존성 관리 | 생명주기 | 테스트 용이성 | 적합한 상황 |
|---|---|---|---|---|
| new 직접 생성 | 수동 | 수동 | 어려움 | 단순 유틸 클래스 |
| 정적 팩토리 + 수동 DI | 수동 | 수동 | 중간 | 프레임워크 없는 Java |
| Spring IoC + DI | 자동 | 자동 | 쉬움 | 실무 표준 |
| Guice | 자동 | 자동 | 쉬움 | Google 생태계 |

**결론:** Spring IoC/DI는 객체 생성과 의존성 주입을 컨테이너가 관리해 개발자가 비즈니스 로직에만 집중하게 한다. `@PostConstruct`/`@PreDestroy`, Bean Scope 등 생명주기 관리까지 자동화해 엔터프라이즈 애플리케이션의 복잡도를 크게 낮춘다.

---

## 실무에서 자주 하는 실수 (보강)

1. **싱글톤 빈에 상태(인스턴스 변수) 보관** — 싱글톤 빈은 모든 스레드가 공유한다. `private int count = 0;` 같은 인스턴스 변수는 멀티스레드 환경에서 경쟁 조건(race condition)을 유발한다. 상태는 요청 스코프 빈이나 메서드 로컬 변수로 관리해야 한다.

2. **`@Component` 스캔 경로 외 클래스에 어노테이션 적용** — `@SpringBootApplication`이 있는 패키지 하위에 없는 클래스는 스캔되지 않는다. 라이브러리 내 클래스에 `@Component`를 붙여도 등록되지 않는다. `@Import`나 `@Bean`으로 명시적으로 등록해야 한다.

3. **`@PostConstruct`에서 예외 발생 시 컨텍스트 초기화 실패** — `@PostConstruct` 메서드에서 외부 API를 호출하거나 DB를 쿼리하면, 해당 서비스가 불안정할 때 애플리케이션 시작 자체가 실패한다. 초기화 로직에는 예외 처리나 재시도 로직이 필요하다.

4. **프로토타입 빈을 싱글톤 빈에 주입** — 싱글톤 빈에 `@Autowired`로 프로토타입 빈을 주입하면, 싱글톤 초기화 시 단 한 번만 주입되어 프로토타입의 의미가 없어진다. `ObjectProvider<PrototypeBean>`이나 `ApplicationContext.getBean()`으로 매번 새 인스턴스를 요청해야 한다.

5. **`@Autowired` 필드 주입으로 테스트 어려움** — 필드 주입은 `new MyService()`로 인스턴스를 생성한 후 의존성을 주입할 방법이 없다. 생성자 주입을 사용하면 테스트에서 `new MyService(mockRepo)`처럼 의존성을 직접 제어할 수 있다.

---

## 면접 포인트 (보강)

**Q. `@Bean`과 `@Component`의 차이는?**
> `@Component`는 클래스에 붙이고 컴포넌트 스캔으로 자동 등록된다. `@Bean`은 `@Configuration` 클래스의 메서드에 붙이고, 메서드가 반환하는 객체를 빈으로 등록한다. 직접 소스를 수정할 수 없는 외부 라이브러리 클래스를 빈으로 등록할 때는 `@Bean`을 사용해야 한다.
