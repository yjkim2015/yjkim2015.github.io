...
```mermaid
graph TD
    A["BeanFactory (interface)\ngetBean / isSingleton"]
    B["ApplicationContext (interface)\npublishEvent / getMessage"]
...
flowchart TD
    ...
    H -->|"예"| D
    H -->|"아니오"| K["NoUniqueBeanDefinitionException"]
```
...
```mermaid
graph TD
    SVC1["UserService"] -->|컴파일타임 의존| IFACE["UserRepository 인터페이스"]
    CTX["스프링 컨테이너"] -->|런타임 생성| IMPL["MySQLUserRepository"]
