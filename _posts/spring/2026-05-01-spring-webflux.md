...
├─ YES
│   ├─ Java 21 이상 사용 가능한가?
│   │   ├─ YES → Spring MVC + Virtual Thread 우선 검토
│   │   │         (코드 단순성, 팀 생산성)
│   │   │   └─ 단, 다음 조건이라면 WebFlux 선택:
│   │   │         ① SSE/WebSocket 중심 아키텍처
│   │   │         ② 완전한 리액티브 스택 (R2DBC 등)
│   │   │         ③ 팀이 리액티브 경험 보유
│   │   └─ NO → Java 17 이하
│   │             ├─ 높은 동시성 필요? → WebFlux
│   │             └─ 그 외 → Spring MVC
│
└─ NO (기존 MVC 프로젝트 마이그레이션)
    ├─ 성능 문제가 있는가?
    │   ├─ NO → 현행 유지
    │   └─ YES → Java 21 업그레이드 + Virtual Thread 먼저 시도
    │              (코드 변경 최소화)
    │              ├─ 해결됨 → 완료
    │              └─ 더 필요함 → WebFlux 점진적 마이그레이션 검토
    └─ 리액티브 외부 의존성 추가됨 → WebFlux 필요 고려
```
