...
..|Yes+MSA| E["OpenFeign"]
..|Yes+동기| F["RestClient"]..|레거시+동기| H["RestTemplate..|레거시+비동기| C
```
...
| 클라이언트 | 블로킹 | 선언적 | Spring Boot 통합 | 적합한 상황 |
|---|---|---|---|---|
| RestTemplate | O | X | 완벽 (deprecated 예정) | 레거시 MVC 유지보수 |
| WebClient | X | X | 완벽 | MVC + WebFlux 모두 |
| RestClient (Spring 6.1+) | O | X | 완벽 | MVC 신규 개발 표준 |
| OpenFeign | O | O | 완벽 | 선언적 HTTP 클라이언트 |
| Retrofit | O/X | O | 보통 | Android, 비Spring 환경 |
| Java HttpClient (JDK 11+) | O/X | X | 없음 | 라이브러리 없는 순수 Java |

**결론:** Spring MVC 신규 프로젝트는 `RestClient`(Spring 6.1+)를 표준으로 사용한다. 마이크로서비스 간 호출이 많고 선언적 인터페이스가 필요하면 `OpenFeign`이 적합하다. WebFlux 환경에서는 `WebClient`가 필수다.
