...
```mermaid
graph LR
    REQ1["요청 1"] --> OBJ1["UserService 인스턴스 1"]
    REQ2["요청 2"] --> OBJ2["UserService 인스턴스 2"]
    REQ3["요청 3"] --> OBJ3["UserService 인스턴스 3"]
    REQ_N["요청 N"] --> OBJ_N["UserService 인스턴스 N"]
    OBJ_N --> PROBLEM["문제: 수백 객체 생성 → GC"]
```
...
graph LR
    ...
    REQ3["요청 3"] --> SINGLE
    REQ_N["요청 N"] --> SINGLE
    SINGLE["UserService 인스턴스(싱"]
    BENEFIT["장점: 객체 재사용, GC부하없음"]
```
...
```mermaid
graph LR
    C["Client"] -->|"getBean('userDao')"| AC["ApplicationContext"]
    AC -->|"캐시 확인→없음"| SR["SingletonRegistry"]
    AC -->|"UserDao 생성"| AC
    AC -->|"레지스트리 등록"| SR
    AC -->|"인스턴스 반환"| C
    C -->|"재요청"| AC
