...
```mermaid
graph LR
    REQ1["요청 1"] --> OBJ1["UserService 인스턴스1"]
    REQ2["요청 2"] --> OBJ2["UserService 인스턴스2"]
    REQ3["요청 N"] --> OBJ3["UserService 인스턴스N"]
    OBJ3 --> PROBLEM["수백 객체 → GC 부하"]
```

```mermaid
graph LR
    REQ1["요청 1"] --> SINGLE["UserService 싱글톤"]
    REQ2["요청 2"] --> SINGLE
    REQ3["요청 N"] --> SINGLE
    SINGLE --> BENEFIT["객체 재사용, GC부하 없음"]
```
