...
graph TD
    ...
    HA --> IC["Interceptor(preHandle)"]
    IC --> CTRL["Controller(ModelAndView)"]
...
```mermaid
graph LR
    OLD["전통 MVC"] --> O["web.xml + servlet-context.xml\nTomcat 설치 + ViewResolver 등록"]
    NEW["Spring Boot"] --> N["@SpringBootApplication 자동설정\napplication.yml + 내장Tomcat\nstarter 의존성으로 View 자동설정"]
```
