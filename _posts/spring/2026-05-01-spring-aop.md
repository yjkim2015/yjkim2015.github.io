...
```mermaid
graph LR
    C["클라이언트"] -->|"createOrder(dto)"| P["Proxy Object..|"Before Advice"| P
    P -->|"joinPoint.proce..| R["Real OrderService"]
    R -->|"결과 반환"| P
    P -->|"After Advice"| P
    P -->|"최종 결과"| C
```
...
```mermaid
graph LR
    A["AOP 적용 대상 Bean"] --> B{"proxy-target-cla.."}
    B -->|"YES"| E["CGLIB Proxy ..|"NO"| C{"IF가 있는가?"}
 ..|"YES"| D["JDK Dynamic ..|"NO"| E
```
...
graph LR
    ...
    R -->|"this.sendNotifi..| N["실제 OrderService.se"]
    style N fill:#ffe0e0
    style R fill:#fff8e0
```
