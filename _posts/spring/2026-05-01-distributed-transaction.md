...
```mermaid
graph LR
    A["1️⃣ 주문 생성"] -->|"성공"| B["2️⃣ 재고 차감"] ..|"성공"| C["3️⃣ 결제 "]
  ..|"결제 실패"| D["보상: 재고 복원"]
    D --> E["보상: 주문 취소"]
...
graph LR
    ...
    T2 -->|원자적 커밋| NOTE["로컬 TX 완료"..|미발행 이벤트| P["outbox 폴링/CDC"]
    P --> PUB["Kafka 발행"]
...
```mermaid
graph LR
    Q1{"강한 일관성?"} -->|YES| R1["단일 TX"]
   ..|NO| SAGA["Saga 패턴"]..|흐름 단순| CHOREO["Choreog..|흐름 복잡| ORCH["Orchestra..|원자적 발행| OUTBOX["Outbox Pattern"]
```
