...
graph LR
    ...
    GW --> APP["3️⃣ Spring Applica"]
    APP --> DB["4️⃣ DB / External"]
```
...
```mermaid
graph LR
    W1["윈도우1 마지막(59s)"] -->|"100건 호출"| EDGE["경계(60s) 2초 내"]
    W2["윈도우2 시작(61s)"] -->|"100건 호출"| EDGE
...
```mermaid
graph LR
    PW["이전 윈도우(0:59~1:00)"] -->|"카운터"| CALC["예상 요청수 계산"]
    CW["현재 윈도우(1:00~1:01)"] -->|"카운터"| CALC
    NOW["현재시각 1:00:45"] -->|"경과 비율"| CALC
    CALC --> RESULT["가중평균 합산"]
```
