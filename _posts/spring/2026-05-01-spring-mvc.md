...
```mermaid
graph LR
    Client["클라이언트"] -->|"HTTP 요청"| DS["DispatcherS..|"위임"| OC["OrderContro..|"위임"| UC["UserControl..|"위임"| PC["ProductController"]
```
...
graph LR
    ...
    DS -->|"컨트롤러 실행"| CM["Controller"..|"ModelAndView"| DS
    DS -->|"postHandle()/Vi..| DS
...
graph LR
    ...
    B -->|"처리 성공"| Z["종료"]
    B -->|"처리 못한 경우"| C["2️⃣ Response..|"처리 성공"| Z
    C -->|"처리 못한 경우"| D["3️⃣ DefaultH..|"처리 성공"| Z
