...
```mermaid
graph LR
    TX["TX"] -->|"커서이동"| CR["Cursor Reader"]
    TX -->|"process/write"| PW["Processor/Writer"]
    TX -->|"COMMIT"| TX
    TX2["TX2"] -->|"SELECT"| CR2["Paging Reader"]
    TX2 -->|"process/write"| PW2["Processor/Writer"]
```
...
### Q. 청크 처리에서 pageSize와 chunkSize를 반드시 같게 설정해야 하는 이유는?

`JpaPagingItemReader`의 `pageSize=100`, `chunkSize=1000`으로 설정하면 하나의 청크를 완성하기 위해 DB에 10번 페이징 쿼리를 실행한다. 10번의 쿼리 사이에 다른 트랜잭션이 데이터를 변경하면 페이지 오프셋이 밀려 같은 행이 두 번 처리되거나 누락될 수 있다. 마치 책 10페이지씩 읽는데 누군가 중간 페이지를 계속 추가하는 것과 같다. `pageSize = chunkSize`로 설정하면 하나의 청크가 정확히 하나의 페이징 쿼리에 대응되어 이 문제가 없다. `JdbcPagingItemReader`도 동일한 원칙이 적용된다.

### Q. Skip과 Retry의 차이와 함께 사용하는 방법은?

`Retry`는 일시적 오류(네트워크 타임아웃, 데드락)가 발생했을 때 같은 항목을 재처리한다. 재처리 후 성공하면 청크가 정상 완료된다. `Skip`은 특정 항목 처리가 반복 실패할 때 해당 항목만 건너뛰고 나머지를 계속 처리한다. 두 가지를 함께 쓰면 "N번 재시도 후에도 실패하면 Skip"하는 전략이 된다. Skip된 항목은 `SkipListener`로 로그에 기록해 나중에 수동 처리하거나 별도 배치로 재처리할 수 있다. 중요 데이터(결제, 정산)는 Skip 대신 Job을 실패시키고 알람을 보내야 한다.

### Q. ItemProcessor를 null 반환하는 패턴은 언제 사용하는가?

`ItemProcessor.process()`에서 `null`을 반환하면 해당 항목이 `ItemWriter`로 전달되지 않고 건너뛰어진다. Skip 카운트에도 포함되지 않는다. 이 패턴은 필터링 목적으로 사용한다. 예를 들어 100만 명 중 특정 조건(마지막 로그인 180일 이상)에 맞는 사용자만 처리하고 싶을 때, DB에서 전체를 읽고 Processor에서 null 반환으로 필터링한다. DB 쿼리에 조건을 추가하는 것과 달리, 복잡한 비즈니스 로직(다른 테이블 조회, 캐시 참조)을 Java 코드로 처리할 수 있는 장점이 있다.

### Q. ItemWriter에서 트랜잭션이 청크 단위로 관리되는 메커니즘은?

Spring Batch의 `ChunkOrientedTasklet`이 하나의 청크 전체를 단일 트랜잭션으로 감싼다. `ItemReader.read()`가 chunkSize만큼 호출되고, `ItemProcessor.process()`가 각 항목을 변환한 후, `ItemWriter.write(List<T>)`가 한 번에 호출된다. `write()` 완료 후 트랜잭션이 커밋되고 `JobRepository`의 `StepExecution`에 진행 상황이 저장된다. `ItemWriter`에서 예외가 발생하면 청크 전체가 롤백된다. `JpaItemWriter`는 `EntityManager.merge()`를 사용하므로 배치에서는 `JdbcBatchItemWriter`가 더 효율적이다. JDBC 배치 인서트(`addBatch()` + `executeBatch()`)로 청크 내 N건을 한 번의 네트워크 왕복으로 처리한다.

### Q. FaultTolerant 청크에서 Skip 발생 시 재처리 방식은?

청크 처리 중 Skip 가능한 예외가 발생하면 Spring Batch는 해당 청크를 롤백하고 단건 처리(single-item chunk) 모드로 재실행한다. 청크 내 각 항목을 하나씩 처리하면서 예외가 발생한 항목만 Skip하고 나머지는 정상 처리한다. 이 방식 덕분에 1000건 청크 중 1건이 실패해도 999건은 성공적으로 처리된다. 단, 재처리 시 `ItemReader`가 해당 청크를 다시 읽어야 하므로, `ItemReader`의 `read()` 호출이 멱등적(동일 결과 보장)이어야 한다. `saveState=false` 설정 시 재시작이 불가하므로 상태 저장이 필요한 배치에는 기본값(true)을 유지해야 한다.
