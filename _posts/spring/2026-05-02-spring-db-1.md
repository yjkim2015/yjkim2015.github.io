...
```mermaid
graph LR
    App -->|getConnection| DM["DriverManag..|TCP+인증| DB
    DB -->|Connection| App
```
...
```mermaid
graph LR
    App["App"] -->|"getConnection()"| Pool["HikariCP"..|"신규 생성(필요시)"| DB["DB"]
    Po..|"커넥션 반환"| App
```
...
```mermaid
graph LR
    S["Service"] -->|"getTransaction()"| TM["TxManager"]..|"Connection 저장(ThreadLocal)"| S
    S -->|"findById/update"| R["Repository"]..|"commit()"| TM
```
