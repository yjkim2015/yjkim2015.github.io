...
```mermaid
graph LR
    RE["RuntimeException"] --> DAE["DataAccessExc"]
    DAE --> NTE["NonTransientExc"]
...
```mermaid
graph LR
    PTM["PlatformTxMgr"] --> DSTM["DataSourceTxMgr"]
...
HikariCP는 커넥션 풀 내부 자료구조로 `ConcurrentBag`을 사용한다. 일반 큐(Queue)와 달리 스레드가 이전에 사용한 커넥션을 우선적으로 재사용하도록 설계되어 CPU 캐시 지역성이 높아진다. 커넥션 획득 시 락 경합을 최소화하기 위해 `AtomicInteger` 기반의 낙관적 잠금을 사용한다. 리플렉션 없이 직접 `PreparedStatement` 프록시를 바이트코드 수준에서 생성한다. `maximumPoolSize` 설정이 잘못되면(너무 크거나 작으면) 오히려 성능이 떨어지므로, `connections = ((core_count * 2) + effective_spindle_count)` 공식이 기본 출발점이 된다.

### Q. DataSource 추상화가 필요한 이유는?

`DriverManager.getConnection()`은 매번 새 물리 커넥션을 생성한다. DB 커넥션 생성은 TCP 연결 + 인증 + 세션 초기화로 수십~수백 ms가 소요된다. 초당 100 요청이면 100번 커넥션을 새로 만들어야 한다. `DataSource`는 커넥션 풀을 추상화해 미리 만들어 둔 커넥션을 재사용한다. 마치 택시를 매번 제조하는 대신 택시 회사(HikariCP)에서 대기 중인 택시를 빌리는 것과 같다. 또한 `DataSource`는 인터페이스이므로 실제 구현체(HikariCP, DBCP2, Embedded)를 교체해도 코드 변경이 없다.

### Q. JdbcTemplate이 checked SQLException을 unchecked로 변환하는 이유는?

JDBC의 `SQLException`은 checked 예외라 모든 DB 코드에 `try-catch` 또는 `throws SQLException`이 필요하다. 하지만 대부분의 SQL 오류(잘못된 쿼리, 제약 조건 위반)는 호출자가 복구할 수 없는 상황이다. JdbcTemplate은 `SQLException`을 Spring의 `DataAccessException` 계층(unchecked)으로 변환한다. `DuplicateKeyException`, `DataIntegrityViolationException` 등 의미 있는 하위 타입으로 변환되어 필요한 경우만 선택적으로 잡을 수 있다. 동시에 JPA, MyBatis 등 다른 데이터 접근 기술과 동일한 예외 계층을 공유해 상위 레이어가 기술에 독립적으로 예외를 처리할 수 있다.

### Q. TransactionSynchronizationManager가 커넥션을 스레드에 바인딩하는 방식은?

`@Transactional` 메서드 진입 시 Spring은 `DataSource`에서 커넥션을 획득하고 `ThreadLocal<Map<DataSource, ConnectionHolder>>`에 바인딩한다. 같은 스레드에서 JdbcTemplate이 SQL을 실행할 때 `DataSourceUtils.getConnection()`이 `ThreadLocal`을 조회해 이미 바인딩된 커넥션을 반환한다. 따라서 하나의 트랜잭션 내의 모든 SQL이 같은 커넥션(같은 트랜잭션)을 사용한다. 트랜잭션 종료 시 `ThreadLocal`에서 커넥션 바인딩을 제거하고 HikariCP에 반환한다. `@Async`로 다른 스레드에서 실행하면 `ThreadLocal`이 다르므로 트랜잭션이 전파되지 않는다.

### Q. JdbcTemplate의 RowMapper와 ResultSetExtractor의 차이는?

`RowMapper<T>`는 ResultSet의 각 행을 T 객체로 변환하는 콜백이다. JdbcTemplate이 반복문을 관리하고, `RowMapper`는 단일 행 변환만 담당한다. `queryForList()`, `query()`에 사용한다. `ResultSetExtractor<T>`는 ResultSet 전체를 직접 순회해 결과를 만드는 콜백이다. 계층형 데이터(1:N 관계)를 하나의 쿼리로 조회해 중첩 객체 구조로 매핑하거나, 첫 번째 행만 조회 후 나머지를 무시하는 등 복잡한 로직이 필요할 때 사용한다. `BeanPropertyRowMapper`는 컬럼명과 필드명을 자동 매핑하는 `RowMapper` 구현체로, 프로토타입 개발에 유용하지만 리플렉션을 사용하므로 대용량 조회에는 직접 구현한 `RowMapper`가 더 빠르다.
