...
graph TD
    subgraph "ACID — 트랜잭션의 4가지 보장"
        A["⚛️ Atomicity"] --> A1["모두 성공 or 모두 실패"]
        B["✅ Consistency"] --> B1["트랜잭션 전후 DB 무결성 유지\"]
        C["🔒 Isolation"] --> C1["동시 트랜잭션 간 간섭 없음"]
        D["💾 Durability"] --> D1["커밋된 데이터는 영구 저장"]
    end
```
...
```mermaid
graph TD
    R1["외부TX 있음"] -->|REQUIRED 참여| R2["같은 TX"]
    R3["외부TX 없음"] -->|REQUIRED 생성| R4["새 TX"]
    N1["외부TX 있음"] -->|REQUIRES_NEW 중단| N2["별도 새 TX"]
    N2 -->|완료 후| N3["외부TX 재개"]
    NE1["외부TX 있음"] -->|NESTED Savepoint| NE2["중첩TX"]
    NE2 -->|내부 실패| NE3["Savepoint 롤백"]
    NE2 -->|내부 성공| NE4["외부커밋 시 함께"]
```
...
graph TD
    subgraph "Spring 예외 계층 — DB 벤더 독립적"
        A["RuntimeException"] --> B["DataAccessExceptio"]
        B --> C["NonTransientDataAc"]
        B --> D["TransientDataAcces"]
        C --> E["DataIntegrityViola"]
        C --> F["BadSqlGrammarExcep"]
        C --> G["InvalidDataAccessA"]
        D --> H["QueryTimeoutExcept"]
        D --> I["ConcurrencyFailure"]
        E --> J["DuplicateKeyExcept"]
    end
```
