...
```mermaid
sequenceDiagram
    HikariCP_10개_준비->>요청2: 수μs 대여
    요청1->>HikariCP_10개_준비: 반납
    요청2->>HikariCP_10개_준비: 반납
```
...
```mermaid
sequenceDiagram
    TxManager->>Service: Connection 저장_ThreadLocal
    Service->>Repository: findById/update
    Service->>TxManager: commit_
```
...
```mermaid
sequenceDiagram
    Proxy->>Proxy: getTransaction_
    Proxy->>Service: transfer_
    Service->>Proxy: 정상→commit_
    Service->>Proxy: 예외→rollback_
```
