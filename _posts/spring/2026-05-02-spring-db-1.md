...
```mermaid
graph LR
    POOL["HikariCP(10개)"] -->|"수μs 대여"| R2["요청2"]
    R1["요청1"] -->|"반납"| POOL
    R2 -->|"반납"| POOL
```
...
```mermaid
graph LR
    TXM["TxManager"] -->|"Connection→ThreadLocal"| SVC["Service"]
    SVC -->|"findById/update"| REPO["Repository"]
    SVC -->|"commit"| TXM
```
...
```mermaid
graph LR
    PROXY["Proxy"] -->|"getTransaction"| SVC["Service.transfer"]
    SVC -->|"정상 완료"| COMMIT["commit"]
    SVC -->|"예외 발생"| ROLLBACK["rollback"]
```
