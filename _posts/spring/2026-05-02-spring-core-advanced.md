...
```mermaid
graph LR
    CLI["클라이언트"] --> PROXY["Proxy"]
    PROXY -->|"위임"| REAL["RealSubject"]
    PROXY -.->|"구현"| SUBJ["Subject 인터페이스"]
    REAL -.->|"구현"| SUBJ
```
...
```mermaid
graph LR
    C["Client"] -->|"operation"| T["TimeDecorator"]
    T -->|"operation"| R["RealSubject"]
    R -->|"결과"| T
    T -->|"결과+시간로그"| C
```
...
```mermaid
graph LR
    C["Container"] -->|"new+의존주입"| O["원본 빈"]
    C -->|"postProcessAfter"| BPP["BeanPostProcessor"]
    BPP -->|"프록시 빈 등록"| C
```
...
```mermaid
graph LR
    A["빈 생성 완료"] --> B["AutoProxyCreator"]
    B --> C{"Advisor 매칭?"}
    C -->|"매칭"| D["ProxyFactory→프록시"]
    C -->|"미매칭"| E["원본 빈 등록"]
    D --> F["컨테이너에 등록"]
    E --> F
```
...
```mermaid
graph LR
    C["Client"] -->|"external 호출"| P["CallService Proxy"]
    P -->|"external 위임"| T["CallService(원본)"]
    T -->|"this.internal 직접호출"| T
    T --> WARN["프록시 우회!\n@Transactional 무시"]
```
...
```mermaid
graph LR
    BEAN["빈 생성+의존주입"] --> BPP["postProcessAfter"]
    BPP --> MATCH{"Advisor 매칭?"}
    MATCH -->|"있음"| PF["ProxyFactory→JDK/CGLIB"]
    MATCH -->|"없음"| ORIG["원본 빈 등록"]
    PF --> CTX["ApplicationContext"]
    ORIG --> CTX
    CTX --> CLI["클라이언트 호출"]
```
