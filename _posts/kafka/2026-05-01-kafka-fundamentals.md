...
graph LR
    ...
    C2["Controller2(Standby)"] --> C1["Controller1(Active)"]
    C2 --> C3["Controller3(Standby)"]
    C3["Controller3(Standby)"] --> C2
    C1 --> C3
    C3 --> C1
...
| **처리량** | 초당 수백만 | 초당 수만~수십만 | 관리형, 자동 확장 |
| **재처리** | 오프셋으로 임의 재처리 | 소비 후 재처리 어려움 | DLQ로 제한적 |
| **적합한 용도** | 이벤트 스트리밍, 감사 로그, CDC | RPC 스타일 작업 큐, 복잡한 라우팅 | 서버리스, 관리형 큐 |

**실무 판단**: 메시지를 여러 시스템이 독립적으로 소비하거나 재처리가 필요하면 Kafka. 단순 작업 큐, 복잡한 라우팅(topic exchange, fanout)은 RabbitMQ. AWS 네이티브 서버리스 환경은 SQS.
...
① Sequential Disk I/O: 메시지를 파일에 순차 append — HDD도 SSD에 준하는 순차 쓰기 속도 ② Zero-Copy: `sendfile()` 시스템 콜로 커널에서 네트워크로 직접 전송(사용자 공간 복사 없음) ③ 배치 압축: 프로듀서가 메시지를 묶어 gzip/snappy로 압축 전송 ④ 페이지 캐시: OS 파일 시스템 캐시를 최대한 활용.

**Q2. 토픽, 파티션, 오프셋의 관계는?**
토픽은 메시지의 논리적 채널이다. 파티션은 토픽을 물리적으로 분할한 단위로 병렬 처리를 가능하게 한다. 오프셋은 파티션 내 메시지의 단조 증가 번호다. Consumer는 파티션-오프셋 쌍으로 자신의 읽기 위치를 추적한다. 오프셋은 파티션 내에서만 의미가 있고 파티션 간에는 독립적이다.

**Q3. Kafka의 메시지 전달 보장 수준은?**
`acks=0`: 브로커 확인 없음(최고 처리량, 유실 가능). `acks=1`: 리더만 확인(기본, 리더 장애 시 유실 가능). `acks=all(-1)`: ISR 전체 확인(가장 강한 보장). `enable.idempotence=true`와 조합하면 exactly-once 쓰기를 달성한다.

**Q4. Consumer Group의 파티션 할당 전략은?**
RangeAssignor(기본): 파티션을 범위로 할당, 토픽 간 불균형 가능. RoundRobinAssignor: 라운드 로빈으로 균등 분배. StickyAssignor: 리밸런싱 시 기존 할당 최대한 유지. CooperativeStickyAssignor: 점진적 리밸런싱(Eager 방식보다 다운타임 최소화). 운영에서는 CooperativeStickyAssignor 권장.

**Q5. Kafka와 이벤트 소싱의 관계는?**
이벤트 소싱은 상태 변경을 이벤트 로그로 저장하는 패턴이다. Kafka의 영속적 로그가 이벤트 소싱의 이벤트 스토어 역할을 한다. Log Compaction으로 최신 상태 스냅샷을 유지하고, 오프셋 재처리로 임의 시점의 상태를 재구성할 수 있다. Kafka Streams의 KTable이 이 패턴을 추상화한다.
