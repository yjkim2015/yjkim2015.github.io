...
```mermaid
graph LR
    M0["P0:오프셋0"] --> M1["오프셋1"] --> M2["오프셋2"] --> M3["오프셋3"]
    N0["P1:오프셋0"] --> N1["오프셋1"] --> N2["오프셋2"]
    O0["P2:오프셋0"] --> O1["오프셋1"]
    Consumer["컨슈머"]
```
...
```mermaid
graph LR
    P["Producer"] -->|"쓰기"| L["리더(Broker1)"]
    L -->|"복제"| F1["팔로워(Broker2)"]
    L -.->|"복제 지연"| F2["팔로워(Broker3)"]
    F2 -->|"ISR 이탈"| OOS["Out-of-Sync"]
    ISR["ISR"] --> L & F1
```
...
graph LR
    ...
    Producer -->|"2️⃣ Avro/Protobuf"| Kafka[Kafka]
    Kafka -->|"3️⃣ Avro/Protobuf"| Consumer
...
```mermaid
graph LR
    Producer --> P0_L["P0 Leader(B1)"]
    Producer --> P1_L["P1 Leader(B2)"]
    P0_L -. 복제 .-> P0_F1["P0 Follower(B2)"]
    P0_L -. 복제 .-> P0_F2["P0 Follower(B3)"]
    CG["Consumer Group"] --> P0_L
    CG --> P1_L
    CG --> P2_L["P2 Leader(B3)"]
```
...
graph LR
    ...
    Fail --> Detect["3️⃣ Zookeeper/KRaf"]
