...
## 왜 Kafka Producer인가? — 대안 vs 트레이드오프

| 선택지 | 장점 | 단점 | 적합한 상황 |
|--------|------|------|-----------|
| Kafka Producer | 높은 처리량, 파티션 병렬성, 내구성 보장 | 설정 복잡도 높음 | 대용량 이벤트 스트리밍 |
| RabbitMQ Publisher | 라우팅 유연, 메시지 우선순위 | 처리량 한계 (수만 TPS) | 복잡한 라우팅이 필요한 경우 |
| AWS SQS SendMessage | 완전 관리형, 설정 거의 불필요 | 순서 보장 어려움, 처리량 제한 | 서버리스/소규모 |
| HTTP 직접 호출 | 구현 가장 단순 | 장애 시 메시지 유실, 결합도 높음 | 프로토타입 |
...
```mermaid
flowchart LR
    APP["Application"] --> SER["Serializer"] --> PART["Partitioner"] --> ACC["RecordAccumulator"]
    ACC -->|"batch.size/ling..| SEND["Sender Thread"]
    SEND --> BROKER["Kafka Broker"] -->|ACK| SEND
    SEND -->|"Future 완료"| APP
```
...
```mermaid
graph LR
    Q1{"중복 방지 범위"} -->|단일 파티션| S1["멱등성 Produce..|여러 파티션/토픽| S2["TX Producer..|Kafka 설정 무관| S3["앱 멱등 키"]
  ..|DB 트랜잭션 필요| S4["Outbox 패턴"]
```
