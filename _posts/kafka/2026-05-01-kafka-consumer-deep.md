...
graph TD
    P0[P0] & P1[P1] & P2[P2]
    CA[order-processor A] & CB[order-processor B] & CC[order-processor C]
    CD[analytics D] & CE[analytics E]
    P0 --> CA
    P1 --> CB
    P2 --> CC
    P0 --> CD
...
sequenceDiagram
    ...
    participant CA as ConsumerA/B
    participant CC as ConsumerC
    GC->>CA: 리밸런싱 시작(STW)
    CA->>GC: JoinGroup
    CC->>GC: JoinGroup
    GC->>CA: SyncGroup(새 할당)
    GC->>CC: SyncGroup(새 할당)
```
...
```mermaid
graph TD
    AC["Auto Commit: poll → 5초마다 자동커밋"]
    AC -->|"장애시"| ACR["마지막 커밋 이후 재처리 (중복 가능)"]
    MC["Manual Commit: poll → 처리"]
    MC --> MCR["ack.acknowledge() 후 정확한 오프셋 커밋"]
    COMMIT["오프셋 커밋 전략"]
    COMMIT -->|"단순"| AC
    COMMIT -->|"신뢰성"| MC
```
