...
@Transactional: or →(DB 커밋)→ Relay: outbox 폴링/C, Relay: outbox 폴링/C → Kafka Topic
...
MySQL / PostgreSQL →(binlog / WAL)→ Debezium Connector, Debezium Connector →(이벤트 발행)→ Kafka Topic
