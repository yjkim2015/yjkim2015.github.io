...
변경 후: user-123 → h → 동일 키가 다른 파티션 → 순서
...
Producer → Leader 단절 → acks=1: 쓰기 가능, Producer → Leader 단절 → acks=all: ISR 감소로 거부
...
participant P as Producer (멱등성 없음) participant B as Broker P->>B: msg(A) 전송
