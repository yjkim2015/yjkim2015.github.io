...
```mermaid
graph LR
    B1["Broker 1 (Leader)"] -->|복제| B2["Broker 2 (F..|복제| B3["Broker 3 (Follow.."]
```
...
```mermaid
graph LR
    S0["ISR=B1(Leader),B2,"] -->|"B3 lag 초과"| S1["ISR=B1,B2"]..|"B2 Full GC"| S2["ISR=B1만"]
 ..|"B1 장애"| S4["ISR 비어있음"]
..|"unclean=true"| UNCLEAN["B3 리더 ..|"unclean=false"| CLEAN["파티션 불가용"]
```
...
```mermaid
graph LR
    PA["Producer A"] -->|"쓰기 시도"| B1["Broker1(zom..|"Split Brain"| B2["Broker2(new..|"복구 후 메시지 폐기"| B1
```
...
```mermaid
graph LR
    P["Producer"] -->|"쓰기 시도"| B1["Broker1(epo..|"리더 확인"| CTL["Controller..|"epoch 5 구 리더"| B1
    B1 -->|"NotLeaderExcept..| P
    P -->|"새 리더 조회"| CTL
