...
Network Layer → KafkaApis, Request Handler Po → ReplicaManager, Request Handler Po → GroupCoordinator, Request Handler Po → LogManager
...
Controller →(Metadata 갱신)→ Producer/Consumer, Producer/Consumer →(재연결)→ Broker3 신규 리더
...
mindmap root(("Controller 역할")) "브로커 장애 감지"
