...
```mermaid
graph LR
    SVC["서비스"] --> LAT["지연시간 Latency"]
    SVC --> TRAF["트래픽 Traffic"]
    SVC --> ERR["오류율 Errors"]
    SVC --> SAT["포화도 Saturation"]
    LAT -->|"p99 > 임계값"| ALERT["알림 발생"]
    ERR -->|"5xx > 1%"| ALERT
    SAT -->|"CPU > 80%"| ALERT
```
...
## VisualVM — JVM 실시간 모니터링의 기본

> **비유**: VisualVM은 JVM의 건강검진 도구입니다. 혈압(CPU), 체온(메모리), 심박수(스레드)를 실시간으로 보여줍니다.
