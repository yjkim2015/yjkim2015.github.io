...
```mermaid
graph LR
    MEASURE["SLI 측정"] --> ANALYZE["기대 분석"]
    ANALYZE --> SET["SLO 설정"]
    SET --> MONITOR["Budget 모니터"]
    MONITOR --> REVIEW["분기 리뷰"]
    REVIEW --> MEASURE
```
...
```mermaid
graph LR
    START["Error Budget"] --> DEPLOY["배포"]
    DEPLOY --> DECIDE{"Budget 충분?"}
    DECIDE -->|"Yes"| CONTINUE["배포 가능"]
    DECIDE -->|"No"| FREEZE["기능 동결"]
```
...
```mermaid
graph LR
    SYM["증상 기반(권장)"] --> DASH["대시보드 탐색"]
    DASH --> ROOT["원인 파악"]
    CAUSE["원인 기반(비권장)"] --> C1["CPU/디스크/GC 알림"]
    C1 --> Q["사용자 영향? 불명확"]
```
...
```mermaid
graph LR
    P0["P0 다운"] -->|즉시 호출| PG["PagerDuty"]
    P1["P1 저하"] -->|15분| IC["#incidents"]
    P2["P2 위험"] -->|근무시간| AL["#alerts"]
    P3["P3 정보"] -->|일괄| AL
```
...
### Q. "4대 Golden Signals을 설명하고, 왜 이 4개인지 설명하시오."
...
### Q. "SLI, SLO, SLA의 차이를 실제 예시와 함께 설명하시오."
...
### Q. "Error Budget이란 무엇이며, 어떻게 운영하는가?"
