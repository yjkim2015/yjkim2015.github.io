...
graph LR
    subgraph "SLO 설정 프로세스"
        MEASURE["1️⃣ 현재 SLI 측정"] --> ANALYZE["2️⃣ 사용자 기대 분석"]
        ANALYZE --> SET["3️⃣ SLO 설정"]
        SET --> MONITOR["4️⃣ Error Budget 모"]
...
```mermaid
graph TD
    SYM["증상 기반(권장): P95 응답"] --> DASH["대시보드 원인 탐색"]
...
graph LR
    subgraph "알림 라우팅"
        P0["P0 — 서비스 다운"] -->|"즉시"| PAGER["PagerDuty"]
        P1["P1 — 성능 저하"] -->|"15분 내"| INCIDENT["Slack #incidents"]
        P2["P2 — 잠재 위험"] -->|"근무 시간"| ALERT["Slack #alerts"]
        P3["P3 — 정보성"] -->|"일괄"| INFO["일일 리포트"]
    end
```
