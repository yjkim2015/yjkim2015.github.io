...
sequenceDiagram
    ...
    A->>B: HTTP traceId=abc span=001
    B->>COL: Span 001
    A->>COL: Span 002
    COL->>COL: traceId=abc 조립
```
...
graph LR
    ...
    ING --> STORE[("ES/Cassandra")]
    QUERY["Jaeger Query(UI)"] --> STORE
```
...
```mermaid
graph LR
    APP["App + OTel"] -->|"OTLP"| DIST["Tempo Dis..|"블록 플러시"| S3[("Object Storage")]
...
graph LR
    ...
    DECIDE -->|"샘플링 O\n(flags=0..| TRACE_YES["전체 Span 기록"]
    DECIDE -->|"샘플링 X\n(flags=0..| TRACE_NO["Span 미기록"]
```
...
graph LR
    ...
    BUFFER --> EVAL{"조건 평가"}
    EVAL -->|에러/지연| KEEP["Save"]
  ..|정상| SAMPLE{"10% 샘플"..|미선택| DROP["폐기"]
```
