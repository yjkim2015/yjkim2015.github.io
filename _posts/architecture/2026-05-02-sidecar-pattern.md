...
```mermaid
graph LR
    ExternalTraffic["외부 트래픽"] --> Sidecar["Sidecar:15001"]
    Sidecar -->|"localhost"| App["App:8080"]
    App -->|"로그 쓰기"| Vol[("공유볼륨")]
    Sidecar -->|"로그 읽기"| Vol
    Pause["Pause Container"] -.->|"네트워크 공유"| App
    Pause -.->|"네트워크 공유"| Sidecar
```
...
participant CP as Istiod participant OE as Order Envoy participant UE as User Envoy
...
```mermaid
graph LR
    P1["관심사 분리"] --> Benefit1["복잡도 감소"]
    P2["언어 독립성"] --> Benefit2["다국어 지원"]
    P3["독립 배포"] --> Benefit3["독립 릴리즈"]
```
