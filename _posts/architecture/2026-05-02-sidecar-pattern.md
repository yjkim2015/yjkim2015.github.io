...
```mermaid
graph LR
    EXT["외부 트래픽"] --> SC["Sidecar:15001"]
    SC -->|"localhost"| APP["App:8080"]
    APP -->|"로그 쓰기"| VOL[("공유볼륨")]
    SC -->|"로그 읽기"| VOL
    PAU["Pause Container"] -.->|"네트워크 공유"| APP
```
...
```mermaid
graph LR
    App["App Container"] -->|"로그 기록"| Vol[("Log Volume")]
    FB["Fluent Bit Sidecar"] -->|"로그 읽기"| Vol
    FB -->|"전송"| ES["Elasticsearch"]
    ES --> Kibana["Kibana"]
```
...
```mermaid
graph LR
    C["Client"] -->|"JWT"| P["Proxy(Sidecar)"]
    P -->|"성공: X-User-ID"| A["App"]
    A --> C
    P -->|"실패: 401"| C
```
...
```mermaid
graph LR
    CP["Istiod"] -->|"xDS 정책"| OE["Order Envoy"]
    CP -->|"xDS 정책"| UE["User Envoy"]
    OE -->|"mTLS 암호화"| UE
    UE -->|"응답"| OE
```
...
graph LR
    ...
    Q1 -- "예" --> Q2{"다국어?"}
    Q2 -- "아니오" --> Q3{"인프라 기능?"}
    Q2 -- "예" --> UseSidecar["사이드카 적합"]
    Q3 -- "아니오" --> DaemonSet["DaemonSet"]
    Q3 -- "예" --> UseSidecar
...
### Q. 사이드카 패턴과 라이브러리 방식의 차이와 각각의 장단점은?

라이브러리 방식은 각 서비스 코드에 직접 SDK를 포함한다. 언어별로 다른 라이브러리를 유지해야 하고, 버전 업그레이드 시 모든 서비스를 재배포해야 한다. 사이드카는 인프라 기능을 별도 프로세스로 분리하므로 언어에 독립적이고, 사이드카만 업그레이드하면 메인 서비스 재배포 없이 전체에 적용된다. 단, 사이드카는 네트워크 홉이 추가되어 수 ms의 지연이 발생하고, Pod당 메모리와 CPU를 추가로 소비한다. 수백 개 서비스에 동일한 보안·관찰가능성 정책을 일관되게 적용해야 하는 규모에서는 사이드카가 더 효율적이다.

### Q. Envoy 사이드카가 mTLS를 투명하게 처리하는 원리는?

메인 서비스는 평문 HTTP로 localhost:8080에 요청을 보낸다. iptables 규칙이 이 트래픽을 Envoy 사이드카로 가로챈다. Envoy가 목적지 서비스의 Envoy와 TLS 핸드셰이크를 수행하고, 인증서를 교환해 상호 인증한다. 목적지 Envoy가 TLS를 해제하고 메인 서비스에 평문으로 전달한다. 양쪽 메인 서비스는 TLS를 전혀 모르고 평문 통신만 한다. 마치 두 사람이 모르는 사이에 통역사(Envoy)가 암호화 채널로 대화를 중계하는 것과 같다. 인증서 갱신도 Envoy가 자동으로 처리한다.

### Q. 사이드카가 메인 컨테이너보다 늦게 시작되면 어떤 문제가 생기는가?

메인 컨테이너가 시작 직후 외부 요청을 받거나 외부 서비스를 호출할 때, 사이드카(예: Envoy 프록시)가 아직 준비되지 않았으면 연결 실패가 발생한다. Istio 초기 버전에서 이 문제가 실제 운영 장애로 이어졌다. Kubernetes 1.29+의 Native Sidecar는 `initContainers`에 `restartPolicy: Always`를 설정해 사이드카를 일반 컨테이너보다 먼저 시작하고 메인 컨테이너 종료 후에 종료되도록 보장한다. 이전 버전에서는 메인 컨테이너의 `preStart` 훅에서 사이드카 준비 상태를 폴링하는 방식으로 우회했다.

### Q. 사이드카 패턴이 적합하지 않은 경우는?

단일 서비스(모놀리스)나 서비스 수가 적을 때는 사이드카 오버헤드가 이득을 초과한다. Pod당 추가 컨테이너 실행 비용, 운영팀의 사이드카 유지 역량, 네트워크 지연에 민감한 고성능 요구사항이 걸림돌이 된다. 특히 마이크로초 단위 지연이 중요한 금융 거래나 HFT(고빈도 매매) 시스템에서 사이드카 네트워크 홉은 허용되지 않는다. 언어 생태계가 단일(모두 Java)이고 라이브러리 버전 통제가 가능한 소규모 팀이라면 라이브러리 방식이 더 간단하고 빠르다.

### Q. 사이드카 컨테이너의 리소스 설정이 중요한 이유는?

사이드카에 `requests`/`limits`를 설정하지 않으면 Kubernetes 스케줄러가 Pod 배치 시 사이드카 리소스를 고려하지 않는다. 실제 사이드카(Envoy)가 CPU 버스트를 일으키면 같은 노드의 다른 Pod까지 성능 저하가 발생한다. `limits`를 너무 낮게 설정하면 트래픽 급증 시 Envoy가 CPU throttling으로 지연이 급증한다. Fluent Bit 로그 수집기는 평소 10m CPU로 충분하지만 로그 버스트 시 200m까지 필요할 수 있다. 부하 테스트를 통해 P99 트래픽에서의 사이드카 리소스 사용량을 측정하고 `requests`를 평균값, `limits`를 P99+여유분으로 설정하는 것이 원칙이다.
