...
```mermaid
sequenceDiagram
    Closed(요청_통과)->>Open(즉시_실패/Fallback): 연속 실패 5회
    Open(즉시_실패/Fallback)->>HalfOpen(제한_허용): 30초 대기
    HalfOpen(제한_허용)->>Closed(요청_통과): 테스트 성공
    HalfOpen(제한_허용)->>Open(즉시_실패/Fallback): 테스트 실패
```
