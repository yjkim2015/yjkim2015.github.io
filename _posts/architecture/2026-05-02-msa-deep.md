...
```mermaid
graph LR
    R_Call["REST: HTTP/JSON"] -->|"텍스트 파싱"| R_Response["Res..|"바이너리"| G_Response["응답(빠름)"]
```
...
graph LR
    ...
    Monitor -->|"정상"| Rollout["4️⃣ 점진..|"문제 감지"| Rollback["4️⃣ 즉시 롤백"]
```
...
graph LR
    Q1{"1️⃣ 팀 규모 > 20명?"}
    Q1 -->|No| Mono["모놀리스 유지"]..|Yes| Q2{"2️⃣ 독립 배포 필..|No| Mono
    Q2 -->|Yes| Q3{"3️⃣ Svc별 확장..|No| Mono
