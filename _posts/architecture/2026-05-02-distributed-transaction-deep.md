...
```mermaid
graph TD
    User["사용자"] -->|즉시 응답| Order["주문 PENDING + Outbox"]
    Order --> K1["Kafka: order-created"]
    K1 --> Pay{"결제 Saga"}
    Pay -->|부족| C1["주문 취소 보상"]
    Pay -->|충분| K2["Kafka: payment-completed"]
    K2 --> Inv{"재고 Saga"}
    Inv -->|부족| C2["결제 취소 보상"]
    Inv -->|있음| Delivery["배송 Saga"]
    Delivery --> Done["주문 완료 + 알림"]
```
...
graph TD
    ...
    Orches -->|"감사 로그 필요"| ES["+ Event Sourcing / CQRS"]
```
