...
```
팀 5명 이하 → MSA 하지 마라. 모놀리스가 답이다.

모놀리스를 먼저 만들고 다음 조건이 충족될 때 분리하라:
□ 특정 모듈 배포가 전체 서비스를 반복적으로 방해하는가?
□ 팀이 50명 이상으로 코드 충돌이 일상적인가?
□ 특정 기능의 스케일 요구가 나머지와 극단적으로 다른가?
□ DevOps/SRE 팀이 K8s, CI/CD, 분산 추적을 운영할 역량이 있는가?
