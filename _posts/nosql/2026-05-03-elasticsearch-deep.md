...
```mermaid
graph LR
    DOC["문서 도착"] --> TL["Translog WAL"]
    DOC --> BUF["In-Memory Buffer"]
    BUF -->|"refresh 1초"| SEG["Segment"]
    SEG -->|"merge"| MSEG["합병 Segment"]
```
...
```mermaid
graph LR
    INPUT["원본 텍스트"] --> CF["Character Filter"]
    CF --> TK["Tokenizer"]
    TK --> TF["Token Filter"]
    TF --> TERMS["토큰"]
```
...
```mermaid
graph LR
    Q1["데이터 크기?"] -->|"30GB 미만"| S1["1 Primary+Replica"]
    Q1 -->|"30~150GB"| S3["3~5 Primary+Replica"]
    Q1 -->|"150GB~1TB"| S10["5~20 Primary"]
    Q1 -->|"1TB 초과"| ILM["ILM 롤오버"]
```
