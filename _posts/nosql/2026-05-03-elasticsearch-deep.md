...
```mermaid
flowchart LR
    D1["Doc1: 무선 블루투스 이어폰"] --> T1["무선→D1,D3"] & T2["이어폰→D1,D2"] & T3["블루투스→D1"]
    D2["Doc2: 유선 이어폰 마이크"] --> T2 & T4["유선/마이크→D2"]
    D3["Doc3: 무선 키보드 마우스"] --> T1 & T5["키보드/마우스→D3"]
```
...
```mermaid
flowchart LR
    W["문서 도착"] --> BUF["In-Memory Buffer"]
    W --> TL["Translog(WAL)"]
    BUF -->|"refresh(1초)"| SEG1["Segment1"]
    BUF --> SEG2["Segment2"] & SEG3["Segment3"]
    SEG1 & SEG2 & SEG3 -->|"merge"| MERGED["합병 Segment"]
```
...
```mermaid
flowchart TB
    P0["Node1: P0"] -. 복제 .-> R0["Node2: R0"]
    P1["Node2: P1"] -. 복제 .-> R1["Node1: R1"]
    P2["Node3: P2"] -. 복제 .-> R2["Node3: R2"]
```
...
sequenceDiagram
    ...
    participant Coord as CoordNode
    participant S as Shard0,1,2
    C->>Coord: 검색 요청
    Coord->>S: 쿼리 전송(병렬)
    S-->>Coord: 상위 N개 결과
    Coord-->>C: 병합·정렬 후 최종 결과
```
