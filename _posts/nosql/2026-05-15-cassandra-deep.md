...
**비유**: 일기장에 글을 쓰는 것과 같다. 일기장(CommitLog)에는 받은 순서대로 끝에 이어서 쓴다(순차 쓰기). 수정이 필요해도 원래 자리를 지우고 다시 쓰지 않고(랜덤 쓰기 없음), 새 페이지에 최신 버전을 덧쓴다. 나중에 정리(Compaction) 시간에 오래된 내용을 일괄 정리한다. **순차 쓰기는 HDD에서도 초당 수백 MB를 낼 수 있지만, 랜덤 쓰기는 수 MB에 그친다.**
...
**비유**: 메모지 정리와 같다. 업무 중에는 포스트잇(SSTable)에 메모를 계속 붙인다. 같은 항목이 여러 포스트잇에 중복 기록되기도 한다. 퇴근 후(Compaction 시간)에 모든 포스트잇을 펼쳐서 항목별로 최신 내용만 남기고 나머지는 버린다. **쓰기 중에는 정리하지 않기 때문에 쓰기 속도가 빠르고, 정리 작업은 백그라운드에서 조용히 진행**된다.
...
**비유**: RDB 설계는 "창고를 먼저 만들고, 나중에 필요한 물건을 꺼내는 방법을 고민"한다. Cassandra 설계는 "어떤 물건을 어떻게 꺼낼지 먼저 정하고, 그에 맞게 선반을 배치"한다. 같은 물건을 여러 방식으로 꺼내야 한다면 같은 물건을 여러 선반에 중복 배치하는 것을 두려워하지 않는다. **저장 공간(디스크)은 싸고, 느린 조회(CPU·네트워크)는 비싸다**는 철학이다.
...
### Q. Cassandra가 마스터리스(masterless) 구조를 가질 수 있는 이유는 무엇인가?
...
### Q. CL=QUORUM 쓰기 + QUORUM 읽기가 강한 일관성을 보장하는 원리를 설명해보라.
...
### Q. VNode가 없는 구성과 비교했을 때 VNode의 장점은?
...
### Q. Cassandra에서 ALLOW FILTERING을 피해야 하는 이유는?
