...
Key-Value → Redis → Document → MongoDB → Column-Family → Cassandra
...
```mermaid
graph LR
    PK["user_id=123"] --> C1["05-01 login"]
    PK --> C2["05-02 purchase"]
    PK --> C3["05-03 logout"]
    PK2["user_id=456"] --> C4["05-01 signup"]
    PK2 --> C5["05-03 login"]
```
...
## 왜 NoSQL인가? (vs RDBMS — 언제 선택하는가)

NoSQL은 RDBMS의 대체가 아니라 특정 문제에 최적화된 도구다. "NoSQL이 빠르다"는 일반화는 틀렸다. NoSQL은 RDBMS가 희생한 것(유연한 스키마, 수평 확장, 특화된 데이터 모델)을 얻는 대신 ACID 트랜잭션, 복잡한 JOIN, 표준 SQL을 포기한다.

| 선택 기준 | RDBMS | NoSQL |
|-----------|-------|-------|
| 데이터 구조 | 정형, 관계 명확 | 비정형, 가변 스키마 |
| 트랜잭션 | 복잡한 다중 테이블 ACID | 단순하거나 최종 일관성 허용 |
| 확장 방식 | 수직 확장 + 읽기 레플리카 | 수평 확장(샤딩) |
| 접근 패턴 | 다양한 쿼리, 임시 분석 | 예측 가능한 패턴, 고처리량 |

**폴리글랏 퍼시스턴스**: 실무에서는 하나의 시스템에 여러 DB를 조합한다. 주문·결제는 PostgreSQL, 세션·캐시는 Redis, 전문 검색은 Elasticsearch, 사용자 피드는 Cassandra — 각 문제에 최적 도구를 선택한다.
