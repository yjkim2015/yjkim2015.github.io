...
```mermaid
graph LR
    A["private"] -->|"좁음"| B["package-private"]
    B -->|"넓음"| C["protected"]
    C -->|"최넓음"| D["public"]
    A -->|"권장"| E["캡슐화"]
    D -->|"주의"| F["API 노출"]
...
상속은 캡슐화를 깨뜨립니다. 상위 클래스의 내부 구현이 변경되면 하위 클래스가 오동작합니다 (`InstrumentedHashSet` 사례). 컴포지션은 기존 클래스를 필드로 참조하므로 내부 구현에 독립적입니다.

마치 부품을 납땜(상속)하는 것과 나사로 조립(컴포지션)하는 차이입니다. 납땜은 교체가 어렵지만, 나사 조립은 필요한 부품만 바꿀 수 있습니다.

### Q. 불변 클래스의 장단점은?

**장점** — 스레드 안전, 자유로운 공유, 실패 원자성. **단점** — 값이 다르면 반드시 새 객체 생성 (다단계 연산 시 성능 저하). 가변 동반 클래스(`StringBuilder`)로 보완합니다.

### Q. 인터페이스와 추상 클래스의 선택 기준은?

**인터페이스** — 다중 구현, 믹스인, 타입 정의. **추상 클래스** — 상태(필드)나 `protected` 메서드가 필요할 때. 보통 인터페이스 + 골격 구현(AbstractXxx) 조합이 최선입니다.

### Q. `public static final Thing[] VALUES`는 왜 위험한가?

배열의 참조는 불변이지만, **내용은 변경 가능**합니다. 외부에서 `VALUES[0] = null`로 배열을 오염시킬 수 있습니다.

### Q. 비정적 멤버 클래스가 메모리 누수를 일으키는 원리는?

비정적 멤버 클래스는 바깥 인스턴스에 대한 **숨은 참조**를 자동으로 가집니다. 이 내부 객체가 바깥 객체보다 오래 살면, 바깥 객체가 GC되지 못합니다.

마치 자식이 부모 손을 꽉 잡고 놓지 않으면 부모도 그 자리를 떠나지 못하는 것처럼, 내부 클래스가 바깥 객체를 붙잡고 있으면 GC도 수거할 수 없습니다.
...
**클래스와 인터페이스 설계 원칙의 핵심은 변경에 강한 코드를 만들기 위해서다.**

인터페이스를 통한 프로그래밍은 구현체를 교체해도 호출자 코드를 수정할 필요가 없게 한다. 접근 제한자를 최소화하면 외부에서 접근하는 범위가 줄어 내부 구현을 자유롭게 바꿀 수 있다.

상속보다 컴포지션을 선호하는 이유는 상속이 상위 클래스의 변경에 하위 클래스를 취약하게 만들기 때문이다. 상위 클래스의 메서드 구현이 바뀌면 하위 클래스의 오버라이딩 로직이 오작동할 수 있다. 컴포지션은 래핑만 하므로 이 문제가 없다.

추상 클래스 대신 인터페이스를 선호하면 Java의 단일 상속 제약을 피하고 다중 타입을 구현할 수 있다. 인터페이스에 default 메서드를 추가하면 기존 구현체를 깨지 않고 기능을 확장할 수 있다.
