...
상속하려면 `public`/`protected` 생성자가 필요하므로, 정적 팩터리만 제공하면 하위 클래스를 만들 수 없습니다. 또한 API 문서에서 생성자처럼 명확히 드러나지 않아 찾기 어렵습니다.

### Q. 빌더 패턴과 자바빈즈 패턴의 차이는?

자바빈즈는 setter로 값을 설정하므로 **객체가 일관성 없는 중간 상태**에 놓일 수 있고, 불변으로 만들 수 없습니다. 빌더는 `build()` 시점에 완전한 객체를 만들므로 **불변성 보장**이 가능합니다.

마치 레고 블록을 조립하는 것처럼, 빌더는 모든 부품을 맞춘 후 `build()`라는 최종 조립 버튼을 눌러야 완성품이 나옵니다. 중간 상태는 외부에 노출되지 않습니다.

### Q. `try-with-resources`에서 예외가 두 개 발생하면?

`try` 블록의 예외가 기본 예외로 전파되고, `close()`의 예외는 **suppressed exception**으로 첨부됩니다. `Throwable.getSuppressed()`로 확인할 수 있습니다.

### Q. 왜 enum 싱글턴이 최선인가?

직렬화/역직렬화 시 새 인스턴스 생성 불가, 리플렉션으로 생성자 호출 불가, `clone()` 불가. JVM 수준에서 단일 인스턴스를 보장합니다.

### Q. 메모리 누수를 의심해야 하는 세 가지 패턴은?

① 자체 메모리를 관리하는 클래스(스택, 캐시), ② 리스너/콜백 등록 후 해지 안 함, ③ `Map`에 넣고 제거하지 않는 캐시.

마치 수도꼭지를 잠그지 않은 채 방치하는 것처럼, 참조를 해제하지 않으면 메모리가 조금씩 새어나가 결국 OutOfMemoryError가 발생합니다.
...
**객체 생성 원칙의 핵심은 생성자의 한계를 극복하고 객체 생성을 명확하고 안전하게 만들기 위해서다.**

정적 팩터리 메서드를 생성자보다 선호하는 이유는 이름을 가질 수 있고, 매번 새 인스턴스를 생성하지 않아도 되며, 반환 타입의 하위 타입을 반환할 수 있기 때문이다. `Boolean.valueOf(true)`는 불필요한 객체 생성 없이 캐싱된 인스턴스를 반환한다.
