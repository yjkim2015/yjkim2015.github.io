...
```mermaid
graph LR
    subgraph "초기 상태 (캐시 크기: 3)"
        A1["A (가장 오래됨)"] --> B1["B"] --> C1["C (가장 최근)"]
    end
    subgraph "D 추가 후 (A 제거)"
        B2["B (가장 오래됨)"] --> C2["C"] --> D2["D (가장 최근)"]
    end
    C1 -->|"D 추가 → A 제거"| B2
```
...
```mermaid
graph TD
    NEW["새 항목"] --> WC["Window (1%)\nLRU"]
    WC -->|TinyLFU 통과| PROB["Probation (20%)\n추방 후보"]
    PROB -->|재접근| PROT["Protected (80%)\n자주 사용"]
    PROT -->|초과 시 강등| PROB
    PROB -->|추방| EVICT["Eviction"]
```
...
```mermaid
graph TD
    subgraph "실제 데이터 (캐시에 값 보관)"
        T1["T1: 최근 한 번 접근\nLRU 방식"]
        T2["T2: 두 번 이상 접근\nLFU+LRU 혼합"]
    end
    subgraph "Ghost 정보 (키만 보관, 값 없음)"
        B1["B1: T1에서 추방된 항목의 Ghost"]
        B2["B2: T2에서 추방된 항목의 Ghost"]
    end
    T1 -->|"추방 시"| B1
    T2 -->|"추방 시"| B2
    B1 -->|"Ghost Hit → T1 비율 증가"| T1
    B2 -->|"Ghost Hit → T2 비율 증가"| T2
```
