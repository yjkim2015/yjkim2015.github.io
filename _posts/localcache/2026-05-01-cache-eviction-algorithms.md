...
```mermaid
sequenceDiagram
    Window_(1%)->>Probation_(20%): TinyLFU 통과
    Probation_(20%)->>Protected_(80%): 재접근
    Protected_(80%)->>Probation_(20%): 초과 시 강등
    Probation_(20%)->>Eviction: 추방
```
...
```mermaid
sequenceDiagram
    T1:_최근_한_번_접근->>B1:_T1_Ghost_키: 추방
    T2:_두_번_이상_접근->>B2:_T2_Ghost_키: 추방
    B1:_T1_Ghost_키->>T1:_최근_한_번_접근: Ghost Hit→T1 비율↑
    B2:_T2_Ghost_키->>T2:_두_번_이상_접근: Ghost Hit→T2 비율↑
```
