...
```mermaid
graph LR
    BL["Business Logic"] -->|조회| LC["Local Cache..|Cache Miss| EXT["External Store"]
```
...
graph LR
    ...
    WC -->|TinyLFU 통과| PROB["Probation..|재접근 시 승진| PROT["Protected..|초과 시 강등| PROB
```
...
```mermaid
graph LR
    HEAP["Heap Tier (수십 MB)<"]
    OFFHEAP["Off-Heap Tier (수백"]
    DISK["Disk Tier (수십 GB~)"]
    HEAP -->|"용량 초과 시 하위 계층으로"| OFFHEAP
    OFF..|"용량 초과 시 하위 계층으로"| DISK
```
