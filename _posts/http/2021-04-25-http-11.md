...
sequenceDiagram
    ...
    participant S as 원 서버
    Note over C,S: no-cache + 원 서버 장애
    C->>P: GET /page.html
    P->>S: 검증 시도 (응답 없음)
    P-->>C: 오래된 캐시 반환 가능(200)
    Note over C,S: must-revalidate + 원 서버 장애
    C->>P: GET /page.html
    P->>S: 검증 시도 (응답 없음)
    P-->>C: 반드시 504 Gateway Timeout
```
...
```mermaid
sequenceDiagram
    participant K1 as 사용자1
    participant K2 as 사용자2
    participant P as 프록시(CDN)
    participant S as 원 서버
    K1->>P: GET /video.mp4
    P->>S: 캐시 없음 - 원 서버 요청
    S-->>P: 200 OK [500MB] max-age=86400
    P-->>K1: 500MB 반환
    K2->>P: GET /video.mp4
    P-->>K2: 캐시 히트 - 즉시 반환
```
