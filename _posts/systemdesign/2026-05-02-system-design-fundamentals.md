...
### 면접 포인트 1️⃣ "99.99% 가용성을 어떻게 달성하나요?"

단순히 "이중화"라고 답하면 안 됩니다. **배포 전략(블루/그린), DB 페일오버 시간(보통 30~60초), 그 동안의 요청 처리 방법(큐잉/재시도)**까지 구체적으로 답해야 합니다.
...
### 면접 포인트 2️⃣ "CAP에서 항상 P를 선택해야 하는 이유는?"

분산 시스템에서 네트워크 장애는 피할 수 없습니다. AWS에서도 연간 수십 번의 네트워크 이슈가 발생합니다. P를 포기하면 분산 시스템을 만드는 의미가 없습니다. 따라서 실제 선택은 C와 A 사이입니다. CP를 선택하면 파티션 시 가용성을 포기하고 일관성을 유지합니다(결제, 재고). AP를 선택하면 오래된 데이터를 서빙하더라도 서비스를 계속합니다(SNS 피드, 좋아요).
...
graph LR
    ...
    A -->|"정상 운영"| C{"지연 vs 일관성"}
    B -->|"가용성"| D["PA/EL: Cassandra·D"]
    B -->|"일관성"| E["PC/EC: Zookeeper·H"]
    C -->|"지연"| D
...
```mermaid
sequenceDiagram
    LB__Nginx/ALB->>서버1_CPU30%: 호출
    LB__Nginx/ALB->>서버2_CPU80%: 호출
    LB__Nginx/ALB->>서버3_CPU15%: 호출
    서버1_CPU30%->>DB: 호출
    서버2_CPU80%->>DB: 호출
    서버3_CPU15%->>DB: 호출
```
