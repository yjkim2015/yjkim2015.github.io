...
| 컴포넌트 | 핵심 역할 | 내부 동작 흐름 |
|----------|----------|--------------|
| **리뷰 작성 서비스** | 구매 검증 + PENDING 저장 | 구매 이력 확인(90일) → PENDING 저장 → Outbox → Kafka `review.created` 발행 |
| **스팸 탐지 파이프라인** | 3단계 가짜 리뷰 필터 | 1단계 규칙(50ms) → 2단계 ML 스코어(0~1) → 3단계 사람 검토 (결과는 재학습 데이터) |
| **평점 집계 서비스** | 베이지안 평균 실시간 유지 | 상태 변경 이벤트 구독 → `rating_stats` 원자 업데이트 → Redis 캐시 무효화 |
| **리뷰 조회 서비스** | 읽기 8,000:1 비율 최적화 | L1 Caffeine(5분) → L2 Redis(30분) → ES → MySQL Replica 순서 조회 |
| **미디어 처리 서비스** | 서버 무부담 미디어 업로드 | Pre-signed URL 발급 → 클라이언트 S3 직접 업로드 → Lambda 트리거 → 썸네일/트랜스코딩 → CDN |
...
> **비유**: 새로 생긴 식당이 손님 2명에게 별점 5점을 받았다고 해서 수천 명이 검증한 4.7점 노포보다 위에 올라가면 안 됩니다. 베이지안 평균은 "리뷰가 적을수록 전체 평균 쪽으로 끌어당기는" 중력 역할을 합니다.
...
리뷰가 적은 상품은 `BAYESIAN_C` 가중치로 전체 평균(`globalMean`)에 가깝게 끌어당깁니다.
...
평점 통계는 별도 `rating_stats` 테이블에 비정규화해서 관리합니다. 리뷰 조회 때마다 SUM/COUNT를 집계하면 리뷰 1만 개 상품에서 매번 1만 행을 스캔해야 합니다. 아래 테이블이 그 집계 비용을 제거합니다.
...
구매 검증 → IP 빈도 → 텍스트 MinHash 중복 → 신규 계정 순서로 체크하며, 위반 2개 이상이면 `flagged` 판정합니다.
...
|---------|---------|
| < 0.5 | 즉시 공개 |
| 0.5 ~ 0.8 | 스팸 플래그만 설정하고 공개 |
| > 0.8 | 사람 검토 큐로 이관 |

1차 규칙을 통과한 리뷰를 Kafka Consumer가 비동기로 ML 모델에 전달합니다.
...
> **비유**: "도움이 됨 5/5"인 리뷰와 "도움이 됨 95/100"인 리뷰는 비율이 같지만 신뢰도가 전혀 다릅니다. Wilson Score는 "투표 수가 적을수록 하단에 배치"하는 통계적 방식입니다.
...
투표 수(`total`)가 적을수록 `z²/n` 항이 커져 하한값이 낮아집니다. 불확실할수록 자동으로 하단 배치됩니다.
...
읽기:쓰기 = 8,000:1이므로 DB를 직접 읽으면 DB가 버티지 못합니다. 아래는 L1→L2→DB 순서로 조회하는 계층 구조입니다.

| 계층 | 저장소 | TTL | 목적 |
|------|-------|-----|------|
| L1 | Caffeine (JVM 로컬) | 5분 | 인스턴스 내 초고속 조회 |
| L2 | Redis | 30분 | 분산 환경 공유 캐시 |
| L3 | MySQL Replica | — | 캐시 미스 시 최종 소스 |

새 리뷰 작성 시 해당 상품의 Redis 캐시를 즉시 무효화합니다.
...
| # | 실수 | 결과 | 올바른 방법 |
|---|------|------|-----------|
| 1 | 단순 평균(`AVG`)으로 별점 집계 | 리뷰 2개짜리 5.0점이 1만 개짜리 4.7점보다 상단 노출, 풀 스캔 부하 | 베이지안 평균 + `rating_stats` 비정규화 테이블 |
| 2 | 스팸 탐지를 동기 처리 | ML 추론 500ms~2초로 작성 응답 지연, 타임아웃 시 중복 제출 | `PENDING` 즉시 저장 후 Kafka Consumer 비동기 탐지 |
| 3 | 구매 검증 없이 리뷰 허용 | 경쟁사 1점 도배, 셀러 5점 양산, 플랫폼 신뢰 붕괴 | 구매 검증 리뷰에 가중치 부여 또는 "구매확인" 배지 분리 |
| 4 | 서버가 이미지를 직접 수신·S3 업로드 | 대역폭 포화, 대용량 업로드 중 커넥션 고갈 | Pre-signed URL 발급 → 클라이언트 S3 직접 업로드 |
| 5 | 리뷰 삭제 시 평점 재계산 없음 | 스팸 100개 삭제해도 평점 그대로, `rating_stats` 불일치 | 상태 변경 이벤트 발행 → 역방향 업데이트, hard delete 대신 soft delete |
