...
```mermaid
graph LR
    FAIL["Redis 장애 감지"] --> CB["Circuit Breaker OPEN"]
    CB --> DB["DB 비관적 락 폴백"]
    DB -->|"느리지만 안전"| ORDER["주문 처리 계속"]
    FAIL --> ALERT["PagerDuty 알림"]
    ALERT --> OPS["운영팀 대응"]
    OPS -->|"Redis 복구"| SYNC["Redis-DB 재고 동기화"]
    SYNC --> RESUME["정상 운영 재개"]
```
...
### Q. Redis 선차감 후 MySQL 기록이 실패하면 어떻게 되나요?
...
### Q. 글로벌 서비스로 확장할 때 재고를 어떻게 관리하나요?
