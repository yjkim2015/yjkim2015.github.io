...
```mermaid
graph LR
    C["클라이언트"] -->|"POST /upload/in..| A["API 서버"]
    A -->|"upload_id, pres..| C
...
```mermaid
graph LR
    C["클라이언트"] -->|"업로드 요청"| A["API Svr"]
  ..|"Presigned URL 요청"| S["AWS S3"]
   ..|"15분 유효 URL"| A
    A -->|"Presigned URL"| C
    C -->|"직접 업로드(API 우회)"| S
    S -->|"완료 이벤트"| A
...
```mermaid
flowchart LR
    E1[엣지 서버 서울] -->|캐시 미스| OS[Origin Shiel..|캐시 미스| OS
    E3[엣지 Sv..|캐시 미스| OS
...
```mermaid
flowchart LR
    CAM[중계 카메라] -->|RTMP| IG[인제스트<br/>다중 ..|LL-HLS| CDN[CDN 엣지<br/>..|5000만 시청자 서빙| V[시청자]
```
