...
### 면접 포인트 1: 멱등성 키를 서버가 생성하지 않고 클라이언트가 생성하는 이유는 무엇인가요?
...
### 면접 포인트 2: PG_PENDING 상태에서 서버가 재시작되면 어떻게 복구하나요?
...
### 면접 포인트 3: 복식부기에서 원장 레코드가 수천만 건이 되면 잔액 조회가 느려지지 않나요?
...
### 면접 포인트 4: 정산 대사(Reconciliation)에서 불일치가 발생하면 어떻게 처리하나요?
...
### 면접 포인트 5: 카드번호를 절대 서버에 저장하지 않으려면 결제 폼을 어떻게 설계하나요?
