...
| 순수 NoSQL (MongoDB) | 문서별 유연한 스키마, 단일 도큐먼트 조회 빠름 | 복잡한 집계 쿼리 약함, 트랜잭션 부재(구버전) | 스키마 변동이 매우 잦은 경우 |
...
검색 QPS (평균): 300만 건/일 ÷ 86,400 ≈ 35 QPS
검색 QPS (피크): 35 × 100 = 3,500 QPS (블프, 특가 이벤트)
...
A. ES 샤드당 권장 크기는 50GB, 문서 2억 건입니다. 10억 건이면 샤드 5개 이상 필요합니다. 카테고리별 인덱스 분리와 ILM(Index Lifecycle Management)으로 삭제된 상품은 콜드 스토리지로 이전해 활성 인덱스 크기를 관리합니다.
