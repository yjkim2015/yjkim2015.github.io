...
모든 상품 CRUD와 검색 요청의 진입점입니다. 셀러가 상품을 등록·수정하면 MySQL에 저장하고 Transactional Outbox 테이블에 이벤트를 기록합니다. 구매자의 검색 요청이 들어오면 L1 로컬 캐시(Caffeine) → L2 Redis → Elasticsearch 순서로 조회를 시도합니다. seller_id 클레임을 JWT에서 강제 주입해 셀러 간 데이터 격리를 보장합니다.

**MySQL (단일 진실 공급원)**
가격·재고·셀러ID·상태 등 핵심 컬럼을 정규화해 저장합니다. 쓰기는 마스터 노드 단독으로 처리하고, ES 장애 시 폴백 조회에는 읽기 레플리카를 사용합니다. 재고 차감은 항상 MySQL 트랜잭션으로 처리하므로 ES 인덱스와 일시적으로 불일치가 생겨도 결제 오류로 이어지지 않습니다.

**Kafka + Debezium CDC 파이프라인**
MySQL 바이너리 로그를 Debezium이 감지하면 Kafka의 `product.events` 토픽으로 발행합니다. ES Indexer Consumer가 이 이벤트를 구독해 Bulk API로 인덱싱합니다. Consumer Lag이 10,000건을 초과하면 경보를 발생시키고 Consumer 인스턴스를 수평 확장합니다. 상품 등록 후 검색 반영 목표는 10초 이내입니다.

**Elasticsearch (검색·집계 엔진)**
nori 형태소 분석기로 한국어 토큰을 분리하고, `function_score`로 판매량·평점을 랭킹에 반영합니다. 단일 쿼리에 `aggs`를 포함해 브랜드별·가격대별 상품 수를 함께 집계하므로 필터 사이드바 UI를 한 번의 네트워크 왕복으로 채울 수 있습니다. 샤드당 권장 크기는 10~50GB이며 3억 건 기준 10개 샤드·1 레플리카로 구성합니다.

**Redis (다계층 캐시)**
상품 상세(TTL 5분)·카테고리 트리(TTL 1시간)·브랜드 목록을 캐싱합니다. 가격·재고 변경 이벤트를 Kafka Consumer가 수신하면 해당 키를 즉시 `DEL`합니다. L1 로컬 캐시(Caffeine, TTL 30초)가 앞에 있어 Redis 왕복조차 생략하므로 인기 상품 상세 응답 시간이 1ms 이내입니다.
