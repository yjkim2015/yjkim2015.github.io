...
**후보 생성기 (Candidate Generator)**
사용자 임베딩 벡터를 Faiss HNSW 인덱스에 쿼리해 코사인 유사도 기준 상위 N개를 반환합니다. 동시에 최근 조회 이력 기반 Item-to-Item, 카테고리별 인기도 소스를 병렬 호출해 합집합을 만듭니다. 이미 구매한 상품 ID 셋을 Redis에서 읽어 즉시 제거하고 최종 2,000개를 반환합니다. 단일 소스 실패 시 나머지 소스로 폴백해 서비스가 끊기지 않습니다.

**랭킹 서비스 (Ranking Service)**
후보 2,000개의 item_id를 받아 Redis Pipeline으로 피처를 한 번에 가져옵니다 (N번 왕복이 아닌 1번 왕복). TorchServe에 배치 추론 요청을 보내 각 아이템의 구매 확률 스코어를 계산합니다. Wide 파트는 카테고리 교차 피처, Deep 파트는 임베딩을 받아 최종 스코어를 합산합니다. 결과는 내림차순 정렬 후 재랭킹 서비스로 전달됩니다.

**임베딩 DB (Faiss + Pinecone)**
상품과 사용자를 128차원 벡터로 표현하며, HNSW 그래프 구조로 ANN(Approximate Nearest Neighbor) 검색을 지원합니다. 쿼리 시 전체 5억 개를 스캔하지 않고 그래프 탐색으로 후보를 압축합니다. 인덱스는 매일 새벽 ALS 배치 결과로 갱신되며, 신규 상품은 CB 임베딩을 생성해 인크리멘탈 추가합니다.

**피처 스토어 (Redis)**
사용자별 피처(최근 조회 카테고리 TOP5, 평균 단가대, 선호 브랜드)를 Redis Hash로 저장합니다. Flink 스트림 프로세서가 Kafka에서 클릭 이벤트를 읽어 1시간 슬라이딩 윈도우로 집계 후 Redis에 HSET으로 기록합니다. TTL은 24시간으로 설정해 비활성 사용자 피처가 자동 삭제되도록 합니다. 랭킹 서버가 MySQL에서 피처를 직접 읽으면 QPS 1만 시 DB 쿼리 1억 건이 발생하므로 이 레이어가 필수입니다.

**이벤트 수집 파이프라인 (Kafka + Flink)**
프론트엔드가 클릭·구매·장바구니 이벤트를 Kafka 토픽으로 발행합니다. Flink Consumer Group이 이벤트를 읽어 사용자별 세션 집계, 카테고리별 실시간 CTR 계산을 처리합니다. 집계 결과는 Redis(온라인 피처)와 HDFS(오프라인 학습 데이터) 두 곳에 씁니다. 클릭 후 Redis 반영까지 평균 5분이 목표입니다.
