...
배송 시스템 설계에서 순서대로 답해야 할 핵심 결정 4가지입니다. 각 결정에서 "왜 이 선택인가"를 명확히 하지 않으면 면접에서 "그냥 DB 폴링으로 하면 되지 않나요?"라는 후속 질문에 답할 수 없습니다.
...
- 이유: 배송 추적은 서버(기사 위치 서버)→클라이언트(고객 앱) 단방향입니다. 웹소켓의 양방향 복잡도가 필요 없습니다. SSE는 HTTP/2 위에서 동작하여 기존 로드밸런서·방화벽 설정 변경 없이 사용 가능합니다. 연결이 끊어지면 브라우저가 자동 재연결합니다.
- 안 하면: 폴링을 5초마다 하는 고객이 100만 명이면 초당 20만 건의 위치 조회 요청이 발생합니다. 실제 위치 변화가 없어도 요청이 오기 때문에 서버 자원의 90%가 "변화 없음"을 응답하는 데 낭비됩니다.
...
- 이유: 새벽배송은 오전 7시 전이라는 절대적 시간 제약이 있습니다. 라운드로빈은 이 제약을 무시합니다. VRP(Vehicle Routing Problem)를 풀면 이론적으로 최적이지만 NP-Hard라 실시간 적용이 불가능합니다. 현실적 대안은 그리디 + 지역 탐색(Local Search)으로 준최적해를 1초 이내에 계산하는 것입니다.
- 안 하면: 라운드로빈으로 배정하면 강남 기사가 잠실 배송을 맡고, 잠실 기사가 강남 배송을 맡는 상황이 발생합니다. 쿠팡 연구에 따르면 최적화 없는 배차 대비 주행 거리가 30~40% 증가합니다.
...
**우리의 선택: 재고+거리 통합 최적화**
- 이유: 최근접 창고에 재고가 없으면 거리 계산이 의미 없습니다. 재고 조회 후 거리 가중치를 적용하여 "재고 있는 창고 중 가장 가까운 곳"을 선택합니다. 상품이 여러 창고에 분산되어 있으면 단일 창고 출고로 합치거나 분할 출고 여부도 결정합니다.
- 안 하면: 2021년 추석 연휴 전날, 특정 창고 재고가 동시에 소진되면서 단순 거리 기반 배정 시스템이 재고 없는 창고로 주문을 배정했습니다. 출고 실패 후 수동 재배정으로 수천 건의 배송이 하루 이상 지연되었습니다.
...
- 이유: "배송 완료" 후 고객이 "받은 적 없다"고 분쟁을 제기하면 배송기사가 언제 어디서 배달했는지 GPS 이력까지 포함한 완전한 증거가 필요합니다. 이벤트 소싱은 모든 상태 전이를 불변 이벤트로 저장합니다. 상태 머신은 "취소됨 → 배달중" 같은 잘못된 전이를 코드 레벨에서 차단합니다.
- 안 하면: 상태 컬럼만 UPDATE하면 "이 주문이 왜 갑자기 배달완료 상태인가?"를 추적할 수 없습니다. CJ대한통운은 배송 분쟁 대응을 위해 모든 스캔 이벤트를 시계열 DB에 7년간 보관합니다.
...
이 규모에서 단일 MySQL 인스턴스로 위치 데이터를 처리하는 것은 불가능합니다. 위치 데이터는 시계열 특성이 강하므로 전용 시계열 DB 또는 Redis를 활용한 계층 구조가 필요합니다.
...
> **보안**: `X-Driver-Id` 헤더 대신 JWT 토큰으로 기사를 인증해야 합니다. GPS 데이터의 속도·가속도 이상치를 탐지하여 GPS 스푸핑을 방어하고, 비현실적인 위치 변화(1초에 100km 이동)는 무시합니다.
...
public class TrackingController {
    ...
    public SseEmitter trackOrder(@PathVariable String orderId) {
        ...
        // ⚠️ 프로덕션에서는 RedisMessageListenerContainer를 사용하여
        // 커넥션 풀을 공유해야 합니다. 요청마다 getConnection()을 호출하면
        // 커넥션 누수가 발생합니다.
...
> **확장성 한계**: Redis Pub/Sub는 구독자가 없으면 메시지가 소실되고, 50만 동시 접속 시 SSE 서버의 커넥션 수가 폭발합니다. 서버가 여러 대이면 각 서버마다 Redis 구독이 필요하여 메시지 중복 수신 문제가 생깁니다. 대규모 서비스에서는 Redis Streams나 Kafka를 SSE 서버 앞단에 두어 at-least-once 보장과 백프레셔를 처리합니다.
...
**상황**: 기사가 아파트 지하 주차장에 진입하여 GPS 신호가 끊겼습니다. 고객 앱에서 기사 위치가 멈췄습니다.
...
**상황**: 오전 10시 특가 세일 시작과 함께 초당 주문이 1,850건으로 10배 급증했습니다. 배차 서버 CPU가 95%에 도달했습니다.
...
**상황**: 배송 상태를 저장하는 MySQL이 Primary 장애로 60초간 쓰기 불가 상태가 됐습니다. 이 시간 동안 수백 건의 배송 완료 이벤트가 유실될 위기입니다.
...
**상황**: 기사가 실수로 옆집 문 앞에 놓고 배달 완료 버튼을 눌렀습니다. 고객은 "받은 적 없다"고 신고했습니다.
