...
| 컴포넌트 | 핵심 역할 | 내부 동작 흐름 |
|----------|----------|--------------|
| **창고 선택기** | 재고 있는 창고 중 최적 선택 | 재고 병렬 조회 → Haversine 거리 계산 → 거리(60%)+마감여유(40%) 가중 합산 → 최저 점수 선택 |
| **배차 서버** | 기사에게 최적 경로로 배송 배정 | 반경 내 가용 기사 필터링 → 최근접 이웃 정렬 → 2-opt 경로 개선 → 기사 앱 푸시 |
| **위치 수집기** | GPS 실시간 수신 + 이력 저장 | Kafka 수신 → Redis `driver:location:{id}` TTL 30초 갱신 + InfluxDB 이력 영속 저장 (병렬) |
| **추적 서버** | 고객에게 기사 위치 실시간 푸시 | SSE 연결 → Redis Pub/Sub `location-update:{driverId}` 구독 → 좌표 변경 시 즉시 push |
| **ETA 예측기** | 도착 예정 시각 계산 | 잔여 배송 건수 × 평균 처리시간(이력) + 지도 API 교통정보 합산, 매일 오차 집계 후 보정 |
...
> **비유**: 가장 가까운 편의점이라도 문 닫기 5분 전이면 다음 편의점으로 가는 게 낫습니다. 창고 선택도 마찬가지 — 거리만 보면 안 되고 마감 여유를 함께 고려합니다.

아래 코드는 재고 있는 창고만 후보에 올리고, 거리·마감 가중 점수가 가장 낮은 창고를 선택합니다.
...
ETA 계산에 들어가는 요소는 다음과 같습니다.

| 요소 | 데이터 소스 | 비고 |
|------|-----------|------|
| 잔여 배송 건수 | 배차 서버 DB | 내 주문 앞에 몇 건 남았는지 |
| 건당 평균 처리 시간 | 이력 DB (지역·시간대별) | 새벽배송은 교통 변수 적어 정확도 높음 |
| 실시간 교통 정보 | 카카오맵/T맵 API | 현재 위치 → 배달지 예상 이동 시간 |
| 기사 이동 속도 | Redis 최신 위치 이력 | 비정상 속도 필터링 포함 |

아래 코드는 세 요소를 합산해 ETA를 계산하고, 신뢰 구간 ±15분을 함께 반환합니다.
...
| # | 실수 | 결과 | 올바른 방법 |
|---|------|------|-----------|
| 1 | 기사 위치를 폴링으로 가져오는 구조 | 50만 명 동시 추적 시 초당 10만 건, 90%가 "변경 없음" 낭비 | SSE로 서버가 변경 시에만 Push |
| 2 | GPS 좌표 유효성 검증 생략 | `(0,0)` 같은 이상 좌표로 ETA 계산 완전 붕괴 | 국가 경계 검사 + 이전 좌표 대비 이동 거리 검증 |
| 3 | Kafka 파티션 키 랜덤 설정 | "완료→배달중" 상태 역행 현상 발생 | 파티션 키를 주문 ID로 고정 |
| 4 | 기사 연락 불가 시 재배차 없이 방치 | 고객이 무한정 "배달중" 화면, CS 폭증 | Redis TTL 30초 감지 → 지연 알림 + 관제 알림 + 자동 재배차 |
| 5 | VRP 최적화를 동기 처리 | 배차 서버 CPU 포화 시 주문 서비스 전체 장애 전파 | Kafka 큐 비동기 처리, 주문 서비스는 즉시 "배차 중" 반환 |
...
### Q. 기사 위치를 Redis에만 저장하면 Redis 재시작 시 위치 데이터가 사라지지 않나요?
...
### Q. VRP가 NP-Hard인데 실시간 배차를 어떻게 처리하나요?
...
### Q. 배송 이벤트가 순서 없이 도착하면 어떻게 처리하나요?
