...
sequenceDiagram
    ...
    participant App as 우리앱
    participant Kakao as 카카오
    U->>App: 카카오로그인 클릭
    App->>U: 카카오 인가 URL로 리다이렉트(code_challenge 포함)
    U->>Kakao: 로그인 + 동의
    Kakao->>App: 인가 코드 전달
    App->>Kakao: 코드+code_verifier로 토큰 교환
    Kakao-->>App: ID Token + Access Token 발급
```
...
sequenceDiagram
    participant U as 사용자
    participant AS as Auth서버
    participant RS as 리소스서버
    U->>AS: 로그인 (ID/PW)
    AS-->>U: Access(15분) + Refresh(7일) 발급
    U->>RS: API 요청 (Access Token)
    RS-->>U: 응답
    U->>AS: Access 만료 후 Refresh로 재발급
    AS-->>U: 새 Access + 새 Refresh (Rotation)
```
...
sequenceDiagram
    participant U as 사용자
    participant Shop as 쇼핑몰
    participant Auth as SSO서버
    participant Seller as 판매자센터
    U->>Shop: 쇼핑몰 접속
    Shop->>Auth: 인증 요청 (redirect)
    U->>Auth: 로그인 (한 번만)
    Auth-->>Shop: ID Token 발급
    U->>Seller: 판매자센터 접속
    Seller->>Auth: 인증 요청
    Auth-->>Seller: 이미 로그인됨 → 즉시 ID Token
```
...
### 면접 포인트 2️⃣ Session vs JWT 어떤 걸 선택하는가?
...
### 면접 포인트 3️⃣ RBAC만으로 "본인 데이터만 접근" 구현이 가능한가?
...
### 면접 포인트 4️⃣ Refresh Token Rotation이 필요한 이유는?
...
### 면접 포인트 5️⃣ SSO에서 한 서비스 로그아웃이 다른 서비스에도 적용되어야 하는가?
