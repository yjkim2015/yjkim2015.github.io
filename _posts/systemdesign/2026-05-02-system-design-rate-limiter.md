...
| User-Agent | Chrome/Safari 등 | python-requests, curl, 위조 UA |
| 쿠키/JS | 쿠키 저장, JS 실행 | 쿠키 무시, JS 미실행 |
| 페이지 패턴 | 홈→목록→상세 (자연스러운 흐름) | 상세 페이지만 순차 접근 |
| 세션 | 5~30분 | 수시간 연속 |
| 마우스/스크롤 | 있음 | 없음 |
| TLS 핑거프린트 | 브라우저 고유 JA3 해시 | 라이브러리 기본값 |
