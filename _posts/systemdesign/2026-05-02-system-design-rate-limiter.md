...
2단계: 쿠키 기반 검증 (가장 가성비 좋은 방어)
   - 첫 요청 시 Set-Cookie로 검증 토큰 발급
   - 이후 요청에 쿠키가 없으면 봇으로 판정
   - 쿠키 값에 타임스탬프+HMAC 서명 → 위조 방지
   - 효과: requests/curl 등 쿠키 미지원 봇 즉시 차단

3단계: User-Agent + IP 패턴 분석
   - 알려진 봇 UA 차단 (python-requests, curl, scrapy)
   - 동일 IP에서 분당 100+ 상세 페이지 접근 시 차단
   - 효과: 초보 스크래퍼 차단

4단계: JS Challenge + 핑거프린팅
...
5단계: 허니팟 트랩 (능동적 탐지)
   - CSS display:none 링크를 페이지에 숨김 (사용자는 안 보이지만 봇은 크롤링)
   - 숨긴 링크를 클릭한 IP = 100% 봇 → 즉시 블랙리스트
   - robots.txt에 Disallow한 경로를 방문한 IP도 봇 확정
   - 효과: 오탐 0%, 확실한 봇만 잡음

6단계: TLS/HTTP2 핑거프린팅 (JA3/JA4)
   - TLS 핸드셰이크의 cipher suite 순서가 브라우저마다 고유
   - Chrome/Safari/Firefox 각각 JA3 해시가 다름
   - python-requests, Go http.Client는 브라우저와 완전히 다른 JA3 해시
   - 효과: User-Agent를 위조해도 TLS 핑거프린트로 봇 탐지

7단계: 행동 분석 (ML 기반)
   - 요청 간격 표준편차 분석 (봇은 σ ≈ 0)
   - 페이지 접근 패턴 분석 (정상: 트리 탐색, 봇: 순차 스캔)
   - 마우스 이벤트/스크롤 깊이 (프론트에서 수집)
   - Referer 체인 검증 (정상: 구글→목록→상세, 봇: 상세 직접 접근)
   - 효과: Headless Chrome 봇까지 탐지

8단계: 응답 변조 (최후의 수단)
   - 봇으로 판정된 요청에 가짜 데이터/지연된 데이터 반환
   - 가격 정보를 랜덤으로 ±20% 변조 → 경쟁사 가격 수집 무력화
   - 응답 지연을 점진적으로 늘림 (Tar Pit) → 봇 자원 소모
...
알고리즘보다 **"무엇을 키(key)로 카운팅하는가"**가 실무에서 더 중요한 결정이다. 기준을 잘못 잡으면 정상 사용자가 차단되거나, 공격자가 우회한다.

| 차단 기준 | 장점 | 단점 | 적합한 상황 |
|-----------|------|------|-----------|
| **IP 주소** | 구현 가장 단순, 인증 불필요 | 회사/학교(NAT) 전체가 하나의 IP → 정상 사용자 연좌제 차단. VPN/프록시로 IP 변경 우회 가능 | DDoS 1차 방어, 비인증 API |
| **API 키** | 개발자/서비스 단위 제한 | 키 탈취 시 무방비. 키 없는 공개 API에 적용 불가 | B2B API, 외부 개발자 포털 |
| **사용자 ID (로그인)** | 가장 정확한 사용자 단위 제한 | 로그인 전 요청은 제한 불가. 계정 대량 생성으로 우회 | 로그인 후 API (결제, 게시글 작성) |
| **디바이스 핑거프린트** | 비로그인에서도 디바이스 구분 | 정확도 80~90%, 브라우저 업데이트 시 변경 | 앱 환경, 부정 가입 방지 |
| **엔드포인트별** | 민감 API(결제)만 강하게 제한 | 엔드포인트 수만큼 정책 관리 필요 | `/api/payment`: 분당 5건, `/api/search`: 분당 100건 |

**실무에서는 단일 기준이 아니라 복합 키를 사용한다**:

```
# 1단계: IP 기반 (비인증 API 보호)
rate_limit("ip:{client_ip}", limit=1000, window=60s)

# 2단계: 사용자 기반 (인증된 요청)
rate_limit("user:{user_id}", limit=100, window=60s)

# 3단계: 엔드포인트별 차등 (민감 API 보호)
rate_limit("user:{user_id}:endpoint:/payment", limit=5, window=60s)
rate_limit("user:{user_id}:endpoint:/search", limit=200, window=60s)

# 4단계: 사용자 티어별 차등 (유료 사용자 우대)
if user.tier == "enterprise":
    rate_limit("user:{user_id}", limit=10000, window=60s)
elif user.tier == "pro":
    rate_limit("user:{user_id}", limit=1000, window=60s)
else:
    rate_limit("user:{user_id}", limit=100, window=60s)
```

**안 하면 어떻게 되는가**:
- IP만 사용 → 대기업 사무실 직원 500명이 하나의 공인 IP를 공유. 한 명이 100건 호출하면 나머지 499명 차단
- 사용자 ID만 사용 → 로그인 안 한 봇이 IP를 바꿔가며 무한 요청. 차단 불가
- 엔드포인트 구분 없음 → 검색 API(무해)와 결제 API(위험)에 같은 한도 적용. 결제 봇 공격에 취약

> **면접 포인트**: "Rate Limiter의 키를 어떻게 설계하겠습니까?" — IP 하나만 답하면 주니어. "IP → 사용자 → 엔드포인트 → 티어 복합 키로 계층화하고, 로그인 전/후를 구분한다"가 시니어 답변이다.

---

### 결정 5: 초과 응답 — 429 즉시 거부 vs 큐잉
