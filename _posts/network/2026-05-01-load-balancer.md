...
graph LR
    L4["L4 LB"] -->|NAT| S1["서버1"]
    L4 -->|NAT| S2["서버2"]
    L7["L7 LB"] -->|/orders| O["Order"]
    L7 -->|/products| P["Product"]
```
...
```mermaid
sequenceDiagram
    L7_로드밸런서->>L7_로드밸런서: SSL 복호화
    L7_로드밸런서->>백엔드_서버: HTTP_평문
    백엔드_서버->>L7_로드밸런서: HTTP 응답
    L7_로드밸런서->>클라이언트: SSL 암호화→HTTPS
```
...
```mermaid
sequenceDiagram
    어떤_로드밸런서?->>NLB_L4: 초고성능 TCP/UDP 게임/금
    어떤_로드밸런서?->>ALB_L7: HTTP/HTTPS URL 기반
    어떤_로드밸런서?->>ALB_L7: SSL 종료 필요
    어떤_로드밸런서?->>NLB_L4: 고정 IP 필요
    어떤_로드밸런서?->>ALB_L7: 카나리 배포
    어떤_로드밸런서?->>NLB_L4: WebSocket 대용량
    어떤_로드밸런서?->>DNS_GSLB___L7: 글로벌 서비스
```
