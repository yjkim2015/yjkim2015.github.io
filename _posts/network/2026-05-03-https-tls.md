...
participant C as "클라이언트" participant S as "서버" C->>S: 1️⃣ ClientHello (버전, CipherSuites, 키파라미터)
...
루트 CA (Root CA) →(서명)→ 중간 CA (Intermediat, 중간 CA (Intermediat →(서명)→ 서버 인증서
...
participant C as "클라이언트" participant S as "서버" C->>S: ClientHello
