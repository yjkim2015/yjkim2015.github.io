...
sequenceDiagram
    ...
    S->>C: 2️⃣ ServerHello _ Certificate _ Finished
...
sequenceDiagram
    ...
    participant S as 서버
    C->>S: ClientHello
    S->>C: ServerHello _ 서버 인증서 _ CertificateRequest
    C->>S: 클라이언트 인증서 _ Client Finished
...
```mermaid
sequenceDiagram
    인증서_발급_CA->>CT_로그_서버: 인증서 제출
    CT_로그_서버->>인증서_발급_CA: SCT 반환 Signed Ce
    CT_로그_서버->>도메인_소유자: 발급 알림
```
...
1. **인증서 만료 모니터링 부재** — 인증서가 만료되면 브라우저가 "안전하지 않음" 경고를 표시하고 API 클라이언트는 연결 오류로 실패한다. 만료 30일 전부터 알람을 발생시키고, Let's Encrypt는 auto-renewal 크론잡을 설정해야 한다.

2. **TLS 구버전(1.0, 1.1) 미비활성화** — PCI DSS 규정과 보안 감사에서 TLS 1.0/1.1은 취약점으로 지적된다. 서버에서 TLS 1.2 이상만 허용하도록 설정하고 정기적으로 SSL Labs에서 등급을 확인해야 한다.

3. **인증서 체인 불완전 전송** — 중간 CA 인증서를 서버에서 함께 전송하지 않으면 일부 클라이언트에서 인증 실패가 발생한다. 서버 인증서 + 중간 CA 인증서를 체인 파일로 구성해야 한다.

4. **Mixed Content 문제** — HTTPS 페이지에서 HTTP 리소스를 로드하면 브라우저가 차단하거나 경고를 표시한다. 모든 리소스 URL을 https://로 통일하고, Content-Security-Policy 헤더로 HTTP 리소스를 차단해야 한다.

5. **HSTS 설정 누락** — HSTS(HTTP Strict Transport Security) 없이는 첫 번째 요청이 HTTP로 오면 중간자 공격에 취약하다. `Strict-Transport-Security: max-age=31536000; includeSubDomains`를 응답 헤더에 추가해야 한다.
