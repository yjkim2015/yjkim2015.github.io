...
```mermaid
graph LR
    O7["7.Application"] --> T4["Application(HTTP/FTP/DNS)"]
    O6["6.Presentation"] --> T4
    O5["5.Session"] --> T4
    O4["4.Transport(TCP/UDP)"] --> T3["Transport(TCP/UDP)"]
    O3["3.Network(IP/ICMP)"] --> T2["Internet(IP/ICMP/ARP)"]
    O2["2.DataLink"] --> T1["Network Access(Ethernet/WiFi)"]
    O1["1.Physical"] --> T1
```
