...
%%{init: {'theme': 'default', 'themeVariables': {'fontSize': '12px', 'nodePadding': '4px'}} }%%
graph LR
    OSI["OSI 7→5계층"] --> T4["Application"]
    O4["OSI 4계층"] --> T3["Transport"]
    O3["OSI 3계층"] --> T2["Internet"]
    O21["OSI 2→1계층"] --> T1["Network Access"]
```
...
## 왜 OSI 7계층인가? (vs TCP/IP 4계층)

실제 인터넷은 TCP/IP 4계층 모델로 동작한다. OSI 7계층은 실제 프로토콜 구현체가 아니라 **개념적 참조 모델**이다.

| 목적 | OSI 7계층 | TCP/IP 4계층 |
|------|----------|-------------|
| 용도 | 개념 이해·장애 진단 | 실제 구현 표준 |
| 계층 수 | 7 | 4 |
| 실무 사용 | 장애 계층 분리, 면접 | 실제 프로토콜 스택 |

OSI 7계층이 유용한 이유: 장애 발생 시 "물리 케이블 문제(1계층)인가, IP 라우팅 문제(3계층)인가, 애플리케이션 프로토콜 문제(7계층)인가"로 원인 범위를 좁히는 구조적 사고 틀을 제공한다.

---

## 실무에서 자주 하는 실수

**실수 1: L4와 L7 로드밸런서 혼동**

```
L4 로드밸런서: IP + Port 기준 분산 — HTTP 헤더·URL 경로를 볼 수 없음
L7 로드밸런서: HTTP 헤더·URL·쿠키 기반 분산
  /api/orders  → order-service
  /api/products → product-service
```

URL 기반 라우팅이 필요한데 L4를 사용하면 모든 트래픽이 하나의 서비스로만 몰린다.

**실수 2: 방화벽 계층 혼동 — 포트 열었는데 연결 안 됨**

```bash
# 3계층(IP) 통신은 되는데 4계층(포트) 막힌 경우
ping 192.168.1.10       # ✅ 응답 → 3계층 정상
telnet 192.168.1.10 80  # ❌ 실패 → 4계층 방화벽 차단

# 4계층은 되는데 7계층(TLS) 문제인 경우
curl http://example.com   # ✅ 성공
curl https://example.com  # ❌ 실패 → 인증서/TLS 문제
```

**실수 3: ARP 스푸핑 취약점 인식 부족**

같은 LAN 내에서 공격자가 가짜 ARP Reply를 보내 ARP 캐시를 오염시킨다. 피해자는 게이트웨이 MAC 대신 공격자 MAC으로 패킷을 보내 MITM 공격이 가능해진다. 방어책: 동적 ARP 검사(DAI), 정적 ARP 항목 설정.

---

## 면접 포인트

**Q1. OSI 7계층을 외우는 것보다 중요한 것은?**

각 계층의 역할 경계와 해당 계층에서 동작하는 장비·프로토콜을 연결하는 것이다. "스위치는 2계층에서 MAC 주소로 동작하고, 라우터는 3계층에서 IP 주소로 경로를 결정한다"처럼 개념을 계층에 매핑할 수 있어야 한다.

**Q2. TCP 3-Way Handshake가 OSI 어느 계층에서 동작하는가?**

4계층(Transport Layer)이다. TCP는 포트 번호로 프로세스를 식별하고, Sequence Number로 순서를 보장하며, ACK로 신뢰성을 제공한다. 3-Way Handshake는 4계층 연결 지향 특성의 구현이다.

**Q3. 같은 LAN의 두 컴퓨터가 통신할 때 사용되는 계층은?**

1·2·3계층 모두 관여한다. 3계층에서 같은 서브넷임을 확인하고, 2계층에서 ARP로 MAC 주소를 조회하며, 1계층에서 전기 신호로 전송한다. 라우터를 거치지 않으므로 3계층 라우팅 결정은 발생하지 않는다.

**Q4. 패킷이 라우터를 통과할 때 변경되는 필드는?**

3계층 IP 헤더의 TTL이 1 감소한다. TTL이 0이 되면 라우터가 패킷을 폐기하고 ICMP Time Exceeded를 보낸다. 2계층 프레임(MAC 주소)은 각 홉마다 새로 생성된다. 즉 IP 헤더의 출발지·목적지 IP는 유지되지만, MAC 주소는 매 홉마다 교체된다.

**Q5. HTTP/3가 UDP 위에서 동작하는 이유는?**

TCP의 Head-of-Line Blocking 문제 때문이다. TCP는 패킷 손실 시 그 뒤 모든 데이터가 블로킹된다. HTTP/3는 UDP 위에 QUIC을 구현해 스트림별 독립 순서 보장을 제공하므로 하나의 스트림 손실이 다른 스트림에 영향을 주지 않는다.
