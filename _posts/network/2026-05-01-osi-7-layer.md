...
```mermaid
graph LR
    SS["Slow Start\ncwnd 지수 증가"] -->|"ssthresh 도달"| CA["혼잡 회피\ncwnd 선형 증가"]
    CA -->|"타임아웃"| SS
...
```mermaid
graph LR
    CLI["클라이언트"] -->|"1 www.example.com?"| RR["재귀 리졸버\n(ISP DNS)"]
    RR -->|"2 .com 어디?"| ROOT["루트 네임서버\n(13개 클러스터)"]
    ROOT -->|"3 TLD 서버 IP"| RR
    RR -->|"4 example.com?"| TLD["TLD 네임서버\n(.com)"]
    TLD -->|"5 권위 서버 IP"| RR
    RR -->|"6 www.example.com?"| AUTH["권위 네임서버\n(example.com)"]
    AUTH -->|"7 93.184.216.34"| RR
```
...
```mermaid
graph LR
    OSI75["OSI 7·6·5계층"] -->|"매핑"| TCPAPP["TCP/IP Application"]
    OSI4["OSI 4계층"] -->|"매핑"| TCPTRANS["TCP/IP Transport"]
    OSI3["OSI 3계층"] -->|"매핑"| TCPNET["TCP/IP Internet"]
    OSI21["OSI 2·1계층"] -->|"매핑"| TCPLINK["TCP/IP Network Access"]
```
