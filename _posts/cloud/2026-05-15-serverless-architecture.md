...
Lambda 함수를 체이닝하는 가장 단순한 방법은 함수 안에서 다른 함수를 직접 호출하는 것이다. 하지만 이 방식에는 치명적인 문제가 있다. **에러가 중간에 발생하면 어느 단계에서 실패했는지 알 수 없고**, 재시도를 어디서부터 해야 할지 코드로 직접 구현해야 한다. 결제 → 재고차감 → 이메일 발송 중 이메일 발송에서 실패했다면, 결제와 재고차감을 다시 실행하면 중복 처리가 된다.

Step Functions는 이 문제를 **항공 관제탑** 방식으로 해결한다. 각 비행기(Lambda)는 자기 임무만 수행하고, 관제탑(Step Functions)이 전체 비행 계획, 비상 상황 대응, 재시도 여부를 중앙에서 통제한다. 워크플로우를 상태 머신으로 정의한다.
...
<details>
<summary>Q. Lambda Cold Start의 발생 원인과 줄이는 방법</summary>
...
<details>
<summary>Q. API Gateway REST API와 HTTP API의 차이</summary>
...
<details>
<summary>Q. 서버리스 아키텍처에서 상태 관리</summary>
...
<details>
<summary>Q. 이벤트 드리븐 아키텍처에서 메시지 중복 처리</summary>
...
<details>
<summary>Q. Lambda 비용이 EC2보다 비싸지는 시점</summary>
