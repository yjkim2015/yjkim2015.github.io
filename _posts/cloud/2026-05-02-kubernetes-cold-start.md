...
```mermaid
graph LR
    HPA["HPA"] -->|"Pod 생성"| K["kubelet"]
    K -->|"이미지 풀"| APP["App"]
    APP -->|"JVM 초기화"| RDY["Ready"]
    RDY --> HPA
```
...
```mermaid
graph LR
    SVC["Service"] -->|"트래픽 100%"| B["Blue"]
    G["Green"] -->|"Warm-up 완료"| SVC
    SVC -->|"트래픽 전환"| G
    B -->|"graceful shutdown"| SVC
```
...
```mermaid
graph LR
    BN["병목 원인"] -->|"이미지"| MS["멀티스테이지 빌드"]
    BN -->|"JVM"| GR["GraalVM/CDS"]
    BN -->|"Probe"| SP["startupProbe"]
    BN -->|"트래픽 예측"| CH["CronHPA"]
    MS --> CS["Cold Start 측정"]
    GR --> CS
    SP --> CS
    CH --> CS
...
### Q. Kubernetes에서 Cold Start를 줄이는 방법을 설명하세요.
...
### Q. Readiness Probe와 Liveness Probe의 차이점은?
...
### Q. HPA(Horizontal Pod Autoscaler) 동작 원리는?
