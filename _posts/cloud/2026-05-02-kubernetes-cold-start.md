...
graph LR
    ...
    SP -->|"성공"| LP["livenessPro..|"성공"| RP["readinessPr..|"실패"| Restart["Contai..|"실패"| Remove["Endpoin..|"성공"| Traffic["트래픽 수신"]
```
...
sequenceDiagram
    ...
    S->>B: 트래픽 100%
    Note over G: 배포 + Warm-up
    G->>G: readiness 통과
    S->>G: 트래픽 전환
    B-->>S: graceful shutdown
```
...
graph LR
    ...
    B -->|"이미지"| ImageOpt["멀티스테이..|"JVM"| JvmOpt["GraalVM..|"Probe"| ProbeOpt["start..|"트래픽 예측"| Cron["CronHPA"]
