...
Module이 필요한 이유는 **복붙(copy-paste)의 저주** 때문이다. dev, staging, prod 환경마다 동일한 VPC 구성을 복사해서 쓰다 보면, 보안 그룹 규칙 하나를 바꿀 때 세 곳을 모두 수정해야 한다. 하나를 빠뜨리면 환경 간 불일치가 생기고, 불일치는 "staging에서는 됐는데 prod에서 안 된다"는 유형의 버그를 만든다.

Module은 인프라 코드의 함수다. 레고 블록 설계도를 한 번만 그려두고, 각 환경에서는 "이 설계도로 파란색으로 만들어", "이 설계도로 빨간색으로 만들어"처럼 변수만 바꿔 호출한다. 같은 패턴을 반복하지 않고 한 번 정의해서 여러 곳에서 호출한다.
...
<details>
<summary>Q. Terraform State 파일의 역할과 Remote Backend가 필요한 이유</summary>
...
<details>
<summary>Q. terraform plan과 apply의 차이 및 CI/CD에서의 역할</summary>
...
<details>
<summary>Q. Module을 사용하는 이유와 설계 원칙</summary>
...
<details>
<summary>Q. State Drift 감지와 대응 방법</summary>
...
<details>
<summary>Q. IaC를 사용할 때 보안 고려사항</summary>
