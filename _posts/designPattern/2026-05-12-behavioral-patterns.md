...
sequenceDiagram
    ...
    Note over Client,ConcreteStrategy: Strategy 패턴 흐름
    Client->>Context: setStrategy(new VipDiscount())
    Client->>Context: applyDiscount(10000)
    Context->>ConcreteStrategy: calculate(10000)
    ConcreteStrategy-->>Context: 8000
    Context-->>Client: 8000

    Note over Subject,Observer: Observer 패턴 흐름
    Client->>Subject: subscribe(observer)
    Client->>Subject: setState(COMPLETED)
    Subject->>Observer: onEvent(OrderCompletedEvent)
    Observer-->>Subject: (처리 완료)
```
...
sequenceDiagram
    ...
    Note over Client,Receiver: Command 패턴 — Undo/Redo 흐름
    Client->>Invoker: execute(InsertTextCommand)
    Invoker->>Command: execute()
    Command->>Receiver: insertText(0, "Hello")
    Receiver-->>Command: 완료
    Invoker->>Invoker: undoStack.push(command)
